...
test.describe("Milestone Workflow E2E Tests", () => {
	...
	test.beforeEach(async () => {
		...
		await page.goto(
			`/app/${testProject.organizationId}/projects/${testProject.id}/components`,
		);
	...
	test.describe("Discrete Milestone Workflow", () => {
		test("should complete discrete milestone with checkbox", async () => {
			...
			const componentRow = page.locator(
				`[data-component-id="${testComponents[0].componentId}"]`,
			);
			await expect(componentRow).toBeVisible();

			// Find milestone checkbox
			const milestoneCheckbox = componentRow
			...
			const refreshedCheckbox = componentRow
				.locator('[data-testid="milestone-checkbox"]')
				.first();
		...
		test("should show loading state during update", async () => {
			...
			await expect(
				componentRow.locator('[data-testid="milestone-loading"]'),
			).toBeVisible();
	...
	test.describe("Percentage Milestone Workflow", () => {
		test("should update percentage milestone with slider", async () => {
			const componentRow = page.locator(
				`[data-component-id="${testComponents[1].componentId}"]`,
			);
			await expect(componentRow).toBeVisible();

			// Open milestone details
			await componentRow
				.locator('[data-testid="milestone-details-button"]')
				.click();

			// Wait for milestone panel to open
			await expect(
				page.locator('[data-testid="milestone-panel"]'),
			).toBeVisible();

			// Find percentage slider
			const percentageSlider = page.locator(
				'[data-testid="milestone-percentage-slider"]',
			);
			...
			await expect(
				page.locator('[data-testid="percentage-display"]'),
			).toContainText("75%");

			// Should not be marked as complete (< 100%)
			await expect(
				page.locator('[data-testid="milestone-completed-indicator"]'),
			).not.toBeVisible();
		});
		...
		test("should support percentage input field", async () => {
			...
			await expect(
				page.locator('[data-testid="milestone-percentage-slider"]'),
			).toHaveValue("85");
		});
	...
	test.describe("Quantity Milestone Workflow", () => {
		test("should update quantity milestone", async () => {
			const componentRow = page.locator(
				`[data-component-id="${testComponents[2].componentId}"]`,
			);
			await componentRow
				.locator('[data-testid="milestone-details-button"]')
				.click();
			...
			await expect(
				page.locator('[data-testid="quantity-progress"]'),
			).toContainText(`${halfQuantity} / ${totalQuantity}`);
		...
		test("should mark quantity milestone complete when quantity equals total", async () => {
			...
			await expect(
				page.locator('[data-testid="quantity-progress"]'),
			).toContainText(`${totalQuantity} / ${totalQuantity}`);
		});
		...
		test("should validate quantity bounds", async () => {
			...
			await expect(
				page.locator('[data-testid="validation-error"]'),
			).toContainText("cannot be negative");
		});
	});
	...
	test.describe("Bulk Milestone Operations", () => {
		test("should select multiple components for bulk update", async () => {
			...
			await page
				.locator(
					`[data-component-id="${testComponents[0].componentId}"] [data-testid="component-select"]`,
				)
				.click();
			await page
				.locator(
					`[data-component-id="${testComponents[1].componentId}"] [data-testid="component-select"]`,
				)
				.click();
			...
			).toContainText("2 components selected");
			...
			await expect(
				page.locator('[data-testid="bulk-update-modal"]'),
			).toBeVisible();
		});
		...
		test("should preview bulk changes before applying", async () => {
			...
			await page
				.locator('[data-testid="milestone-select"]')
				.selectOption("Design Review");
			await page.locator('[data-testid="update-type-discrete"]').click();
			await page
				.locator('[data-testid="discrete-value-complete"]')
				.click();

			// Click preview button
			await page
				.locator('[data-testid="preview-changes-button"]')
				.click();

			// Preview should show
			await expect(
				page.locator('[data-testid="bulk-preview"]'),
			).toBeVisible();
			await expect(
				page.locator('[data-testid="preview-changes-count"]'),
			).toContainText("2 changes");
		...
		test("should execute bulk update with progress tracking", async () => {
			...
			await expect(
				page.locator('[data-testid="bulk-progress-bar"]'),
			).toBeVisible();

			// Wait for completion
			await expect(
				page.locator('[data-testid="bulk-update-success"]'),
			).toBeVisible({ timeout: 10000 });
			await expect(
				page.locator('[data-testid="bulk-update-success"]'),
			).toContainText("2 milestones updated");
		});

		test("should handle bulk update errors gracefully", async () => {
			// Mock API error for bulk update
			await page.route(
				"**/api/pipetrak/milestones/bulk-update",
				(route) => {
					route.fulfill({
						status: 500,
			...
			await expect(
				page.locator('[data-testid="bulk-update-error"]'),
			).toBeVisible();
			await expect(
				page.locator('[data-testid="bulk-update-error"]'),
			).toContainText("Failed to update milestones");
		});
		...
		test("should support undo bulk operations", async () => {
			...
			await expect(
				page.locator('[data-testid="undo-bulk-update"]'),
			).toBeVisible();
			...
			// Should confirm undo
			await expect(
				page.locator('[data-testid="undo-confirmation"]'),
			).toBeVisible();
			await page.locator('[data-testid="confirm-undo"]').click();

			// Should show undo success
			await expect(
				page.locator('[data-testid="undo-success"]'),
			).toBeVisible();
		});
	});
	...
	test.describe("Real-time Features", () => {
		test("should show other users editing the same component", async () => {
			...
			try {
				...
				const secondUserComponentRow = secondPage.locator(
					`[data-component-id="${testComponents[0].componentId}"]`,
				);
				await expect(
					secondUserComponentRow.locator(
						'[data-testid="user-editing-indicator"]',
					),
		...
		test("should sync milestone updates across users", async () => {
			...
			try {
				...
				const secondUserCheckbox = secondUserComponentRow
					.locator('[data-testid="milestone-checkbox"]')
					.first();
		...
		test("should handle conflict resolution", async () => {
			...
			await expect(
				page.locator('[data-testid="server-version"]'),
			).toContainText("Completed");
			await expect(
				page.locator('[data-testid="client-version"]'),
			).toContainText("Not Completed");
	...
	test.describe("Offline Support", () => {
		test("should queue updates when offline", async () => {
			...
			await expect(
				page.locator('[data-testid="queued-updates-badge"]'),
			).toContainText("1");
		...
		test("should sync queued updates when coming back online", async () => {
			...
			await expect(
				page.locator('[data-testid="sync-success-notification"]'),
			).toBeVisible();
		});
		...
		test("should handle offline sync errors", async () => {
			...
			await expect(
				page.locator('[data-testid="sync-error-notification"]'),
			).toBeVisible();
	...
	test.describe("Mobile Responsiveness", () => {
		...
		test("should show mobile-optimized milestone interface", async () => {
			...
			await expect(
				page.locator('[data-testid="milestone-bottom-sheet"]'),
			).toBeVisible();
		...
		test("should support swipe gestures for milestone updates", async () => {
			...
			await expect(
				page.locator('[data-testid="swipe-complete-action"]'),
			).toBeVisible();
		...
		test("should handle mobile bulk selection", async () => {
			...
			await expect(
				page.locator('[data-testid="selection-mode-active"]'),
			).toBeVisible();
			...
			await expect(
				page.locator('[data-testid="mobile-bulk-action-bar"]'),
			).toBeVisible();
		});
	});
	...
	test.describe("Performance and Scale", () => {
		test("should handle large component lists efficiently", async () => {
			...
			await page.goto(
				`/app/${testProject.organizationId}/projects/large-dataset/components`,
			);
			...
			await expect(
				page.locator('[data-testid="virtualized-items"]'),
			).toHaveCount(50, { timeout: 2000 });
		});
		...
		test("should handle bulk operations on large datasets", async () => {
			...
			await expect(
				page.locator('[data-testid="selected-count"]'),
			).toContainText(/\d+ components selected/);
	...
	test.describe("Error Handling and Recovery", () => {
		test("should recover from network errors", async () => {
			...
			await expect(
				page.locator('[data-testid="update-error"]'),
			).toBeVisible();
		...
		test("should handle component not found errors", async () => {
			...
			await expect(
				page.locator('[data-testid="milestone-not-found-error"]'),
			).toBeVisible();
