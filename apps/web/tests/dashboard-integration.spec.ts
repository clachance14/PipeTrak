...
test.describe("Dashboard Integration", () => {
	test.describe("Data Loading Flow", () => {
		...
		test("loads dashboard with empty project data", async ({ page }) => {
			...
			await expect(
				page.locator("text=No area/system data available"),
			).toBeVisible();
			await expect(
				page.locator("text=No drawings with components found"),
			).toBeVisible();
			await expect(
				page.locator("text=No test packages configured"),
			).toBeVisible();
	...
	test.describe("Error Handling", () => {
		test("displays error state when API fails", async ({ page }) => {
			...
			await expect(
				page.locator("text=Please try refreshing the page"),
			).toBeVisible();
		});
	...
	test.describe("Interactive Components", () => {
		test("area system grid cell click opens drill-down", async ({
			page,
		}) => {
			await page.goto("/app/pipetrak/small-project/dashboard");
			...
			await expect(
				page.locator("text=Area-01 × System-01"),
			).toBeVisible();
		...
		test("drawing hierarchy expand/collapse works", async ({ page }) => {
			...
			const parentDrawing = page
				.locator("text=DWG-001")
				.locator("..")
		...
		test("test package table sorting works", async ({ page }) => {
			...
			const firstPackage = page
				.locator('[data-testid="test-package-row"]')
				.first();
	...
	test.describe("Manual Refresh", () => {
		test("refresh button updates data", async ({ page }) => {
			...
			const originalTimestamp = await page
		...
		test("auto-refresh updates data periodically", async ({ page }) => {
			await page.goto(
				"/app/pipetrak/small-project/dashboard?autoRefresh=true",
			);
	...
	test.describe("Responsive Behavior", () => {
		test("switches to tablet layout on medium screen", async ({ page }) => {
			...
			await expect(
				page.locator('[data-testid="tablet-dashboard"]'),
			).toBeVisible();
	...
	test.describe("Project Switching", () => {
		test("project selector works correctly", async ({ page }) => {
			...
			await expect(
				page.locator("text=Large Test Project Dashboard"),
			).toBeVisible();
		});
	...
	test.describe("Search and FileFiltering", () => {
		test("component search works across dashboard", async ({ page }) => {
			...
			// Use global search
			await page
				.locator('[data-testid="global-search"]')
				.fill("VALVE-001");
	...
	test.describe("Accessibility", () => {
		...
		test("screen reader announcements work correctly", async ({ page }) => {
			...
			await expect(page.locator("[aria-describedby]")).toHaveCount({
				min: 1,
			});
