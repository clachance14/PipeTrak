...
export async function setupLargeDataset(
	projectId: string,
	componentCount = 1000,
): Promise<void> {
	const milestoneTemplateId = randomUUID();
  ...
  `,
	...
	for (
		let batch = 0;
		batch < Math.ceil(componentCount / batchSize);
		batch++
	) {
...
export async function cleanupTestProject(projectId: string): Promise<void> {
	try {
		...
		await executeSQL(
			'DELETE FROM "ComponentMilestone" WHERE "componentId" IN (SELECT id FROM "Component" WHERE "projectId" = $1)',
			[projectId],
		);
		await executeSQL('DELETE FROM "Component" WHERE "projectId" = $1', [
			projectId,
		]);
		await executeSQL('DELETE FROM "Drawing" WHERE "projectId" = $1', [
			projectId,
		]);
		await executeSQL('DELETE FROM "AuditLog" WHERE "projectId" = $1', [
			projectId,
		]);
...
export async function cleanupTestUser(user: TestUser): Promise<void> {
	try {
		...
		await executeSQL('DELETE FROM "Organization" WHERE id = $1', [
			user.organizationId,
		]);
	} catch (error) {
...
export async function seedMilestoneTestData(): Promise<void> {
	...
	try {
		...
		const existingOrg = await executeSQL(
...
export { executeSQL, type TestUser, type TestProject, type TestComponent };
