...
test.describe("Dashboard E2E Tests", () => {
	...
	test.describe("Dashboard Loading and Navigation", () => {
		...
		test("displays correct project information", async ({ page }) => {
			...
			await expect(projectSelector).toContainText(
		...
		test("shows last updated timestamp", async ({ page }) => {
			...
			const now = new Date();
			const timestampMatch = timestampText?.match(
	...
	test.describe("KPI Metrics Display", () => {
		...
		test("KPI cards are responsive on different screen sizes", async ({
			page,
		}) => {
			await setupTestProject(page);
	...
	test.describe("Area System Grid3x3 Interactions", () => {
		test("grid displays with correct structure", async ({ page }) => {
			...
			await expect(
				page.locator('[data-testid="area-system-grid"]'),
			).toBeVisible();
			...
			await expect(page.locator("text=/Area-\d+/")).toHaveCount({
				min: 1,
			});
			await expect(page.locator("text=/System-\d+/")).toHaveCount({
				min: 1,
			});
		});
		...
		test("clicking cell opens drill-down sheet", async ({ page }) => {
			...
			await expect(
				page.locator('[data-testid="drill-down-sheet"]'),
			).toBeVisible();
		...
		test("grid shows color coding for completion levels", async ({
			page,
		}) => {
			await setupTestProject(page);
			...
			await expect(
				page.locator('text="Has stalled components"'),
			).toBeVisible();
		});
	});
	...
	test.describe("Drawing Hierarchy Navigation", () => {
		test("displays drawing tree structure", async ({ page }) => {
			...
			await expect(page.locator("text=/DWG-\w+/")).toHaveCount({
				min: 1,
			});

			// Should show completion percentages
			await expect(page.locator("text=/\d+% complete/")).toHaveCount({
				min: 1,
			});
		});
		...
		test("expanding parent shows child drawings", async ({ page }) => {
			...
			await expect(
				page.locator('[data-testid="child-drawing"]'),
			).toBeVisible();
		});

		test("clicking drawing navigates to drawing detail", async ({
			page,
		}) => {
			await setupTestProject(page);

			// Click on first drawing
			const firstDrawing = page
				.locator('[data-testid="drawing-link"]')
				.first();
	...
	test.describe("Test Package Management", () => {
		test("displays test package table correctly", async ({ page }) => {
			...
			const packageTable = page.locator(
		...
		test("sorting by completion percentage works", async ({ page }) => {
			...
			const firstRowBefore = await page
				.locator('[data-testid="package-row"]')
				.first()
				.textContent();
		...
		test("ready badge displays correctly", async ({ page }) => {
			...
			const notReadyRows = page
				.locator('[data-testid="package-row"]')
				.filter({ hasText: "Not Ready" });
	...
	test.describe("Activity Feed", () => {
		test("displays recent activity correctly", async ({ page }) => {
			...
			await expect(
				page.locator("text=/\d+ hours? ago|\d+ minutes? ago|Just now/"),
			).toBeVisible();
			await expect(page.locator('[data-testid="user-name"]')).toHaveCount(
				{ min: 1 },
			);
		});
		...
		test("filtering by user works", async ({ page }) => {
			...
			const userOption = page
				.locator('[data-testid="user-option"]')
				.first();
			...
			for (let i = 0; i < itemCount; i++) {
				await expect(activityItems.nth(i)).toContainText(
					userName || "",
				);
			}
		});
	...
	test.describe("Manual Refresh Functionality", () => {
		...
		test("refresh handles errors gracefully", async ({ page }) => {
			...
			await expect(
				page.locator('text="Unable to load dashboard metrics"'),
			).toBeVisible();
		});
	});
