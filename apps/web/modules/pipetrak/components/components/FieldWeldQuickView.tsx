...
export function FieldWeldQuickView({ 
  ...
  if (component.type !== 'FIELD_WELD' || !component.fieldWelds || component.fieldWelds.length === 0) {
    return <>{children}</>;
  }
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  onClick={() => {
                    window.open(`/app/${organizationSlug}/pipetrak/${projectId}/qc/field-welds`, '_blank');
                  }}
                >
              ...
              <div className="text-sm text-muted-foreground">
                Weld ID: <span className="font-mono font-medium">{fieldWeld.weldIdNumber}</span>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  ...
                  <div className="font-medium">{fieldWeld.weldSize}</div>
                </div>
                <div>
                  <span className="text-muted-foreground">Schedule:</span>
                  <div className="font-medium">{fieldWeld.schedule}</div>
                </div>
                <div>
                  <span className="text-muted-foreground">Type:</span>
                  <div className="font-medium">{fieldWeld.weldType.code}</div>
                </div>
                <div>
                  <span className="text-muted-foreground">Date:</span>
                  <div className="font-medium">
                    {format(new Date(fieldWeld.dateWelded), 'MMM dd, yyyy')}
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">NDE Result:</span>
                  {getNdeResultBadge(fieldWeld.ndeResult)}
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">PWHT:</span>
              ...
              {fieldWeld.welder && (
                <div className="flex items-center gap-2 p-2 bg-muted/50 rounded-md">
                  ...
                  <div className="text-sm">
                    ...
                    <div className="text-muted-foreground">{fieldWeld.welder.name}</div>
                  </div>
                </div>
              )}
