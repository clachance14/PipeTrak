...
import type { ComponentWithMilestones, ComponentMilestone, WorkflowType } from "../../types";
...
export function MilestoneUpdatePanel({ 
  ...
  const totalCredits = milestones.reduce((sum, m) => sum + (m.creditWeight || 0), 0);
  const earnedCredits = milestones.reduce((sum, m) => {
    if (workflowType === "MILESTONE_DISCRETE") {
      ...
      return sum + ((m.percentageComplete || 0) / 100) * (m.creditWeight || 0);
    } else {
      const percent = (m.quantityTotal || 0) > 0 
  ...
  const overallProgress = totalCredits > 0 
  ...
  const isMilestoneLocked = (milestone: ComponentMilestone): boolean => {
    ...
    if (!milestone.dependencies || milestone.dependencies.length === 0) {
      ...
      if (workflowType === "MILESTONE_DISCRETE") {
        ...
        return (previousMilestone.quantityComplete || 0) < (previousMilestone.quantityTotal || 0);
      }
    }
  ...
  const handleMilestoneUpdate = async (milestoneId: string, value: number | boolean) => {
  ...
  const handleSaveAll = async () => {
    ...
    startTransition(async () => {
      try {
        ...
        const updates = Array.from(pendingUpdates.entries()).map(([milestoneId, value]) => ({
          milestoneId,
          value,
          workflowType
        }));
        ...
        const response = await apiClient.pipetrak.milestones["bulk-update"].$post({
          json: {
            updates: Array.from(pendingUpdates.entries()).map(([milestoneId, value]) => {
              const milestone = milestones.find(m => m.id === milestoneId);
              return {
                componentId: component.id,
                milestoneName: milestone?.milestoneName || '',
                isCompleted: workflowType === "MILESTONE_DISCRETE" ? value as boolean : undefined,
                percentageValue: workflowType === "MILESTONE_PERCENTAGE" ? value as number : undefined,
                quantityValue: workflowType === "MILESTONE_QUANTITY" ? value as number : undefined,
              };
            })
          }
        });
        ...
        toast.success(`${updates.length} milestone${updates.length > 1 ? 's' : ''} updated successfully`);
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Badge 
              ...
              className={cn(
                "text-sm",
                overallProgress === 100 && "bg-fieldComplete/10 text-fieldComplete border-fieldComplete/20",
                overallProgress > 0 && overallProgress < 100 && "bg-blue-100 text-blue-700 border-blue-200",
                overallProgress === 0 && "bg-fieldPending/10 text-fieldPending border-fieldPending/20"
              )}
            >
              {overallProgress === 100 ? (
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-medium">
                {earnedCredits.toFixed(1)} / {totalCredits} credits
              </span>
            </div>
        ...
        {milestones
          ...
          .map((milestone) => (
            <MilestoneUpdateCard
              ...
              onUpdate={(value) => handleMilestoneUpdate(milestone.id, value)}
          ...
          <CardContent className="pt-6">
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div className="space-y-1">
                ...
                <p className="font-medium">{component.area || "N/A"}</p>
              </div>
              <div className="space-y-1">
                <p className="text-muted-foreground">System</p>
                <p className="font-medium">{component.system || "N/A"}</p>
              </div>
            </div>
          </CardContent>
        </Card>
