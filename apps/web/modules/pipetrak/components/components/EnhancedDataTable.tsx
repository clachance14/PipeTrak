		...
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (e.key === "Escape") {
				...
				const currentColIndex = columns.findIndex(
					(c) => c.key === editingCell?.columnKey,
				);
				const nextEditableCol = columns
					.slice(currentColIndex + 1)
					.find((c) => c.editable !== false);
							...
							{columns.map((column) => (
								<TableHead
									...
									style={{
										...
										left: pinnedColumns.has(column.key)
											? 0
											: undefined,
										position: pinnedColumns.has(column.key)
											? "sticky"
											: undefined,
										background: pinnedColumns.has(
											column.key,
										)
											? "inherit"
											: undefined,
										zIndex: pinnedColumns.has(column.key)
											? 11
											: undefined,
									}}
									className={cn(
