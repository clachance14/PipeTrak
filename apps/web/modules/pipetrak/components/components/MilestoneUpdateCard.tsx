...
export function MilestoneUpdateCard({ 
  ...
  const renderDiscreteWorkflow = () => {
    const isChecked = tempValue !== null ? tempValue as boolean : milestone.isCompleted;
  ...
  const renderPercentageWorkflow = () => {
    const currentValue = tempValue !== null 
      ? tempValue as number 
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <div className="space-y-3">
            <Slider
              value={[currentValue]}
              onValueChange={(value) => !isLocked && setTempValue(value[0])}
              max={100}
              step={5}
              disabled={isLocked}
              className={cn(
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Input
                  type="number"
                  value={currentValue}
                  onChange={(e) => !isLocked && setTempValue(Math.min(100, Math.max(0, parseInt(e.target.value) || 0)))}
        ...
        {tempValue !== null && tempValue !== milestone.percentageComplete && (
          <div className="flex gap-2">
            <Button 
              onClick={handleUpdate} 
              disabled={isUpdating}
  ...
  const renderQuantityWorkflow = () => {
    ...
    const percent = total > 0 ? Math.round((currentValue / total) * 100) : 0;
    ...
    return (
      <div className="space-y-4">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <Input
                type="number"
                value={currentValue}
                onChange={(e) => !isLocked && setTempValue(Math.min(total, Math.max(0, parseFloat(e.target.value) || 0)))}
                className="flex-1"
                style={{ minHeight: `${touchTargetSize}px` }}
                disabled={isLocked}
                min={0}
                max={total}
                step={milestone.unit === "ft" || milestone.unit === "m" ? 0.1 : 1}
              />
            ...
            <div className="flex items-center justify-between text-sm text-muted-foreground">
              <span>{currentValue} of {total} {milestone.unit} complete</span>
        ...
        {tempValue !== null && tempValue !== milestone.quantityComplete && (
          <div className="flex gap-2">
            <Button 
              onClick={handleUpdate} 
              disabled={isUpdating}
  ...
  return (
    <Card className={cn(
        ...
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg flex items-center gap-2">
            <Badge variant="outline" className="text-xs">
              {milestone.milestoneOrder || milestone.sequenceNumber || 1} of {milestone.totalInWorkflow || milestone.milestoneOrder || 1}
            </Badge>
      ...
      <CardContent>
        {workflowType === "MILESTONE_DISCRETE" && renderDiscreteWorkflow()}
        {workflowType === "MILESTONE_PERCENTAGE" && renderPercentageWorkflow()}
        {workflowType === "MILESTONE_QUANTITY" && renderQuantityWorkflow()}
