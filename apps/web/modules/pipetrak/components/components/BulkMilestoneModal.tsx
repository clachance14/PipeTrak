...
export function BulkMilestoneModal({
	...
	const handleUpdate = async () => {
		...
		try {
					...
					groups: groups
						.map((group) => ({
							...
							milestones: Array.from(
								groupSelections[group.templateId] || [],
							),
						}))
			...
			setProgress({
				current: selectedComponents.length,
		...
		<Dialog open={isOpen} onOpenChange={handleClose}>
			<DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
				<DialogHeader>
					...
					<DialogDescription>
						Update milestones for {selectedComponents.length}{" "}
						selected component
						{selectedComponents.length !== 1 ? "s" : ""}
						{groups.length > 1 &&
							` (${groups.length} different types)`}
					</DialogDescription>
				</DialogHeader>
					...
					{isUpdating ? (
						...
						<div className="flex flex-col items-center justify-center py-12 space-y-4">
							...
							<div className="text-center">
								...
								<p className="text-sm text-muted-foreground">
									Processing {progress.current} of{" "}
									{progress.total} components
								</p>
							</div>
						...
						<div className="space-y-6 overflow-auto">
											...
											commonMilestones.length > 0 && (
												<Alert>
													...
													<AlertDescription>
														...
														Apply the same milestone
														to all component types.
														Only works for
														milestones that exist in
							...
							{((updateMode === "quick" && quickMilestone) ||
								(updateMode === "advanced" &&
									Object.values(groupSelections).some(
										(set) => set.size > 0,
									))) && (
								<Alert>
					...
					<DialogFooter>
						...
						<Button
							...
							disabled={
								(updateMode === "quick" && !quickMilestone) ||
								(updateMode === "advanced" &&
									Object.values(groupSelections).every(
										(set) => set.size === 0,
									))
							}
						>
					...
					onRetry={async (failedItems) => {
						try {
							...
							if (updateMode === "quick" && quickMilestone) {
								...
								retryUpdates.componentIds = failedItems.map(
									(f) => f.componentId,
								);
							} else if (updateMode === "advanced") {
									...
									if (failure.milestoneName) {
										...
										if (
											...
											if (
												...
												group.componentIds.push(
													failure.componentId,
												);
											}
											if (
												!group.milestones.includes(
								...
								retryUpdates.groups = Array.from(
									retryGroups.values(),
								);
							}
