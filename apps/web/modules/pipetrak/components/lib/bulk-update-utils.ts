 ...
 */
export function groupComponentsByTemplate(components: ComponentWithMilestones[]): ComponentGroup[] {
  ...
  const groups = Array.from(templateMap.entries()).map(([templateId, groupComponents]) => {
    ...
    return {
      ...
      templateId,
      templateName: firstComponent.milestoneTemplate?.name || 'Unknown Template',
 ...
 */
export function findCommonMilestones(components: ComponentWithMilestones[]): string[] {
  if (components.length === 0) return [];
  
  const milestoneArrays = components.map(comp => 
    comp.milestones?.map(m => m.milestoneName) || []
  );
...
export function generateBulkUpdatePayload(
  ...
  const updates: Array<{ componentId: string; milestoneName: string; type: string }> = [];
...
export function validateBulkUpdate(
  ...
  groups.forEach(group => {
    ...
    if (!selectedMilestones || selectedMilestones.size === 0) {
      validation.warnings.push(`No milestones selected for ${group.type} components (${group.components.length} components)`);
      return;
    }
    ...
    selectedMilestones.forEach(milestoneName => {
      if (!group.availableMilestones.includes(milestoneName)) {
        validation.errors.push(`Milestone "${milestoneName}" not found for ${group.type} components`);
  ...
  if (!hasSelections) {
    validation.errors.push('No milestones selected for any component group');
    validation.valid = false;
  }
...
export function applyComponentFilters(
  ...
  return components.filter(component => {
    // Area filter
    if (filters.area && filters.area !== 'all' && component.area !== filters.area) {
      return false;
    }
    
    // Test package filter
    if (filters.testPackage && filters.testPackage !== 'all' && component.testPackage !== filters.testPackage) {
      return false;
    }
    
    // System filter
    if (filters.system && filters.system !== 'all' && component.system !== filters.system) {
      return false;
    }
    
    // Type filter
    if (filters.type && filters.type !== 'all' && component.type !== filters.type) {
      return false;
    }
