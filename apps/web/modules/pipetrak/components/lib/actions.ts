...
export async function getComponents(projectId: string): Promise<ComponentWithMilestones[]> {
  try {
    ...
    const components = data.data.map((comp: any) => ({
      ...
      description: comp.description || `${comp.type || ""} ${comp.spec || ""} ${comp.size || ""}`.trim() || "-"
    }));
...
export async function updateComponentMilestone(
  ...
  try {
    ...
    const response = await fetch(`${baseUrl}/api/pipetrak/milestones/${milestoneId}`, {
      method: 'PATCH',
      headers: {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (errorData.error) {
          ...
          if (errorData.error.includes("not a member of organization")) {
            errorMessage += ". Please contact your administrator to be added to the organization.";
          }
        }
      } catch (e) {
...
export async function getComponentDetails(componentId: string): Promise<ComponentWithMilestones | null> {
  try {
    const apiClient = await getServerApiClient();
    ...
    return {
      ...
      description: data.description || `${data.type || ""} ${data.spec || ""} ${data.size || ""}`.trim() || "-"
    };
  } catch (error) {
...
export async function exportComponents(projectId: string, format: "csv" | "excel" = "csv"): Promise<Blob> {
  try {
    const apiClient = await getServerApiClient();
