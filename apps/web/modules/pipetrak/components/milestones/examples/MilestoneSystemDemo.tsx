...
export function MilestoneSystemDemo({
		...
		return {
			...
			preview:
				updates.updates?.map((u: any) => ({
					...
					currentValue: false,
					newValue:
						u.isCompleted || u.percentageValue || u.quantityValue,
					hasChanges: true,
				})) || [],
			invalid: [],
		};
	...
	const stats = {
		...
		completedComponents: components.filter(
			(c) => c.completionPercent === 100,
		).length,
		inProgressComponents: components.filter(
			(c) => c.completionPercent > 0 && c.completionPercent < 100,
		).length,
		pendingComponents: components.filter((c) => c.completionPercent === 0)
			.length,
		totalMilestones: components.reduce(
			(sum, c) => sum + (c.milestones?.length || 0),
			0,
		),
				...
				<div className="space-y-6">
					...
					<Tabs defaultValue="table" className="w-full">
						...
						<TabsContent value="table" className="space-y-4">
							<div className="flex items-center justify-between">
								<h3 className="text-lg font-semibold">
									Component Table with Inline Milestone
									Editing
								</h3>
								<Button
							...
							<div className="text-sm text-muted-foreground">
								• Click on milestone progress bars to edit
								inline • Select multiple components for bulk
								operations • Real-time updates from other users
								appear instantly
							</div>
						</TabsContent>
						...
						<TabsContent value="mobile" className="space-y-4">
							...
							<div className="text-sm text-muted-foreground">
								• 52px+ touch targets optimized for industrial
								gloves • Swipe gestures for quick milestone
								completion • Offline-first with automatic sync
								when connectivity returns • Bottom sheet
						...
						<TabsContent value="bulk" className="space-y-4">
							...
							<div className="text-sm text-muted-foreground">
								• Select multiple components to enable bulk
								operations • Configure different values for each
								milestone type (discrete, percentage, quantity)
								• Preview shows exactly which components and
								milestones will be affected • Operations are
								logged and can be undone if needed
							</div>
						</TabsContent>
						...
						<TabsContent value="realtime" className="space-y-4">
									...
									<CardContent className="text-sm">
										<ul className="space-y-2">
											<li className="flex items-center gap-2">
												...
												Instant milestone updates across
												all clients
											</li>
											<li className="flex items-center gap-2">
												<Zap className="h-4 w-4 text-blue-600" />
												Progress recalculation in
												real-time
											</li>
											<li className="flex items-center gap-2">
												<Zap className="h-4 w-4 text-blue-600" />
							...
							<div className="text-sm text-muted-foreground">
								• Uses Supabase Realtime for instant
								synchronization • WebSocket connections with
								automatic reconnection • Optimistic updates
								provide immediate feedback • Conflict resolution
