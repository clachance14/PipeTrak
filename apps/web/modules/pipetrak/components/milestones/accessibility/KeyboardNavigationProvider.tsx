...
export function KeyboardNavigationProvider({
	...
	const registerFocusable = useCallback(
		(id: string, element: HTMLElement, callbacks?: FocusCallbacks) => {
	...
	const setCurrentFocus = useCallback((id: string) => {
		...
		if (currentFocusId.current) {
			const prevElement = focusableElements.current.get(
				currentFocusId.current,
			)?.element;
			const prevCallbacks = focusableElements.current.get(
				currentFocusId.current,
			)?.callbacks;
			if (prevElement) {
				prevElement.blur();
	...
	const focusNext = useCallback(() => {
		...
		const currentIndex = currentFocusId.current
			? ids.indexOf(currentFocusId.current)
			: -1;
	...
	useEffect(() => {
		const handleKeyDown = (event: KeyboardEvent) => {
			...
			const callbacks = currentId
				? focusableElements.current.get(currentId)?.callbacks
				: null;
