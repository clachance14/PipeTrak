...
interface RealtimeUpdate {
  type: 'milestone_update' | 'bulk_milestone_update' | 'user_presence' | 'conflict_resolved' | 'bulk_operation_undone';
  payload: any;
  timestamp: string;
  userId: string;
}
...
export function RealtimeManager({
  ...
  const supabase = useRef(createClient(SUPABASE_URL, SUPABASE_ANON_KEY));
    ...
    switch (update.type) {
      case 'milestone_update':
        ...
        if (milestone) {
          ...
          queryClient.invalidateQueries({ queryKey: ['component-milestones', milestone.componentId] });
          
          // Show notification if update is from another user
          if (update.userId !== userId) {
            toast.info(`Milestone "${milestone.milestoneName}" updated by another user`);
          }
      ...
      case 'bulk_milestone_update':
        ...
        if (update.userId !== userId) {
          ...
          toast.info(`${updated} milestones updated by another user`);
        }
      ...
      case 'user_presence':
        ...
        if (presence.userId !== userId) {
          ...
          if (presence.action === 'editing_start' && presence.componentId) {
            toast.info(`Another user is editing component ${presence.componentId}`, {
      ...
      case 'conflict_resolved':
        const { milestoneId, componentId, resolution } = update.payload;
        
        // Refresh the affected milestone
        queryClient.invalidateQueries({ queryKey: ['milestone', milestoneId] });
        queryClient.invalidateQueries({ queryKey: ['component-milestones', componentId] });
        
        if (update.userId !== userId) {
          toast.info(`Conflict resolved for milestone using ${resolution} strategy`);
        }
      ...
      case 'bulk_operation_undone':
        ...
        if (update.userId !== userId) {
          ...
          toast.info(`Bulk operation with ${undone} changes was undone by another user`);
        }
        break;
    }
  }, [queryClient, projectId, userId, onMilestoneUpdate, onBulkUpdate, onPresenceUpdate, onConflictResolved]);

  // Broadcast presence update
  const broadcastPresence = useCallback(async (componentId: string, action: UserPresence['action']) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.current.channel(`project:${projectId}`)
      ...
      .on('broadcast', { event: 'bulk_milestone_update' }, ({ payload }) => {
        handleRealtimeUpdate({
      ...
      .on('broadcast', { event: 'bulk_operation_undone' }, ({ payload }) => {
        handleRealtimeUpdate({
  ...
  useEffect(() => {
    return () => {
      if (presenceRef.current && presenceRef.current.action === 'editing_start') {
        // Broadcast editing end when component unmounts
        broadcastPresence(presenceRef.current.componentId!, 'editing_end');
      }
    };
  }, [broadcastPresence]);
...
interface RealtimeContextValue {
  ...
  broadcastPresence: (componentId: string, action: 'editing_start' | 'editing_end' | 'viewing') => Promise<void>;
  isConnected: boolean;
}
