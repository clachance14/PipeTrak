...
export function MilestoneQuantityRenderer({ 
  ...
  const currentValue = optimisticValue !== null ? optimisticValue : (milestone.quantityComplete || 0);
  const totalQuantity = milestone.quantityTotal || 0;
  const unit = milestone.unit || "units";
  const percent = totalQuantity > 0 ? Math.round((currentValue / totalQuantity) * 100) : 0;
  const hasChanges = optimisticValue !== null && optimisticValue !== (milestone.quantityComplete || 0);
  ...
  const handleInputChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isLocked && !isUpdating) {
      ...
      const clampedValue = Math.min(totalQuantity, Math.max(0, numValue));
  ...
  const handleStepChange = useCallback((delta: number) => {
    if (!isLocked && !isUpdating) {
      const newValue = Math.min(totalQuantity, Math.max(0, currentValue + delta));
  ...
  const getStatusIcon = () => {
    ...
    if (isUpdating || isLoading) {
      return <Loader2 className="h-5 w-5 text-blue-600 animate-spin" aria-label="Updating" />;
    }
    
    if (percent === 100) {
      return <CheckCircle2 className="h-5 w-5 text-fieldComplete" aria-label="Completed" />;
    } else if (percent > 0) {
      return <Clock className="h-5 w-5 text-blue-600" aria-label="In Progress" />;
    } else {
      return <AlertCircle className="h-5 w-5 text-fieldPending" aria-label="Not Started" />;
    }
  };
  ...
  return (
    <div className={cn("space-y-4", className)}>
          ...
          {milestone.sequenceNumber && (
            <div className="flex items-center gap-2 mt-1">
              <Badge variant="outline" className="text-xs">
                Step {milestone.sequenceNumber} of {milestone.totalInWorkflow || 1}
              </Badge>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <Button
            ...
            style={{ minHeight: `${Math.min(touchTargetSize, 48)}px`, minWidth: `${Math.min(touchTargetSize, 48)}px` }}
          ...
          <div className="flex-1 space-y-2">
            <Input
              ...
              className={cn(
                ...
                hasChanges && "border-blue-500 ring-2 ring-blue-200"
              )}
          ...
          <Button
            ...
            size="sm"
            onClick={() => handleStepChange(stepSize)}
            disabled={isLocked || isUpdating || currentValue >= totalQuantity}
            className="h-12 w-12 p-0"
            style={{ minHeight: `${Math.min(touchTargetSize, 48)}px`, minWidth: `${Math.min(touchTargetSize, 48)}px` }}
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between text-sm text-muted-foreground">
            <span>
              {formatValue(currentValue)} of {formatValue(totalQuantity)} {unit} complete
            </span>
          ...
          <div className="flex gap-2">
            {[1, 5, 10].map(increment => (
              <Button
                ...
                disabled={isLocked || isUpdating || currentValue + increment > totalQuantity}
                className="text-xs"
      ...
      {hasChanges && (
        <div className="flex gap-2">
          <Button 
            ...
            disabled={isUpdating}
            className="flex-1"
            style={{ minHeight: `${Math.min(touchTargetSize, 44)}px` }}
      ...
      {/* Completion metadata */}
      {milestone.completedAt && milestone.completedBy && percent === 100 && (
        <div className="text-xs text-muted-foreground">
          Completed {new Date(milestone.completedAt).toLocaleDateString()} 
          {milestone.completer && (
            <> by {milestone.completer.name || milestone.completer.email}</>
          )}
        </div>
      )}
    </div>
