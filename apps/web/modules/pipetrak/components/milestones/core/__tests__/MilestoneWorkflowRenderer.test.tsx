...
vi.mock('../MilestonePercentageRenderer', () => ({
  MilestonePercentageRenderer: ({ milestone, onUpdate, disabled }: any) => (
    <div data-testid="percentage-renderer">
      ...
      />
      <span data-testid="percentage-value">{milestone.percentageComplete || 0}%</span>
    </div>
  )
}));
  ...
  MilestoneQuantityRenderer: ({ milestone, onUpdate, disabled }: any) => (
    <div data-testid="quantity-renderer">
      ...
      <span data-testid="quantity-display">
        {milestone.quantityComplete || 0} / {milestone.quantityTotal || 0}
      </span>
    </div>
  )
}));
...
describe('MilestoneWorkflowRenderer', () => {
  ...
  describe('Workflow Type Detection', () => {
    it('should render discrete milestone for MILESTONE_DISCRETE workflow', () => {
      ...
      expect(screen.queryByTestId('percentage-renderer')).not.toBeInTheDocument();
      expect(screen.queryByTestId('quantity-renderer')).not.toBeInTheDocument();
    });
    ...
    it('should render percentage milestone for MILESTONE_PERCENTAGE workflow', () => {
      ...
      expect(screen.getByTestId('percentage-renderer')).toBeInTheDocument();
      expect(screen.getByTestId('percentage-slider')).toBeInTheDocument();
      expect(screen.queryByTestId('discrete-renderer')).not.toBeInTheDocument();
      expect(screen.queryByTestId('quantity-renderer')).not.toBeInTheDocument();
    });
    ...
    it('should render error state for unknown workflow type', () => {
      ...
      render(
        <MilestoneWorkflowRenderer
          milestone={milestone}
          workflowType="UNKNOWN_TYPE" as any
          onUpdate={onUpdate}
        />
      );

      expect(screen.getByText(/unknown workflow type/i)).toBeInTheDocument();
  ...
  describe('Percentage Milestone Interactions', () => {
    ...
    it('should display current percentage value', () => {
      const milestone = createPercentageMilestone({ percentageComplete: 60 });
      const onUpdate = vi.fn();
      ...
      expect(screen.getByTestId('percentage-value')).toHaveTextContent('60%');
    });

    it('should handle zero percentage', () => {
      const milestone = createPercentageMilestone({ percentageComplete: 0 });
      const onUpdate = vi.fn();
      ...
      expect(screen.getByTestId('percentage-value')).toHaveTextContent('0%');
      expect(screen.getByTestId('percentage-slider')).toHaveValue('0');
    });

    it('should handle null percentage as zero', () => {
      const milestone = createPercentageMilestone({ percentageComplete: null });
      const onUpdate = vi.fn();
  ...
  describe('Quantity Milestone Interactions', () => {
    ...
    it('should display quantity progress', () => {
      ...
      expect(screen.getByTestId('quantity-display')).toHaveTextContent('8 / 12');
    });
    ...
    it('should handle zero quantity', () => {
      ...
      expect(screen.getByTestId('quantity-display')).toHaveTextContent('0 / 5');
    ...
    it('should handle null quantity values', () => {
      ...
      expect(screen.getByTestId('quantity-display')).toHaveTextContent('0 / 0');
      expect(screen.getByTestId('quantity-input')).toHaveValue(0);
    });
  });
