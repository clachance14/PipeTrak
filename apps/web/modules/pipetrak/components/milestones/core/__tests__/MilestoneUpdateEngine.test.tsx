...
import { MilestoneUpdateEngine, useMilestoneUpdateEngine, useMilestone, useComponentMilestones } from '../MilestoneUpdateEngine';
...
function TestEngineConsumer({ projectId }: { projectId: string }) {
  ...
  return (
    <div>
      ...
      <button 
        onClick={engine.syncOfflineQueue}
...
function TestMilestoneConsumer({ milestoneId }: { milestoneId: string }) {
  const { milestone, hasPendingUpdates, operationStatus, update } = useMilestone(milestoneId);
  ...
  return (
    <div>
      ...
      <button 
        onClick={() => update('component-1', 'Test Milestone', 'MILESTONE_DISCRETE', true)}
...
function TestComponentMilestonesConsumer({ componentId }: { componentId: string }) {
  const { milestones, hasPendingUpdates, updateMilestone } = useComponentMilestones(componentId);
  
  return (
    <div>
      <div data-testid="milestones-count">
        {milestones.length}
      </div>
      ...
      <button 
        onClick={() => updateMilestone('milestone-1', 'Test Milestone', 'MILESTONE_DISCRETE', true)}
...
describe('MilestoneUpdateEngine', () => {
  ...
  const renderWithEngine = (component: React.ReactNode, projectId = 'test-project') => {
    return render(
  ...
  describe('Provider Setup', () => {
    ...
    it('should throw error when used outside provider', () => {
      ...
      }).toThrow('useMilestoneUpdateEngine must be used within a MilestoneUpdateEngine');
      ...
      server.use(
        http.post(`${API_BASE}/milestones/bulk-update`, () => {
          return HttpResponse.json({
            ...
            results: [
              { componentId: 'component-1', milestoneName: 'Test Milestone 1', success: true },
              { componentId: 'component-1', milestoneName: 'Test Milestone 2', success: true }
            ]
          });
        })
      );
  ...
  describe('useMilestone Hook', () => {
    it('should provide milestone-specific operations', () => {
      renderWithEngine(<TestMilestoneConsumer milestoneId="milestone-1" />);
      
      expect(screen.getByTestId('milestone-completed')).toHaveTextContent('not-completed');
      expect(screen.getByTestId('pending-updates')).toHaveTextContent('no-pending');
      expect(screen.getByTestId('operation-status')).toHaveTextContent('none');
    });
  ...
  describe('useComponentMilestones Hook', () => {
    ...
    it('should provide component-specific milestone operations', async () => {
      renderWithEngine(<TestComponentMilestonesConsumer componentId="component-1" />);
      
      // Wait for data to load
      await waitFor(() => {
        expect(screen.getByTestId('milestones-count')).toHaveTextContent('3'); // mockMilestones for component-1
      });
      
      expect(screen.getByTestId('component-pending')).toHaveTextContent('no-pending');
    });
    ...
    it('should update milestone through component hook', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('milestones-count')).not.toHaveTextContent('0');
      });
  ...
  describe('Real-time Features', () => {
    it('should subscribe to project updates', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Subscribing to project:', 'test-project');
    ...
    it('should unsubscribe on unmount', () => {
      ...
      const { unmount } = renderWithEngine(<TestEngineConsumer projectId="test-project" />);
      
      unmount();
      
      expect(consoleSpy).toHaveBeenCalledWith('Unsubscribed from project:', 'test-project');
  ...
  describe('Memory Management', () => {
    ...
    it('should handle rapid mount/unmount cycles', () => {
      for (let i = 0; i < 10; i++) {
        const { unmount } = renderWithEngine(<TestEngineConsumer projectId={`project-${i}`} />);
        unmount();
      }
