...
describe("OptimisticUpdateManager", () => {
	...
	describe("Optimistic Updates", () => {
		it("should apply optimistic update immediately", () => {
			...
			expect(manager.getOperationStatus(mockMilestone.id)).toBe(
				"pending",
			);
		});
	...
	describe("API Integration", () => {
		...
		it("should rollback failed update", async () => {
			...
			expect(
				manager.getMilestoneState(mockMilestone.id)?.isCompleted,
			).toBe(false);
	...
	describe("Offline Support", () => {
		...
		it("should restore offline queue from localStorage", () => {
			...
			expect(newManager.getOfflineQueue()).toHaveLength(
				testScenarios.offlineQueue.length,
			);
	...
	describe("State Management", () => {
		it("should return optimistic state over server state", () => {
			...
			expect(
				manager.getAllMilestoneStates().get(mockMilestone.id)
					?.isCompleted,
			).toBe(true);
		});
		...
		it("should clear all optimistic state", () => {
			...
			expect(
				manager.getMilestoneState(mockMilestone.id)?.isCompleted,
			).toBe(false); // Back to server state
		});
