...
export function MilestoneDiscreteRenderer({ 
  ...
  const currentValue = optimisticValue !== null ? optimisticValue : milestone.isCompleted;
  const hasChanges = optimisticValue !== null && optimisticValue !== milestone.isCompleted;
  ...
  const handleToggle = async (checked: boolean) => {
    ...
    try {
      ...
      window.announceError?.(`Failed to update milestone ${milestone.milestoneName}`);
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  const getStatusIcon = () => {
    ...
    return currentValue 
      ? <CheckCircle2 className="h-5 w-5 text-fieldComplete" aria-label="Completed" />
    ...
    <div className={cn("space-y-3", className)}>
      ...
      <div 
        className={cn(
          ...
          !isLocked && !isUpdating && "hover:bg-muted/50 cursor-pointer focus:ring-2 focus:ring-blue-500 focus:ring-offset-2",
          hasChanges && "border-blue-500 bg-blue-50/50",
          isUpdating && "bg-blue-50/30"
        )}
        style={{ minHeight: `${touchTargetSize}px` }}
        onClick={() => !isLocked && !isUpdating && handleToggle(!currentValue)}
        role="checkbox"
        ...
        aria-describedby={milestone.sequenceNumber ? `${milestone.id}-sequence` : undefined}
      >
        <Checkbox 
        ...
        <div className="flex-1 min-w-0">
          ...
          {milestone.sequenceNumber && (
            <div className="flex items-center gap-2 mt-1" id={`${milestone.id}-sequence`}>
              <Badge variant="outline" className="text-xs" role="img" aria-label={`Step ${milestone.sequenceNumber} of ${milestone.totalInWorkflow || 1}`}>
                Step {milestone.sequenceNumber} of {milestone.totalInWorkflow || 1}
              </Badge>
              {milestone.creditWeight && (
                <Badge variant="secondary" className="text-xs" role="img" aria-label={`Worth ${milestone.creditWeight} credits`}>
        ...
        <div className="flex gap-2 px-3">
          ...
          >
            {isUpdating ? (
              ...
              <>Save {optimisticValue ? 'Complete' : 'Incomplete'}</>
            )}
          </Button>
          <Button 
            variant="outline" 
