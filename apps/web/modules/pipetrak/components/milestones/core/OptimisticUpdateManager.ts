...
export interface UpdateCallback {
  ...
  onConflict?: (update: MilestoneUpdate, conflict: { local: ComponentMilestone; remote: ComponentMilestone }) => void;
}
...
export class OptimisticUpdateManager {
  ...
  applyOptimisticUpdate(update: MilestoneUpdate): ComponentMilestone {
    ...
    const currentMilestone = this.state.optimisticMilestones.get(update.milestoneId) ||
    ...
    const optimisticMilestone = this.applyUpdateToMilestone(currentMilestone, update);
    this.state.optimisticMilestones.set(update.milestoneId, optimisticMilestone);
  ...
  confirmUpdate(updateId: string, serverMilestone: ComponentMilestone): void {
    ...
    const optimisticMilestone = this.state.optimisticMilestones.get(update.milestoneId);
    if (optimisticMilestone && this.hasConflict(optimisticMilestone, serverMilestone)) {
  ...
  getMilestoneState(milestoneId: string): ComponentMilestone | undefined {
    return this.state.optimisticMilestones.get(milestoneId) ||
  ...
  hasPendingUpdates(milestoneId: string): boolean {
    return Array.from(this.state.pendingUpdates.values())
      .some(update => update.milestoneId === milestoneId);
  }
  ...
  updateServerState(milestones: ComponentMilestone[]): void {
    ...
    for (const [id, optimisticMilestone] of this.state.optimisticMilestones) {
      const serverMilestone = this.state.serverMilestones.get(id);
      if (serverMilestone && this.hasConflict(optimisticMilestone, serverMilestone)) {
        const update = Array.from(this.state.pendingUpdates.values())
  ...
  private async executeUpdate(update: MilestoneUpdate): Promise<void> {
    ...
    try {
      ...
      const response = await this.apiClient.patch(`/milestones/${update.milestoneId}`, payload);
  ...
  private applyUpdateToMilestone(milestone: ComponentMilestone, update: MilestoneUpdate): ComponentMilestone {
    ...
    switch (update.workflowType) {
      ...
      case "MILESTONE_QUANTITY":
        ...
        updated.isCompleted = (update.value as number) >= (milestone.quantityTotal || 0);
        updated.completedAt = updated.isCompleted ? now : null;
        break;
    }
   ...
   */
  private hasConflict(local: ComponentMilestone, remote: ComponentMilestone): boolean {
  ...
  private persistState(): void {
    ...
    try {
      ...
      localStorage.setItem('pipetrak_milestone_state', JSON.stringify(persistedState));
    } catch (error) {
