...
describe("Milestone Performance Tests", () => {
	...
	describe("OptimisticUpdateManager Performance", () => {
		it("should handle large numbers of optimistic updates efficiently", () => {
			...
			const endMeasure = performanceMeasurer.start(
		...
		it("should efficiently manage large offline queues", () => {
			...
			for (let i = 0; i < 500; i++) {
				manager.applyOptimisticUpdate({
					...
					milestoneName:
						milestones[i % milestones.length].milestoneName,
					workflowType: "MILESTONE_DISCRETE",
	...
	describe("Bulk Operations Performance", () => {
		it("should handle large bulk updates efficiently", async () => {
			...
			server.use(
				http.post(
					"http://localhost:3000/api/pipetrak/milestones/bulk-update",
					async ({ request }) => {
		...
		it("should efficiently batch process operations", async () => {
			...
			for (const batchSize of batchSizes) {
				for (const updateCount of updateCounts) {
						...
						http.post(
							...
							async ({ request }) => {
								...
								const batches = Math.ceil(
									updates.length / actualBatchSize,
								);
								const batchProcessingTime = batches * 100; // 100ms per batch

								await new Promise((resolve) =>
									setTimeout(resolve, batchProcessingTime),
								);
					...
					const stats = performanceMeasurer.getStats(
						`batch-${batchSize}-${updateCount}`,
					);
		...
		it("should handle concurrent bulk operations", async () => {
			...
			const promises = Array.from(
				...
				(_, i) => {
					const updates =
						generateMockBulkUpdates(updatesPerOperation);
					return fetch(
						"http://localhost:3000/api/pipetrak/milestones/bulk-update",
						{
							method: "POST",
	...
	describe("Memory Management Performance", () => {
		it("should not leak memory with large datasets", () => {
			...
			let manager: OptimisticUpdateManager | null =
				new OptimisticUpdateManager(mockApiClient);
			...
			for (let cycle = 0; cycle < 10; cycle++) {
				...
				const stateSize =
					manager.getAllMilestoneStates().size +
	...
	describe("Network Performance", () => {
		it("should handle slow network conditions gracefully", async () => {
			...
			server.use(
				http.patch(
					"http://localhost:3000/api/pipetrak/milestones/:id",
					async () => {
						await new Promise((resolve) =>
			...
			const response = await fetch(
				"http://localhost:3000/api/pipetrak/milestones/test-milestone",
				{
					method: "PATCH",
					headers: { "Content-Type": "application/json" },
	...
	describe("Performance Benchmarks", () => {
		it("should meet performance SLOs", async () => {
			...
			const singleUpdateP95 = singleUpdateTimes.sort((a, b) => a - b)[
				Math.floor(singleUpdateTimes.length * 0.95)
			];
			...
			server.use(
				http.post(
					...
					async ({ request }) => {
						const { updates } = (await request.json()) as any;
						await new Promise((resolve) =>
							setTimeout(resolve, updates.length * 2),
						); // 2ms per update
			...
			for (const {
				...
				for (let i = 0; i < updateCount; i++) {
					manager.applyOptimisticUpdate({
						...
						milestoneName:
							milestones[i % milestoneCount].milestoneName,
						workflowType: "MILESTONE_DISCRETE",
						value: Math.random() > 0.5,
						timestamp: Date.now(),
					});
