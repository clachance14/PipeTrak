...
function MilestoneQuickEdit({ milestone, component, isLocked = false }: MilestoneQuickEditProps) {
  const { updateMilestone, hasPendingUpdates, getOperationStatus } = useMilestoneUpdateEngine();
  ...
  const getStatusIndicator = () => {
    ...
    const completionPercent = getCompletionPercent(milestone, component.workflowType);
...
function getCompletionPercent(milestone: ComponentMilestone, workflowType: WorkflowType): number {
  switch (workflowType) {
...
function calculateOverallProgress(milestones: ComponentMilestone[], workflowType: WorkflowType): number {
  ...
  const totalWeight = milestones.reduce((sum, milestone) => sum + (milestone.creditWeight || 1), 0);
  
  return totalWeight > 0 ? Math.round(totalProgress / totalWeight) : 0;
}

export function TableMilestoneColumn({ component, className }: TableMilestoneColumnProps) {
  ...
  const overallProgress = calculateOverallProgress(component.milestones, component.workflowType);
  const hasAnyPending = component.milestones.some(m => hasPendingUpdates(m.id));
  const completedCount = component.milestones.filter(m => m.isCompleted).length;
