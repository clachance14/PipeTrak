...
export function MobileMilestoneSheet({
  ...
  const [selectedMilestone, setSelectedMilestone] = useState<ComponentMilestone | null>(null);
  ...
  useEffect(() => {
    if (selectedMilestoneId && component.milestones) {
      const milestone = component.milestones.find(m => m.id === selectedMilestoneId);
      if (milestone) {
  ...
  const stats = {
    ...
    completed: component.milestones?.filter(m => m.isCompleted).length || 0,
    inProgress: component.milestones?.filter(m => !m.isCompleted && getCompletionPercent(m, component.workflowType) > 0).length || 0,
    pending: component.milestones?.filter(m => !m.isCompleted && getCompletionPercent(m, component.workflowType) === 0).length || 0
  };
    ...
    <ScrollArea className="flex-1 px-4">
      <div className="space-y-3 pb-4">
        {component.milestones?.map((milestone, index) => {
          ...
          const isLocked = index > 0 && !component.milestones![index - 1].isCompleted;
  ...
  const renderMilestoneDetail = () => {
    ...
    const milestone = component.milestones?.find(m => m.id === selectedMilestone.id) || selectedMilestone;
    const milestoneIndex = component.milestones?.findIndex(m => m.id === milestone.id) || 0;
    const isLocked = milestoneIndex > 0 && !component.milestones![milestoneIndex - 1].isCompleted;
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <Badge variant="outline" className="text-xs">
                  Step {milestoneIndex + 1} of {component.milestones?.length || 1}
                </Badge>
    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      ...
      >
        <div className="flex flex-col h-full">
          ...
          {selectedMilestone ? renderMilestoneDetail() : renderMilestoneList()}
          ...
          <div className="flex justify-center p-2 border-t">
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setSheetHeight(
                sheetHeight === "full" ? "partial" : 
                sheetHeight === "partial" ? "collapsed" : "partial"
              )}
              className="h-8 px-4"
            >
