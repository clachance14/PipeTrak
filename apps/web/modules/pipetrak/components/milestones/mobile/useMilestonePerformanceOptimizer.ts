 ...
 */
export function useMilestonePerformanceOptimizer(options: BatchUpdateOptions = {}) {
  const {
    debounceMs = 300,
    maxBatchSize = 10,
    maxWaitMs = 2000,
  ...
  const processBatch = useCallback(async () => {
    ...
    try {
      ...
      batch.forEach(update => {
        ...
        updatesByComponent.get(key)!.push(update);
      });

      // Process each component's updates
      const promises = Array.from(updatesByComponent.values()).map(componentUpdates => {
        return engine.bulkUpdateMilestones(componentUpdates.map(update => ({
  ...
  const batchedUpdateMilestone = useCallback((
    ...
    batchRef.current = batchRef.current.filter(u => u.milestoneId !== milestoneId);
  ...
  const immediateUpdateMilestone = useCallback((
    ...
    return engine.updateMilestone(milestoneId, componentId, milestoneName, workflowType, value);
  }, [engine]);
  ...
  const batchStatus = useMemo(() => ({
    ...
    timeUntilFlush: firstUpdateTimeRef.current 
      ? Math.max(0, maxWaitMs - (Date.now() - firstUpdateTimeRef.current))
      : 0,
  }), [maxWaitMs]);
