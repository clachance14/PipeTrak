...
export const mockMilestones: ComponentMilestone[] = [
	{
		...
		weight: 10, // ROC weight for design review
	},
	{
		id: "milestone-2",
		componentId: "component-1",
		...
		weight: 30, // ROC weight for procurement
	},
	{
		id: "milestone-3",
		componentId: "component-1",
		...
		weight: 60, // ROC weight for installation
	},
	{
		id: "milestone-4",
		...
		weight: 15, // ROC weight for quality check
	},
];

// Factory functions for generating test data
export function createMockMilestone(
	overrides: Partial<ComponentMilestone> = {},
): ComponentMilestone {
	...
	return {
		...
		createdAt: new Date(),
		updatedAt: new Date(),
		weight: 10, // Default ROC weight for test milestone
		...overrides,
	};
}

export function createDiscreteMilestone(
	overrides: Partial<ComponentMilestone> = {},
): ComponentMilestone {
...
export function createPercentageMilestone(
	overrides: Partial<ComponentMilestone> = {},
): ComponentMilestone {
	return createMockMilestone({
		milestoneName: "Material Procurement",
...
export function createQuantityMilestone(
	overrides: Partial<ComponentMilestone> = {},
): ComponentMilestone {
	return createMockMilestone({
		milestoneName: "Installation",
...
export function generateMockMilestones(
	count: number,
	componentId?: string,
): ComponentMilestone[] {
	...
	for (let i = 0; i < count; i++) {
		...
		if (workflowType === WorkflowType.MILESTONE_DISCRETE) {
		...
		} else if (workflowType === WorkflowType.MILESTONE_PERCENTAGE) {
...
export function createMockBulkUpdate(
	overrides: Partial<MockBulkUpdate> = {},
): MockBulkUpdate {
	return {
	...
	offlineQueue: [
		{
			...
			milestoneName: "Design Review",
			workflowType: WorkflowType.MILESTONE_DISCRETE as WorkflowType,
			value: true,
		...
		},
		{
			...
			milestoneName: "Installation",
			workflowType: WorkflowType.MILESTONE_PERCENTAGE as WorkflowType,
			value: 85,
