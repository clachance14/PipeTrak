import type { ComponentWithMilestones, ComponentMilestone, Drawing, Project } from '../../types';
...
export const mockDrawing: Drawing = {
  ...
  number: 'P-35F11',
  description: 'Piping ISO for Area 401',
  revision: 'A',
  parentId: null,
  createdAt: new Date('2024-01-01'),
  updatedAt: new Date('2024-01-01'),
...
export const mockComponents: ComponentWithMilestones[] = [
  ...
  },
  {
    ...
    milestones: mockMilestones.map(m => ({ ...m, isCompleted: true, percentageComplete: 100 })),
    drawing: mockDrawing,
  },
];

// Generate large dataset for performance testing
export function generateMockComponents(count: number): ComponentWithMilestones[] {
  ...
  const types = ['VALVE', 'PIPE', 'GASKET', 'FLANGE', 'INSTRUMENT', 'SUPPORT'];
  const statuses = ['NOT_STARTED', 'IN_PROGRESS', 'COMPLETED'];
  const workflows = ['MILESTONE_DISCRETE', 'MILESTONE_PERCENTAGE', 'MILESTONE_QUANTITY'];
  ...
  for (let i = 0; i < count; i++) {
    ...
    const progress = status === 'COMPLETED' ? 100 : status === 'IN_PROGRESS' ? Math.floor(Math.random() * 99) : 0;
    ...
    components.push({
      ...
      totalQuantity: workflow === 'MILESTONE_QUANTITY' ? Math.floor(Math.random() * 500) + 50 : 1,
...
export const accessibilityTestComponent: ComponentWithMilestones = {
  ...
  description: 'Component with very long description that should be truncated properly in the UI to maintain layout consistency',
  area: '999',
