...
const getStepDescriptions = (importType: ImportType): Record<ImportStep, string> => ({
...
export function ImportWizard({ projectId }: ImportWizardProps) {
  ...
  React.useEffect(() => {
    if (isComplete && currentStep === "import") {
      ...
      if (jobData) {
        const successCount = (jobData.processedRows || 0) - (jobData.errorRows || 0);
  ...
  const getStepNumber = (step: ImportStep): number => {
    const steps: ImportStep[] = ["upload", "mapping", "validation", "import", "complete"];
  ...
  const handleMappingComplete = async (mappings: Record<string, string>) => {
    ...
    if (uploadData) {
      ...
      const mappingsArray = Object.entries(mappings).map(([targetField, sourceColumn]) => ({
        sourceColumn,
        targetField,
        required: targetField === 'componentId' || targetField === 'type' || targetField === 'drawingId'
      }));
  ...
  const handleStartImport = async () => {
    ...
    const mappingsArray = Object.entries(columnMappings).map(([targetField, sourceColumn]) => ({
      sourceColumn,
      targetField,
      required: targetField === 'componentId' || targetField === 'type' || targetField === 'drawingId'
    }));
    
    await startImport(uploadData, mappingsArray, validationResults?.options);
  ...
  const handleTabChange = (newType: ImportType) => {
    ...
    if (newType !== importType) {
      ...
      if (currentStep === "upload") {
        ...
        console.warn("Cannot change import type after uploading. Please reset to change type.");
        return;
      }
    }
  };
  ...
  const canGoNext = currentStep === "validation" && validationResults?.isValid;

  return (
    <div className="space-y-6">
      {/* Import Type Tabs */}
      <Tabs value={importType} onValueChange={(value) => handleTabChange(value as ImportType)}>
        ...
        <TabsContent value={importType} className="mt-6">
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span className="font-medium">Step {currentStepNumber} of 5</span>
      ...
      <Card className="p-6">
        <div className="mb-6">
          <h2 className="text-2xl font-semibold mb-2">{stepTitles[currentStep]}</h2>
          ...
          {currentStep === "upload" && (
            <div className="space-y-6">
              ...
              <Alert>
                ...
                <AlertDescription>
                  <strong>Import Type: {importType === "components" ? "Components" : "Field Welds"}</strong>
                        ...
                        <>
                          <Button
                            ...
                            onClick={() => {
                              ...
                              downloadFile(blob, `PipeTrak_Component_Template_${new Date().toISOString().split('T')[0]}.xlsx`);
                            }}
                          >
                            <FileDown className="h-4 w-4 mr-2" />
                            Component Template (Excel)
                          </Button>
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => {
                              ...
                              downloadFile(csv, `PipeTrak_Component_Template_${new Date().toISOString().split('T')[0]}.csv`);
                            }}
                          >
                            <FileDown className="h-4 w-4 mr-2" />
                            Component Template (CSV)
                          </Button>
                        </>
                      ) : (
                        <>
                          <Button
                            ...
                            onClick={() => {
                              ...
                              downloadFile(blob, `PipeTrak_FieldWeld_Template_${new Date().toISOString().split('T')[0]}.xlsx`);
                            }}
                            className="border-orange-200 text-orange-700 hover:bg-orange-50"
                          >
                            <FileDown className="h-4 w-4 mr-2" />
                            Field Weld Template (Excel)
                          </Button>
                        </>
                      )}
                    </div>
            ...
            <div className="space-y-6 py-8">
              <div className="text-center space-y-4">
                <div className={`mx-auto w-16 h-16 rounded-full flex items-center justify-center ${
                  ...
                  importStatus === "failed" ? "bg-red-100" : "bg-orange-100"
                }`}>
                ...
                <p className="text-muted-foreground max-w-md mx-auto">
                  {jobData ? (
                    <>
                      Successfully imported {(jobData.processedRows || 0) - (jobData.errorRows || 0)} components.
                      {(jobData.errorRows || 0) > 0 && (
                        <span className="block mt-2 text-orange-600">
                          {jobData.errorRows} components had errors and were skipped.
                        </span>
                      )}
                      <span className="block mt-2 text-sm">
                        Total processed: {jobData.processedRows || 0} of {jobData.totalRows || 0} rows
                      </span>
                    </>
                  ) : (
              ...
              <div className="flex justify-center gap-4">
                ...
                {(jobData?.errorRows || 0) > 0 && importJob && (
                  <Button 
                    variant="outline"
                    onClick={() => window.open(`/api/pipetrak/import/${importJob.id}/errors`, '_blank')}
                  >
                    <Download className="h-4 w-4 mr-2" />
                    Download Error Report
                  </Button>
                )}
                <Button onClick={() => window.location.href = `/app/pipetrak/${projectId}/components`}>
                  View Components
                </Button>
              </div>
            </div>
