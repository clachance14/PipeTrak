...
export function ValidationPreview({
	...
	React.useEffect(() => {
		...
		console.log(
			"ValidationPreview - validation.preview sample data:",
			validation.preview.slice(0, 1),
		);
		if (validation.preview.length > 0) {
			console.log(
				"ValidationPreview - first row data keys:",
				Object.keys(validation.preview[0].data),
			);
			console.log(
				"ValidationPreview - first row sample data:",
				validation.preview[0].data,
			);
		}
	...
	const canImport =
		validation.isValid ||
		(importOptions.skipErrors && validation.validRows > 0);
			...
			{validation.isValid ? (
				<Alert className="border-green-200 bg-green-50">
					...
					<AlertDescription className="text-green-700">
						All {validation.totalRows} rows passed validation and
						are ready to import.
					</AlertDescription>
				</Alert>
				...
				<Alert className="border-orange-200 bg-orange-50">
					...
					<AlertDescription className="text-orange-700">
						{validation.validRows} of {validation.totalRows} rows
						are valid. You can skip error rows and import valid data
						only.
					</AlertDescription>
				</Alert>
			) : (
				<Alert variant="error">
					...
					<AlertDescription>
						No valid rows found. Please review the errors and fix
						your data.
					</AlertDescription>
				</Alert>
			)}
				...
				<TabsContent value="summary" className="space-y-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Configure how the import should handle various
								scenarios
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-4">
							<div className="flex items-center space-x-2">
								...
								>
									Skip rows with errors and import valid rows
									only
								</Label>
							</div>
							...
							<div className="flex items-center space-x-2">
								<Checkbox
									...
									onCheckedChange={() =>
										handleOptionChange("updateExisting")
									}
								/>
								<Label
							...
							<div className="flex items-center space-x-2">
								<Checkbox
									...
									checked={
										importOptions.createMissingDrawings
									}
									onCheckedChange={() =>
										handleOptionChange(
								...
								/>
								<Label
									htmlFor="createMissingDrawings"
									className="cursor-pointer"
								>
									Automatically create missing drawing
									references
								</Label>
							</div>
							...
							<div className="flex items-center space-x-2">
								...
								<Label
									...
									className="cursor-pointer"
								>
									Dry run (preview only, don't actually
									import)
								</Label>
							</div>
						</CardContent>
							...
							{validation.errors.length === 0 &&
								validation.warnings.length === 0 && (
									<div className="text-center py-8">
										<CheckCircle22 className="h-12 w-12 text-green-600 mx-auto mb-4" />
