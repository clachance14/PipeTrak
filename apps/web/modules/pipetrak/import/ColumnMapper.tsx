...
const COMPONENT_FIELDS = [
  // Required fields
  { value: "componentId", label: "Component ID", required: true, description: "Part number or commodity code" },
  { value: "type", label: "Component Type", required: true, description: "VALVE, FITTING, GASKET, SUPPORT, FIELD_WELD, etc." },
  { value: "drawingId", label: "Drawing ID", required: true, description: "Drawing number this component belongs to" },
  
  // Physical attributes
  { value: "spec", label: "Specification", required: false },
  { value: "size", label: "Size / Weld Size", required: false, description: "Component size or weld diameter" },
  { value: "material", label: "Material / Base Metal", required: false, description: "Material specification or base metal for welds" },
  { value: "pressureRating", label: "Pressure Rating / Schedule", required: false, description: "Pressure rating or pipe schedule" },
  ...
  { value: "totalQuantity", label: "Quantity", required: false, description: "Number of identical instances to create (defaults to 1)" }
];

// Field weld-specific fields (shown when FIELD_WELD components are detected)
const FIELD_WELD_FIELDS = [
  // Field weld identifier
  { value: "weldId", label: "Weld ID", required: false, description: "Unique weld identifier (required for field welds)" },
  // Weld-specific QC fields
  { value: "weldSize", label: "Weld Size", required: false, description: "Weld diameter (can map to 'size' if not specified)" },
  { value: "schedule", label: "Schedule", required: false, description: "Pipe schedule (can map to 'pressureRating' if not specified)" },
  { value: "weldTypeCode", label: "Weld Type Code", required: false, description: "BW, SW, TW, etc." },
  { value: "baseMetal", label: "Base Metal", required: false, description: "Material being welded (can map to 'material' if not specified)" },
  { value: "xrayPercent", label: "X-ray Percent", required: false, description: "Percentage requiring RT (0-100)" },
  { value: "pwhtRequired", label: "PWHT Required", required: false, description: "TRUE/FALSE for post-weld heat treatment" },
  { value: "ndeTypes", label: "NDE Types", required: false, description: "RT,PT,MT,UT,VT (comma-separated)" },
  { value: "welderId", label: "Welder Stencil", required: false, description: "Welder identification" },
  { value: "dateWelded", label: "Date Welded", required: false, description: "YYYY-MM-DD format" },
  { value: "tieInNumber", label: "Tie-In Number", required: false, description: "Connection reference" },
  { value: "comments", label: "Comments", required: false, description: "Additional weld notes" }
];
...
export function ColumnMapper({ 
    ...
    try {
      ...
      headers.forEach(header => {
        ...
        if (!autoMappings.componentId) {
          if (headerLower === 'component id' || headerLower === 'componentid' || 
              headerLower === 'commodity code' || headerLower === 'commoditycode' ||
              headerLower === 'part number' || headerLower === 'tag number' || 
              headerLower === 'tag') {
            autoMappings.componentId = header;
            mapped.add('componentId');
            console.log('Auto-mapped column:', header, '→ componentId');
          }
        }
        
        // Separate auto-mapping for weld ID (only for field weld imports)
        if (!autoMappings.weldId && importType === "field_welds") {
          if (headerLower === 'weld id' || headerLower === 'weldid' ||
              headerLower === 'weld number' || headerLower === 'weldnumber') {
        ...
        if (!autoMappings.type) {
          if (headerLower === 'component type' || headerLower === 'type' || 
        ...
        if (!autoMappings.drawingId) {
          if (headerLower === 'drawing id' || headerLower === 'drawingid' || 
              headerLower === 'drawing number' || headerLower === 'drawing') {
        ...
        if (!autoMappings.spec) {
          if (headerLower === 'specification' || headerLower === 'spec') {
        ...
        if (!autoMappings.pressureRating) {
          if (headerLower === 'pressure rating' || headerLower === 'pressurerating' || 
        ...
        if (!autoMappings.description) {
          if (headerLower === 'description' || headerLower === 'desc') {
        ...
        if (!autoMappings.testPackage) {
          if (headerLower === 'test package' || headerLower === 'testpackage' || 
        ...
        if (!autoMappings.testPressure) {
          if (headerLower === 'test pressure' || headerLower === 'testpressure') {
        ...
        if (!autoMappings.testRequired) {
          if (headerLower === 'test required' || headerLower === 'testrequired') {
        ...
        if (!autoMappings.totalQuantity) {
          if (headerLower === 'quantity' || headerLower === 'qty' || 
              headerLower === 'total quantity' || headerLower === 'totalquantity') {
        ...
        if (!autoMappings.weldSize) {
          if (headerLower === 'weld size' || headerLower === 'weldsize') {
        ...
        if (!autoMappings.schedule) {
          if (headerLower === 'schedule' || headerLower === 'sch' || 
        ...
        if (!autoMappings.weldTypeCode) {
          if (headerLower === 'weld type' || headerLower === 'weld type code' || 
              headerLower === 'weldtype' || headerLower === 'weldtypecode') {
        ...
        if (!autoMappings.baseMetal) {
          if (headerLower === 'base metal' || headerLower === 'basemetal') {
        ...
        if (!autoMappings.xrayPercent) {
          if (headerLower === 'x-ray percent' || headerLower === 'xray percent' || 
              headerLower === 'xraypercent' || headerLower === 'rt percent') {
        ...
        if (!autoMappings.pwhtRequired) {
          if (headerLower === 'pwht required' || headerLower === 'pwht' || 
              headerLower === 'pwht req' || headerLower === 'pwhtrequired') {
        ...
        if (!autoMappings.ndeTypes) {
          if (headerLower === 'nde types' || headerLower === 'nde' || 
              headerLower === 'ndetypes' || headerLower === 'nde methods') {
        ...
        if (!autoMappings.welderId) {
          if (headerLower === 'welder' || headerLower === 'welder stencil' || 
              headerLower === 'welder id' || headerLower === 'welderid' ||
              headerLower === 'stencil') {
        ...
        if (!autoMappings.dateWelded) {
          if (headerLower === 'date welded' || headerLower === 'weld date' || 
              headerLower === 'datewelded' || headerLower === 'welddate') {
        ...
        if (!autoMappings.tieInNumber) {
          if (headerLower === 'tie-in number' || headerLower === 'tie in' || 
              headerLower === 'tiein' || headerLower === 'tieinnumber') {
        ...
        if (!autoMappings.comments) {
          if (headerLower === 'comments' || headerLower === 'notes' || 
              headerLower === 'remarks') {
      ...
      if (sampleData && sampleData.length > 0) {
        ...
        const typeColumns = headers.filter(header => 
        ...
        hasFieldWeldData = sampleData.some(row => {
          return typeColumns.some(typeCol => {
            const value = row[typeCol]?.toString().toLowerCase() || '';
            return value.includes('field') && value.includes('weld');
          });
        });
      ...
      const fieldWeldIndicators = [
        'weld size', 'weldsize', 'weld type', 'weldtype', 'schedule', 'base metal', 
        'basemetal', 'x-ray', 'xray', 'pwht', 'nde', 'welder', 'stencil', 
        'tie-in', 'tiein', 'weld id', 'weldid'
      ];
  ...
  const validateMappings = (): boolean => {
    ...
    const basicValidation = requiredFields.every(field => mappings[field.value]);
    ...
    requiredFields.forEach(field => {
      if (!mappings[field.value]) {
        if (field.value === 'drawingId') {
          errors.push('Drawing ID is required - all components must be associated with a drawing');
        } else {
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">Parsing file headers...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="grid gap-4">
          ...
          {relevantFields.filter(f => f.required).map(field => (
              ...
              >
                ...
                <SelectContent>
                  ...
                  {fileHeaders.map(header => (
                    ...
                    >
                      {header}
                      {isHeaderMapped(header) && getMappedHeader(field.value) !== header && (
                        <span className="text-muted-foreground ml-2">(mapped to other field)</span>
                      )}
                    </SelectItem>
                  ))}
                </SelectContent>
          ...
          {relevantFields.filter(f => !f.required).map(field => (
            <div key={field.value} className="grid grid-cols-2 gap-4 items-center">
              <Label htmlFor={field.value} className="flex items-center gap-2">
                ...
                {field.value === 'weldId' && hasFieldWelds && (
                  <span className="text-red-500">*</span>
                )}
                {field.label}
                {field.value === 'weldId' && hasFieldWelds && (
        ...
        {importType === "field_welds" && (
          <Alert className="mb-4">
            <AlertCircle className="h-4 w-4" />
            <AlertDescription>
              <strong>Field Weld Import:</strong> QC-specific fields will create enhanced tracking records. 
              If not mapped, default values will be used and can be updated later in the QC module.
            </AlertDescription>
          </Alert>
        )}
      </div>
