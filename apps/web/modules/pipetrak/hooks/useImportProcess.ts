...
export function useImportProcess(projectId: string, importType: "components" | "field_welds" = "components") {
  ...
  const startImport = async (
    ...
    try {
      ...
      if (importType === "field_welds") {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(errorData.error || "Failed to start field weld import");
        }
      ...
      if (!fileBuffer) {
        throw new Error("File data not available. Please re-upload the file.");
      }

      console.log('Frontend startImport: Starting component import with:', {
        projectId,
      ...
      const response = await fetch('/api/pipetrak/components/import-full', {
        method: 'POST',
        headers: {
      ...
      console.log('Frontend startImport: Response status:', response.status, response.ok);

      if (!response.ok) {
        const errorData = await response.json();
        console.error('Frontend startImport: Request failed:', errorData);
        throw new Error(errorData.error || "Failed to start component import");
      }
      ...
      const totalProcessed = (result.summary?.created || 0) + (result.summary?.updated || 0) + (result.summary?.skipped || 0);
      const importJob: ImportJob = {
        id: `bulk-${Date.now()}-${totalProcessed}-${result.summary?.errors || 0}`, // Include stats in job ID
        status: result.success ? "COMPLETED" : "FAILED",
        successCount: (result.summary?.created || 0) + (result.summary?.updated || 0),
  ...
  const checkImportStatus = async (jobId: string): Promise<ImportJob | null> => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || "Failed to check import status");
      }
      ...
      const importJob: ImportJob = {
        id: jobData.id,
        status: jobData.status,
        successCount: (jobData.processedRows || 0) - (jobData.errorRows || 0),
  ...
  const downloadErrorReport = async (jobId: string): Promise<void> => {
    try {
      const response = await fetch(`/api/pipetrak/import/${jobId}/errors`);
