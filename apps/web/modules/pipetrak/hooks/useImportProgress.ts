...
export function useImportProgress(jobId?: string, projectId?: string) {
	...
	useEffect(() => {
		...
		if (jobId.startsWith("bulk-")) {
			console.log(
				`[useImportProgress] Detected bulk-import job: ${jobId}`,
			);

			// Extract stats from job ID if available (format: bulk-timestamp-processed-errors)
			const jobParts = jobId.split("-");
			const processedRows =
				jobParts.length > 2 ? Number.parseInt(jobParts[2]) || 0 : 0;
			const errorRows =
				jobParts.length > 3 ? Number.parseInt(jobParts[3]) || 0 : 0;
		...
		const checkInitialStatus = async () => {
			try {
				console.log(
					`[useImportProgress] Checking initial status for job: ${jobId}`,
				);
				console.log(
					`[useImportProgress] Request URL: /api/pipetrak/import/${jobId}`,
				);
				...
				console.log(
					`[useImportProgress] Response status: ${response.status}`,
				);
				...
				if (response.ok) {
					...
					console.error(
						`[useImportProgress] HTTP ${response.status}: ${response.statusText}`,
					);
					setState((prev) => ({
						...prev,
						status: "failed",
				...
				console.error(
					"[useImportProgress] Error checking initial import status:",
					error,
				);
				setState((prev) => ({
					...prev,
		...
		const setupRealtime = async () => {
			try {
				...
				const response =
					await apiClient.pipetrak.realtime.subscribe.$post({
						json: {
							projectId,
							presence: {},
						},
				...
				if (!response.ok) {
					console.warn(
						"Failed to setup realtime subscription, falling back to polling",
					);
					startPolling();
					return;
				}
				...
				channel = supabase
					...
					.subscribe((status) => {
						if (status === "SUBSCRIBED") {
							console.log(
								"Successfully subscribed to import progress updates",
							);
							// Also start polling as a backup
							startPolling();
						} else if (status === "CHANNEL_ERROR") {
							console.warn(
								"Realtime channel error, falling back to polling only",
							);
							startPolling();
						}
					});
		...
		const startPolling = () => {
			...
			const pollFunction = async () => {
					...
					setState((prev) => ({
						...
						logs: [
							...prev.logs,
							{
								timestamp: new Date().toLocaleTimeString(),
								message:
									"Import timed out - taking longer than expected. Please check the import job status or try again.",
								type: "error",
							},
						],
					}));
				...
				try {
					console.log(
						`[useImportProgress] Polling attempt ${pollCount} for job: ${jobId}`,
					);

					const response = await apiClient.pipetrak.import[
						":id"
					].$get({
						param: { id: jobId },
					});
					...
					console.log(
						`[useImportProgress] Poll response status: ${response.status}`,
					);
					...
					if (!response.ok) {
						...
						if (response.status === 404) {
							console.error(
								`[useImportProgress] Job ${jobId} not found (404)`,
							);
							setState((prev) => ({
								...prev,
								status: "failed",
								...
								logs: [
									...prev.logs,
									{
										timestamp:
											new Date().toLocaleTimeString(),
										message:
											"Import job not found - it may have been deleted or expired.",
										type: "error",
									},
								],
							}));
						...
						if (consecutiveErrors < 10) {
							// Continue polling for up to 10 consecutive errors
							scheduleNextPoll();
						}
						return;
					}
					...
					if (status === "processing" && progress === lastProgress) {
						...
						if (stuckCounter >= maxStuckPolls) {
							console.warn(
								`[useImportProgress] Job ${jobId} appears stuck - progress hasn't changed for ${maxStuckPolls * 2} seconds`,
							);
							setState((prev) => ({
								...prev,
								status: "failed",
								...
								logs: [
									...prev.logs,
									{
										timestamp:
											new Date().toLocaleTimeString(),
										message:
											"Import appears to be stuck. The job may have encountered an error. Please check the server logs or try again.",
										type: "error",
									},
								],
							}));
