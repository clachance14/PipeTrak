      ...
      .on(
        ...
        (payload) => {
          const update: ComponentUpdate = {
            ...
            drawing_id: payload.new?.drawingId || payload.old?.drawingId,
            status: payload.new?.status || payload.old?.status,
            completion_percent: payload.new?.completionPercent || payload.old?.completionPercent || 0,
          };
...
export function useMilestoneUpdates(
  ...
  options: {
    ...
    onMilestoneCelebration?: (celebration: { componentId: string; milestoneName: string; completedBy: string }) => void;
    enabled?: boolean;
  } = {}
) {
  const { enabled = true } = options;
      ...
      .on(
        ...
        (payload) => {
          const update: MilestoneUpdate = {
            type: 'milestone',
            action: payload.eventType as any,
            component_id: payload.new?.componentId || payload.old?.componentId,
            milestone_name: payload.new?.milestoneName || payload.old?.milestoneName,
          ...
          if (update.action === 'UPDATE' && update.is_completed && update.completed_by) {
  ...
  }, [projectId, enabled, session, options.onMilestoneUpdate, options.onMilestoneCelebration]);
...
export function usePresence(
  ...
  const updatePresence = useCallback(async (updates: Partial<Pick<UserPresence, 'viewingDrawingId' | 'editingComponentId' | 'isTyping'>>) => {
  ...
  }, [session, projectId, options.viewingDrawingId, options.editingComponentId]);
  ...
  }, [options.viewingDrawingId, options.editingComponentId, isConnected, updatePresence]);
...
export function useOptimisticUpdates<T>(
  ...
  const applyOptimisticUpdate = useCallback((id: string, optimisticValue: T) => {
...
export function useRealtimeSubscription(
  ...
  useEffect(() => {
    const connected = componentUpdates.isConnected || 
...
export function useBroadcast(projectId: string) {
  ...
  const broadcast = useCallback(async (
    ...
    try {
      const response = await fetch('/api/pipetrak/realtime/broadcast', {
        method: 'POST',
        headers: {
