...
export function calculateROCWeight(
  ...
  const systemMultiplier = system ? (systemMultipliers[system] || systemMultipliers["Other"]) : 1.0;
  
  // Apply area multiplier  
  const areaMultiplier = area ? (areaMultipliers[area] || areaMultipliers["Other"]) : 1.0;
 ...
 */
export function calculateOverallROCProgress(components: ComponentDetail[]): ROCWeightedProgress {
  let totalWeight = 0;
  let completedWeight = 0;
  
  // Area and system breakdowns
  const areaBreakdowns: Record<string, { weight: number; completed: number; count: number }> = {};
  const systemBreakdowns: Record<string, { weight: number; completed: number; count: number }> = {};
  ...
  const overallROCProgress = totalWeight > 0 ? (completedWeight / totalWeight) * 100 : 0;

  // Transform breakdowns to response format
  const areaBreakdownsArray = Object.entries(areaBreakdowns).map(([area, data]) => ({
    area,
    rocProgress: data.weight > 0 ? (data.completed / data.weight) * 100 : 0,
    totalWeight: data.weight,
    completedWeight: data.completed,
    componentCount: data.count,
  }));

  const systemBreakdownsArray = Object.entries(systemBreakdowns).map(([system, data]) => ({
    system,
    rocProgress: data.weight > 0 ? (data.completed / data.weight) * 100 : 0,
    totalWeight: data.weight,
    completedWeight: data.completed,
  ...
  return {
    ...
    areaBreakdowns: areaBreakdownsArray.sort((a, b) => b.totalWeight - a.totalWeight),
    systemBreakdowns: systemBreakdownsArray.sort((a, b) => b.totalWeight - a.totalWeight),
 ...
 */
export function compareROCvsStandardProgress(components: ComponentDetail[]): Array<{
  name: string;
  rocProgress: number;
  ...
  return Object.entries(areaGroups).map(([area, areaComponents]) => {
    const rocData = calculateOverallROCProgress(areaComponents);
    const standardProgress = areaComponents.reduce((sum, comp) => sum + comp.completionPercent, 0) / areaComponents.length;
...
export function calculateROCVelocity(
  ...
  for (let i = 1; i < historicalData.length; i++) {
    const previousROC = calculateOverallROCProgress(historicalData[i - 1].components);
    const currentROC = calculateOverallROCProgress(historicalData[i].components);
    
    const dailyProgress = currentROC.overallROCProgress - previousROC.overallROCProgress;
    dailyROCProgress.push(Math.max(0, dailyProgress)); // Only count positive progress
  }

  // Calculate metrics
  const averageDailyROCCompletion = dailyROCProgress.reduce((sum, progress) => sum + progress, 0) / dailyROCProgress.length;
  
  const recentDays = Math.min(7, dailyROCProgress.length);
  const currentVelocity = dailyROCProgress
    .slice(-recentDays)
  ...
  const firstHalfAvg = firstHalf.reduce((sum, progress) => sum + progress, 0) / firstHalf.length;
  const secondHalfAvg = secondHalf.reduce((sum, progress) => sum + progress, 0) / secondHalf.length;
  ...
  if (currentVelocity > 0) {
    const latestROC = calculateOverallROCProgress(historicalData[historicalData.length - 1].components);
    const remainingProgress = 100 - latestROC.overallROCProgress;
    const daysToCompletion = remainingProgress / currentVelocity;
    
    const today = new Date();
    const projectedDate = new Date(today.getTime() + daysToCompletion * 24 * 60 * 60 * 1000);
...
export function identifyCriticalPathComponents(
  ...
  const componentsWithROC = components.map(component => ({
    component,
    rocWeight: calculateROCWeight(component.type, component.system, component.area),
  }));
...
export function calculateROCEfficiency(components: ComponentDetail[]): {
  ...
  const completedCritical = criticalComponents.filter(comp => comp.completionPercent >= 100);
  
  const efficiencyScore = criticalComponents.length > 0 
  ...
  if (efficiencyScore < 50) {
    recommendations.push('Focus on high-ROC components in critical systems');
    recommendations.push('Prioritize major equipment and control valves');
  }
  
  if (efficiencyScore < 75) {
    const incompleteCritical = criticalComponents.filter(comp => comp.completionPercent < 100);
    const areaFocus = Array.from(new Set(incompleteCritical.map(c => c.area))).slice(0, 2);
    recommendations.push(`Focus completion efforts in: ${areaFocus.join(', ')}`);
  }
