 ...
 */
export function generateProgressCharts(data: ProgressReportResponse): ProgressChartData {
  const { comprehensiveReport, rocWeightedProgress } = data.data;

  // Time series data from trends
  const timeSeriesData: ChartDataPoint[] = comprehensiveReport.trends?.dailyProgress.map(point => ({
    name: point.date,
    date: point.date,
  ...
  // Area distribution
  const areaDistribution: ChartDataPoint[] = comprehensiveReport.areaBreakdowns.map(area => ({
    name: area.area,
  ...
  // System distribution
  const systemDistribution: ChartDataPoint[] = comprehensiveReport.systemBreakdowns.map(system => ({
    name: system.system,
  ...
  const statusDistribution: ChartDataPoint[] = [
    ...
    {
      name: 'In Progress',
      value: comprehensiveReport.overview.totalComponents - comprehensiveReport.overview.completedComponents,
    },
  ];
...
export function generateROCCharts(data: ProgressReportResponse): ROCChartData {
  ...
  // ROC by area
  const rocByArea: ChartDataPoint[] = rocWeightedProgress.areaBreakdowns.map(area => ({
    name: area.area,
  ...
  // ROC by system
  const rocBySystem: ChartDataPoint[] = rocWeightedProgress.systemBreakdowns.map(system => ({
    name: system.system,
...
export function generateTrendCharts(data: TrendAnalysisResponse): {
  ...
  const forecastData: ChartDataPoint[] | undefined = forecast?.projectedProgress.map(point => ({
    name: point.date,
    date: point.date,
...
export function formatComponentDetails(data: ComponentDetailsResponse): {
  formattedComponents: Array<ComponentDetail & { formattedUpdatedAt: string; statusColor: string }>;
  summary: {
  ...
  const completionStats = {
    completed: data.data.components.filter(c => c.completionPercent >= 100).length,
    inProgress: data.data.components.filter(c => c.completionPercent > 0 && c.completionPercent < 100).length,
    notStarted: data.data.components.filter(c => c.completionPercent === 0).length,
  };

  const stalledAnalysis = {
    count: data.data.components.filter(c => c.stalledDays && c.stalledDays > 7).length,
    percentage: data.data.components.length > 0 
      ? (data.data.components.filter(c => c.stalledDays && c.stalledDays > 7).length / data.data.components.length) * 100
      : 0,
  };
 ...
 */
export function formatTestPackageReadiness(data: TestPackageReadinessResponse): {
    ...
    case 'progress':
      ...
      sheets = [
          ...
          data: [
            {
              ...
              'Value': data.data.comprehensiveReport.overview.totalComponents,
            },
            {
              'Metric': 'Completed Components',
              'Value': data.data.comprehensiveReport.overview.completedComponents,
            },
            {
              'Metric': 'Overall Completion %',
              'Value': data.data.comprehensiveReport.overview.overallCompletionPercent,
            },
            {
              'Metric': 'ROC-Weighted Progress %',
              'Value': data.data.rocWeightedProgress.overallROCProgress,
            },
          ],
        },
        {
          name: 'Area Breakdown',
          data: data.data.comprehensiveReport.areaBreakdowns.map((area: any) => ({
            'Area': area.area,
        ...
        },
        {
          ...
          data: data.data.comprehensiveReport.systemBreakdowns.map((system: any) => ({
      ...
      sheets = [
        {
          ...
          data: data.data.components.map((component: ComponentDetail) => ({
...
export function preparePDFContent(
  ...
  const content: Array<{ type: 'header' | 'paragraph' | 'table' | 'chart'; data: any }> = [];
      ...
      content.push(
        ...
        },
        {
          ...
          data: {
            ...
            rows: [
              ['Total Components', data.data.comprehensiveReport.overview.totalComponents],
              ['Completed Components', data.data.comprehensiveReport.overview.completedComponents],
              ['Overall Completion %', `${data.data.comprehensiveReport.overview.overallCompletionPercent}%`],
              ['ROC-Weighted Progress %', `${data.data.rocWeightedProgress.overallROCProgress.toFixed(1)}%`],
            ],
          },
        }
      );
...
export function generateInsights(
  ...
  switch (reportType) {
    case 'progress':
      const rocProgress = data.data.rocWeightedProgress.overallROCProgress;
      const standardProgress = data.data.comprehensiveReport.overview.overallCompletionPercent;
      
      if (rocProgress > standardProgress + 5) {
        insights.push('Project is ahead of schedule on critical path components.');
      } else if (rocProgress < standardProgress - 5) {
        insights.push('Focus needed on high-priority components to maintain schedule.');
      }
      
      if (data.data.comprehensiveReport.trends?.velocityMetrics?.averageDailyCompletion < 1) {
        insights.push('Current completion velocity is below optimal. Consider resource reallocation.');
      }
      break;

    case 'components':
      const stalledCount = data.data.components.filter((c: ComponentDetail) => c.stalledDays && c.stalledDays > 14).length;
      const totalCount = data.data.components.length;
      
      if (stalledCount > totalCount * 0.15) {
        insights.push('High number of stalled components detected. Review resource allocation.');
      }
      break;
  }
