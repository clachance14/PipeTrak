...
export async function generateTrendAnalysis(
  ...
  if (!response.ok) {
    const error = await response.json();
    throw new Error(error.error || 'Failed to generate trend analysis report');
  }
...
export async function clearReportCache(
  projectId: string,
  reportType?: string
): Promise<{ success: boolean; data: { deletedEntries: number; reportType: string } }> {
  const params = reportType ? { reportType } : {};
  
  const response = await apiClient.pipetrak.reports.cache[":projectId"].$delete({
    param: { projectId },
    query: params,
  });
...
export async function generateBulkReports(
  ...
  if (!response.ok) {
    const error = await response.json();
    throw new Error(error.error || 'Failed to start bulk report generation');
  }
