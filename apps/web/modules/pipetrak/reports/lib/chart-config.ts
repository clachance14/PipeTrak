...
import type { ChartDataPoint, ProgressChartData, TrendDataPoint } from "../types";
...
export const CHART_FORMATTERS = {
  ...
  shortDate: (value: string) => new Date(value).toLocaleDateString("en-US", {
    month: "short",
    day: "numeric",
  }),
 ...
 */
export function getResponsiveChartConfig(screenSize: "mobile" | "tablet" | "desktop" = "desktop") {
...
export const CHART_TRANSFORMERS = {
   ...
   */
  areaBreakdown: (data: Array<{ area: string; completionPercent: number; totalComponents: number }>): ChartDataPoint[] => {
   ...
   */
  systemBreakdown: (data: Array<{ system: string; completionPercent: number; totalComponents: number }>): ChartDataPoint[] => {
    return data.map(item => ({
      name: item.system,
      value: item.completionPercent,
      totalComponents: item.totalComponents,
