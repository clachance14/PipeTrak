...
import type { ReportFilters, ComponentDetailsFilters, AuditFilters } from "../types";
...
export function useReportFilters({
  ...
  const initializeFiltersFromURL = useCallback((): ReportFilters => {
    ...
    if (completionMin || completionMax) {
      urlFilters.completionRange = {
        min: completionMin ? parseInt(completionMin, 10) : 0,
        max: completionMax ? parseInt(completionMax, 10) : 100,
      };
    }
  ...
  const [filters, setFilters] = useState<ReportFilters>(initializeFiltersFromURL);
  ...
  const updateURL = useCallback((newFilters: ReportFilters, newSearchQuery?: string) => {
    ...
    if (newFilters.componentTypes?.length) {
      params.set('componentTypes', newFilters.componentTypes.join(','));
    }
    ...
    if (newFilters.completionRange) {
      if (newFilters.completionRange.min !== undefined) {
        params.set('completionMin', newFilters.completionRange.min.toString());
      }
      if (newFilters.completionRange.max !== undefined) {
        params.set('completionMax', newFilters.completionRange.max.toString());
      }
    }

    // Add search query
    const searchValue = newSearchQuery !== undefined ? newSearchQuery : searchQuery;
    if (searchValue) {
  ...
  const toComponentDetailsFilters = useCallback((): ComponentDetailsFilters => {
    return {
      areas: filters.areas,
  ...
  const validateFilters = useCallback((): string[] => {
    ...
    if (filters.completionRange) {
      ...
      if (min !== undefined && max !== undefined && min > max) {
        errors.push('Minimum completion percentage cannot be greater than maximum');
      }
