...
const reportsKeys = {
  ...
  project: (projectId: string) => [...reportsKeys.all, "project", projectId] as const,
  progress: (projectId: string, filters?: ReportFilters) => 
    [...reportsKeys.project(projectId), "progress", filters] as const,
  components: (projectId: string, filters?: ComponentDetailsFilters, pagination?: ReportPagination, sorting?: ReportSorting) => 
    [...reportsKeys.project(projectId), "components", filters, pagination, sorting] as const,
  testPackages: (projectId: string, filters?: any) => 
    [...reportsKeys.project(projectId), "test-packages", filters] as const,
  trends: (projectId: string, days?: number, granularity?: 'daily' | 'weekly') => 
    [...reportsKeys.project(projectId), "trends", days, granularity] as const,
  audit: (projectId: string, filters?: any, pagination?: ReportPagination) => 
    [...reportsKeys.project(projectId), "audit", filters, pagination] as const,
  status: (projectId: string) => [...reportsKeys.project(projectId), "status"] as const,
  filters: (projectId: string) => [...reportsKeys.project(projectId), "filters"] as const,
};
...
export function useRefreshProgressReport() {
  ...
  return useMutation({
    mutationFn: async ({ projectId, filters }: { projectId: string; filters?: ReportFilters }) => {
...
export function useComponentDetailsReport(
  ...
  return useQuery({
    queryKey: reportsKeys.components(projectId, filters, pagination, sorting),
    queryFn: async () => {
      const request = buildComponentDetailsQuery(filters, pagination, sorting);
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.setQueryData(
        reportsKeys.components(projectId, variables.filters, variables.pagination, variables.sorting),
        data
      );
    },
    onError: (error) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const request: TestPackageReadinessRequest = {
        ...
        filters,
        options: {
          includeBlockingComponents: options?.includeBlockingComponents ?? true,
          includeVelocityAnalysis: options?.includeVelocityAnalysis ?? true,
...
export function useTrendAnalysis(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const request = buildTrendAnalysisQuery(projectId, days, granularity, options);
...
export function useClearReportCache() {
  ...
  return useMutation({
    mutationFn: async ({ projectId, reportType }: { projectId: string; reportType?: string }) => {
...
export function useReportsDashboard(
  ...
  const progressQuery = useProgressReport(
    ...
    {},
    {
      enabled: options?.includeProgress !== false && options?.enabled !== false,
    }
  );
  ...
  const testPackagesQuery = useTestPackageReadiness(
    ...
    {},
    {
      enabled: options?.includeTestPackages !== false && options?.enabled !== false,
    }
  );
  ...
  const isLoading = progressQuery.isLoading || testPackagesQuery.isLoading || trendsQuery.isLoading;
  const isError = progressQuery.isError || testPackagesQuery.isError || trendsQuery.isError;
  const error = progressQuery.error || testPackagesQuery.error || trendsQuery.error;
  ...
  return useQuery({
    ...
    onSuccess: (data) => {
      ...
      const recentCompletions = data.data.recentGenerations.filter(
        ...
        new Date(gen.completedAt || '').getTime() > Date.now() - 60000 // Last minute
      );
      ...
      if (recentCompletions.length > 0) {
        ...
        recentCompletions.forEach((completion) => {
          queryClient.invalidateQueries({
            queryKey: [...reportsKeys.project(projectId), completion.reportType],
          });
        });
      }
    },
