...
export function useReportExport() {
	...
	const excelExport = useMutation({
		mutationFn: async (options: ExportOptions) => {
			...
			const filename =
				options.filename ||
				`${options.reportType}-report-${timestamp}.xlsx`;
		...
		onError: (error: any) => {
			toast({
				...
				description:
					error.message || "Failed to export to Excel format",
				variant: "destructive",
			});
		},
	});
	...
	const pdfExport = useMutation({
		mutationFn: async (options: ExportOptions) => {
			...
			const filename =
				options.filename ||
				`${options.reportType}-report-${timestamp}.pdf`;
	...
	const csvExport = useMutation({
		mutationFn: async (options: ExportOptions) => {
			...
			const filename =
				options.filename ||
				`${options.reportType}-report-${timestamp}.csv`;
	...
	const exportReport = async (options: ExportOptions) => {
		...
		try {
			switch (options.type) {
				...
				default:
					throw new Error(
						`Unsupported export format: ${options.type}`,
					);
			}
		} finally {
