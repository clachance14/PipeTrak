...
export function ProgressSummaryReportContent({ projectId }: ProgressSummaryReportContentProps) {
  // State
  ...
  const [groupBy, setGroupBy] = useState<"area" | "system" | "testPackage">("area");
  ...
  const generateReport = async () => {
    ...
    try {
      const response = await fetch('/api/pipetrak/reports/progress-summary', {
        method: 'POST',
        headers: {
  ...
  const handleExport = async (format: 'csv' | 'excel' | 'pdf') => {
    ...
    try {
      const response = await fetch('/api/pipetrak/exports/progress-summary', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const getGroupColumnName = (): string => {
    return groupBy === 'testPackage' ? 'Test Package' : 
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-2xl">Progress Summary Report</CardTitle>
              <p className="text-muted-foreground mt-1">
                Weekly progress report for P6 schedule updates with Tuesday 9 AM cutoff
              </p>
            </div>
            <div className="flex items-center gap-2">
              ...
              {reportData && (
                <Badge variant="outline" className="text-xs">
                  Generated: {format(new Date(reportData.reportInfo.generatedAt), 'MMM d, h:mm a')}
                </Badge>
              )}
            </div>
          </div>
        ...
        <CardContent>
          ...
          <Alert className={cn(
            "mb-4",
            isFinal ? "border-green-200 bg-green-50" : "border-yellow-200 bg-yellow-50"
          )}>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">Week Ending (Sunday)</label>
              <Popover>
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
                    mode="single"
                    selected={weekEnding}
                    onSelect={(date) => date && setWeekEnding(date)}
                    initialFocus
                  />
                </PopoverContent>
              </Popover>
            ...
            <div className="space-y-2">
              ...
              <Select value={groupBy} onValueChange={(value: any) => setGroupBy(value)}>
                ...
                <SelectContent>
                  <SelectItem value="area">Area/Sub-Area</SelectItem>
                  <SelectItem value="system">System</SelectItem>
                  <SelectItem value="testPackage">Test Package</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-2">
                  ...
                  />
                  <label htmlFor="showDeltas" className="text-sm">Show week-over-week delta</label>
                </div>
                <div className="flex items-center space-x-2">
                  <Checkbox
                  ...
                  />
                  <label htmlFor="includeGrandTotal" className="text-sm">Show grand total</label>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Download the report in various formats for distribution and P6 updates
                  </p>
                </div>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle>Progress by {getGroupColumnName()}</CardTitle>
                  <p className="text-sm text-muted-foreground">
                    Week ending {format(weekEnding, 'PPPP')} • {reportData.summary.totalComponents.toLocaleString()} total components
                  </p>
                </div>
                <Badge variant="outline" className="text-sm font-mono">
                  {Math.round(reportData.summary.overallProgress)}% Overall
                </Badge>
              </div>
            </CardHeader>
            <CardContent>
                ...
                <table className="w-full border-collapse border border-gray-300 text-sm">
                  <thead>
                    <tr className="bg-gray-50">
                      <th className="border border-gray-300 px-3 py-2 text-left font-semibold">{getGroupColumnName()}</th>
                      <th className="border border-gray-300 px-3 py-2 text-center font-semibold">Budget</th>
                      <th className="border border-gray-300 px-3 py-2 text-center font-semibold">Received</th>
                      <th className="border border-gray-300 px-3 py-2 text-center font-semibold">Installed</th>
                      <th className="border border-gray-300 px-3 py-2 text-center font-semibold">Punched</th>
                      <th className="border border-gray-300 px-3 py-2 text-center font-semibold">Tested</th>
                      <th className="border border-gray-300 px-3 py-2 text-center font-semibold">Restored</th>
                      ...
                      return (
                        <React.Fragment key={key}>
                          <tr className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                            <td className="border border-gray-300 px-3 py-2 font-medium">{key}</td>
                            <td className="border border-gray-300 px-3 py-2 text-center">{row.component_count}</td>
                          ...
                          {showDeltas && delta && (
                            <tr className="text-xs text-muted-foreground">
                              ...
                              <td className="border border-gray-300 px-3 py-1 text-center">
                                <span className={cn(
                                  delta.received > 0 ? "text-green-600" : delta.received < 0 ? "text-red-600" : "text-gray-500"
                                )}>
                              ...
                              <td className="border border-gray-300 px-3 py-1 text-center">
                                <span className={cn(
                                  delta.installed > 0 ? "text-green-600" : delta.installed < 0 ? "text-red-600" : "text-gray-500"
                                )}>
                              ...
                              <td className="border border-gray-300 px-3 py-1 text-center">
                                <span className={cn(
                                  delta.punched > 0 ? "text-green-600" : delta.punched < 0 ? "text-red-600" : "text-gray-500"
                                )}>
                              ...
                              <td className="border border-gray-300 px-3 py-1 text-center">
                                <span className={cn(
                                  delta.tested > 0 ? "text-green-600" : delta.tested < 0 ? "text-red-600" : "text-gray-500"
                                )}>
                              ...
                              <td className="border border-gray-300 px-3 py-1 text-center">
                                <span className={cn(
                                  delta.restored > 0 ? "text-green-600" : delta.restored < 0 ? "text-red-600" : "text-gray-500"
                                )}>
                              ...
                              <td className="border border-gray-300 px-3 py-1 text-center">
                                <span className={cn(
                                  delta.overall > 0 ? "text-green-600" : delta.overall < 0 ? "text-red-600" : "text-gray-500"
                                )}>
              ...
              {reportData.data.length === 0 && (
                <div className="text-center py-8 text-muted-foreground">
                  No data available for the selected week and grouping.
                </div>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent className="pt-6">
              <div className="grid gap-4 md:grid-cols-3">
                ...
                <div>
                  ...
                  <div className="text-sm space-y-1">
                    <div><span className="font-medium">Week Ending:</span> {format(weekEnding, 'PPP')}</div>
                    <div><span className="font-medium">Grouped By:</span> {getGroupColumnName()}</div>
                    <div><span className="font-medium">Show Deltas:</span> {showDeltas ? 'Yes' : 'No'}</div>
                  </div>
                </div>
                <div>
                  ...
                  <div className="text-sm space-y-1">
                    ...
                    <div><span className="font-medium">Data Cutoff:</span> {format(tuesday9AM, 'PPP p')}</div>
