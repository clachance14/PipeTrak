...
import { formatters, transformers } from "../lib/report-utils";
import { 
  RefreshCw, 
  AlertCircle, 
  Search,
...
import type { ComponentDetailsResponse, ComponentDetail, ReportSorting } from "../types";
...
export function ComponentReportContent({
  ...
  const [reportData, setReportData] = useState<ComponentDetailsResponse | null>(null);
  ...
  const { mutate: generateReport, isPending: isGenerating, error } = useComponentReportGeneration();
  const {
    filters,
    updateFilters,
    clearFilters,
  ...
  }, [projectId, filters, searchQuery, currentPage, pageSize, sorting, generateReport, toComponentDetailsFilters]);
  ...
  const getStatusVariant = (completion: number): "default" | "secondary" | "destructive" | "outline" => {
              ...
              <div className="grid gap-4 md:grid-cols-4">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-600">
                    {reportData.data.summary.averageCompletion.toFixed(1)}%
                  </div>
                  <div className="text-sm text-muted-foreground">Average Completion</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-orange-600">
                    {reportData.data.summary.stalledComponents}
                  </div>
                  <div className="text-sm text-muted-foreground">Stalled Components</div>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardHeader className="pb-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  ...
                  {reportData.data.pagination.totalCount.toLocaleString()} components
                  ...
                  <TableHeader>
                    <TableRow>
                      <TableHead className="w-[200px]">
                        <Button
                          variant="ghost"
                          onClick={() => handleSort('componentId')}
                  ...
                  <TableBody>
                    {tableData.map((component) => (
                      <TableRow key={component.componentId}>
                        ...
                        <TableCell>
                          <div className="space-y-1">
                            <div className="flex items-center justify-between text-xs">
                              <span>{component.formattedCompletion}</span>
                              <span className={component.statusColor}>
                        ...
                        <TableCell>
                          <Badge variant={getStatusVariant(component.completionPercent)}>
                            {component.status}
                          </Badge>
                        </TableCell>
                ...
                <div className="flex items-center justify-between mt-6 no-print">
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    <span>
                      ...
                      {Math.min(currentPage * pageSize, reportData.data.pagination.totalCount)} of{' '}
                      {reportData.data.pagination.totalCount.toLocaleString()} components
                    </span>
                  </div>
          ...
          <Card>
            <CardContent className="flex items-center justify-center py-12">
              <div className="text-center text-muted-foreground">
                ...
                <p>No components found matching the current filters</p>
                <Button 
                  variant="outline" 
