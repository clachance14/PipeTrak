            ...
            <Select>
              ...
              <SelectContent>
                {filterOptions?.areas?.map((area) => {
                  const isSelected = filters.areas?.includes(area) || false;
                  return (
                    <SelectItem
                      key={area}
                      value={area}
                      onClick={(e) => {
                        e.preventDefault();
                        handleMultiSelectChange('areas', area, !isSelected);
                      }}
                ...
                {filters.areas.map((area) => (
                  ...
                  >
                    ...
                    <Button
                      ...
                      onClick={() => handleMultiSelectChange('areas', area, false)}
                    >
                      <X className="h-3 w-3" />
                    </Button>
                  </Badge>
            ...
            <Select>
              ...
              <SelectContent>
                {filterOptions?.systems?.map((system) => {
                  const isSelected = filters.systems?.includes(system) || false;
                  return (
                    <SelectItem
                      key={system}
                      value={system}
                      onClick={(e) => {
                        e.preventDefault();
                        handleMultiSelectChange('systems', system, !isSelected);
                      }}
                      className="cursor-pointer"
                    >
                      <div className="flex items-center gap-2">
                ...
                {filters.systems.map((system) => (
                  ...
                  >
                    ...
                    <Button
                      ...
                      size="sm"
                      className="h-auto p-0 ml-1"
                      onClick={() => handleMultiSelectChange('systems', system, false)}
                    >
                      <X className="h-3 w-3" />
                    </Button>
                  </Badge>
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
              ...
              {filterOptions?.testPackages?.length && (
                <div className="space-y-2">
                  <Label className="text-sm font-medium">Test Packages</Label>
                  <Select>
                    <SelectTrigger>
                      <SelectValue placeholder="Select test packages" />
                    </SelectTrigger>
                    <SelectContent>
                      {filterOptions.testPackages.map((pkg) => {
                        const isSelected = filters.testPackages?.includes(pkg) || false;
                        return (
                          <SelectItem
                            key={pkg}
                            value={pkg}
                            onClick={(e) => {
                              e.preventDefault();
                              handleMultiSelectChange('testPackages', pkg, !isSelected);
                            }}
                            className="cursor-pointer"
                          >
                            <div className="flex items-center gap-2">
                  ...
                  <Select>
                    ...
                    <SelectContent>
                      {filterOptions.componentTypes.map((type) => {
                        const isSelected = filters.componentTypes?.includes(type) || false;
                        return (
                          <SelectItem
                            key={type}
                            value={type}
                            onClick={(e) => {
                              e.preventDefault();
                              handleMultiSelectChange('componentTypes', type, !isSelected);
                            }}
                            className="cursor-pointer"
                          >
                            <div className="flex items-center gap-2">
                      ...
                      {filterOptions.statuses.map((status) => {
                        ...
                        return (
                          <SelectItem
                            ...
                            onClick={(e) => {
                              e.preventDefault();
                              handleMultiSelectChange('statuses', status, !isSelected);
                            }}
                            className="cursor-pointer"
                          >
                            <div className="flex items-center gap-2">
              ...
              <div className="space-y-2 md:col-span-2">
                <Label className="text-sm font-medium">Completion Range (%)</Label>
                <div className="flex items-center gap-2">
                  <Input
                    type="number"
                    min="0"
                    ...
                    onChange={(e) => {
                      ...
                      handleCompletionRangeChange(value, filters.completionRange?.max);
                    }}
                    className="w-20"
                  />
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      const value = e.target.value ? parseInt(e.target.value) : undefined;
                      handleCompletionRangeChange(filters.completionRange?.min, value);
                    }}
                    className="w-20"
                  />
                </div>
