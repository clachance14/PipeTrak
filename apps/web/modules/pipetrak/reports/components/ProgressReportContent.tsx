...
export function ProgressReportContent({
  ...
  const { mutate: generateReport, isPending: isGenerating, error } = useProgressReportGeneration();
  const {
    filters,
    updateFilters,
    clearFilters,
  ...
  const trendData = reportData?.data.comprehensiveReport.trends?.dailyProgress || [];
        ...
        {isGenerating && isInitialLoad ? (
          ...
          <div className="space-y-6">
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                  ...
                  <div className="text-center p-4 bg-blue-50 rounded-lg">
                    <div className="text-2xl font-bold text-blue-700">
                      {reportData.data.comprehensiveReport.overview.activeDrawings}
                    </div>
                    <div className="text-sm text-muted-foreground">Active Drawings</div>
                  </div>
                  <div className="text-center p-4 bg-purple-50 rounded-lg">
                    <div className="text-2xl font-bold text-purple-700">
                      {reportData.data.comprehensiveReport.overview.testPackagesReady}
                    </div>
                    <div className="text-sm text-muted-foreground">Test Packages Ready</div>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              ...
              <Card className="print-break-inside-avoid">
                <CardContent className="p-6">
                  <h3 className="text-lg font-semibold mb-4">Top Areas by Progress</h3>
                        ...
                        <div key={area.area} className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            <div className="font-semibold">
                              {area.completionPercent.toFixed(1)}%
                            </div>
                            <div className="text-xs text-muted-foreground">
                              {area.completedComponents} / {area.totalComponents}
                            </div>
                          </div>
                        </div>
                      ))}
              ...
              <Card className="print-break-inside-avoid">
                <CardContent className="p-6">
                  <h3 className="text-lg font-semibold mb-4">Top Systems by Progress</h3>
                  <div className="space-y-3">
                    {reportData.data.comprehensiveReport.systemBreakdowns
                      ...
                      .map((system, index) => (
                        <div key={system.system} className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            <div className="font-semibold">
                              {system.completionPercent.toFixed(1)}%
                            </div>
                            <div className="text-xs text-muted-foreground">
                              {system.completedComponents} / {system.totalComponents}
                            </div>
                          </div>
                        </div>
                      ))}
