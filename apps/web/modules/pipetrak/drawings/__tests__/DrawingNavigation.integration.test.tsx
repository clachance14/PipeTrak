...
describe("Drawing Navigation Integration", () => {
	...
	describe("complete navigation flow", () => {
		it("should load hierarchy and allow navigation", async () => {
			...
			const childDrawing = screen
				.getByText("P&ID-001-A")
				.closest('[role="treeitem"]');
			fireEvent.click(childDrawing!);

			// Verify selection is tracked
			expect(screen.getByTestId("selected-drawing")).toHaveTextContent(
				"Selected: d2",
			);
		});
		...
		it("should handle search and selection", async () => {
			...
			const searchInput =
				await screen.findByPlaceholderText(/search by drawing/i);
			...
			await waitFor(() => {
				expect(
					screen.getByTestId("selected-drawing"),
				).toBeInTheDocument();
			});
		});
		...
		it("should handle empty project", async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText("No drawings available"),
				).toBeInTheDocument();
			});
		});
	...
	describe("tree interaction with API", () => {
		...
		it("should filter tree based on search", async () => {
			...
			const treeSearchInput =
				screen.getByPlaceholderText("Search drawings...");
			await user.type(treeSearchInput, "Secondary");

			// Only matching drawing should be visible
			await waitFor(() => {
				expect(
					screen.getByText("Secondary Process"),
				).toBeInTheDocument();
				expect(
					screen.queryByText("Main Process Flow"),
			...
			await waitFor(() => {
				expect(
					screen.getByText("Main Process Flow"),
				).toBeInTheDocument();
				expect(
					screen.getByText("Secondary Process"),
				).toBeInTheDocument();
