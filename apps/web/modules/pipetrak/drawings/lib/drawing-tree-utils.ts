import type { DrawingTreeNode, Drawing, DrawingComponentCount } from "../../types";

/**
 * Flattens a tree structure into a flat array
 */
export function flattenDrawingTree(drawings: DrawingTreeNode[]): DrawingTreeNode[] {
    ...
    return nodes
      .map(node => {
        const matchesSearch =
          ...
          (node.revision && node.revision.toLowerCase().includes(lowerQuery));
...
export function updateDrawingComponentCount(
  ...
  return drawings.map(drawing => {
    ...
    if (drawing.children && drawing.children.length > 0) {
      return {
        ...drawing,
        children: updateDrawingComponentCount(drawing.children, drawingId, componentCount),
      };
    }
