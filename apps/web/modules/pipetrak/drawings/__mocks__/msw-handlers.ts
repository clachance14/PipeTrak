...
export const drawingHandlers = [
  ...
  http.get(`${BASE_URL}/api/pipetrak/drawings/project/:projectId/hierarchy`, ({ params }) => {
  ...
  http.get(`${BASE_URL}/api/pipetrak/drawings/:drawingId/details`, ({ params, request }) => {
    ...
    const components = Array.from({ length: limit }, (_, i) => ({
      ...
      status: status || ["NOT_STARTED", "IN_PROGRESS", "COMPLETED", "ON_HOLD"][i % 4],
      ...
      // Apply search filter
      if (search) {
        return comp.componentId.toLowerCase().includes(search.toLowerCase());
      }
      return true;
    });
  ...
  http.get(`${BASE_URL}/api/pipetrak/drawings/project/:projectId/search`, ({ request }) => {
    const url = new URL(request.url);
    ...
    const searchResults = mockFlatDrawings
      .filter(drawing => 
        drawing.number.toLowerCase().includes(query.toLowerCase()) ||
      ...
      .map(drawing => ({
        ...
        breadcrumb: drawing.parentId ? [
          { id: "d1", number: "P&ID-001", title: "Main Process Flow" }
        ] : [],
      }));
  ...
  http.get(`${BASE_URL}/api/pipetrak/drawings/project/:projectId`, ({ params }) => {
    const { projectId } = params;
    ...
    return HttpResponse.json(mockFlatDrawings.map(d => ({
      ...d,
...
export const errorHandlers = [
  ...
  http.get(`${BASE_URL}/api/pipetrak/drawings/project/:projectId/hierarchy`, () => {
    return HttpResponse.error();
  }),

  // Timeout simulation
  http.get(`${BASE_URL}/api/pipetrak/drawings/:drawingId/details`, async () => {
    await new Promise(resolve => setTimeout(resolve, 60000)); // 60 second delay
    return HttpResponse.json({});
  }),

  // Server error simulation
  http.get(`${BASE_URL}/api/pipetrak/drawings/project/:projectId/search`, () => {
    return HttpResponse.json(
...
export const performanceHandlers = [
  ...
  http.get(`${BASE_URL}/api/pipetrak/drawings/project/large/hierarchy`, () => {
  ...
  http.get(`${BASE_URL}/api/pipetrak/drawings/large/details`, ({ request }) => {
    const url = new URL(request.url);
    const page = parseInt(url.searchParams.get("page") || "1");
    const limit = parseInt(url.searchParams.get("limit") || "50");
...
export const allDrawingHandlers = [
  ...drawingHandlers,
  ...performanceHandlers,
];