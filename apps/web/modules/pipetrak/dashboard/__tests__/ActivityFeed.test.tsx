...
import { smallRecentActivity, generateRecentActivity } from '../__fixtures__/dashboard-data';
...
describe('ActivityFeed', () => {
  ...
  describe('User Filtering', () => {
    it('displays user filter dropdown', () => {
      ...
      expect(screen.getByTestId('activity-user-filter')).toBeInTheDocument();
    });
  ...
  describe('Activity Icons and Visual Elements', () => {
    ...
    it('uses different colors for different activity types', () => {
      const { container } = render(<ActivityFeed data={smallRecentActivity} />);
      
      // Should have different styling for completion vs update activities
      const completionItems = container.querySelectorAll('[class*="text-green"], [class*="bg-green"]');
      const updateItems = container.querySelectorAll('[class*="text-blue"], [class*="bg-blue"]');
      
      expect(completionItems.length + updateItems.length).toBeGreaterThan(0);
    });
  });
  ...
  describe('Sparklines', () => {
    ...
    it('sparklines contain SVG elements', () => {
      ...
      const sparklineSvgs = container.querySelectorAll('[data-testid="activity-sparkline"] svg');
  ...
  describe('Performance', () => {
    ...
    it('virtual scrolling works with large datasets', () => {
      ...
      const { container } = render(<ActivityFeed data={largeActivityData} />);
      
      // Should use virtual scrolling for large datasets
      const scrollContainer = container.querySelector('[data-testid="activity-scroll-container"]');
  ...
  describe('Edge Cases', () => {
    ...
    it('handles activities with very long component names', () => {
        ...
        activities: [
          {
            ...
            userId: 'user-1',
            userName: 'Test User',
            componentId: 'VERY-LONG-COMPONENT-ID-THAT-MIGHT-OVERFLOW-THE-UI-LAYOUT',
            componentType: 'Very Long Component Type Name That Should Be Handled Gracefully',
            milestoneName: 'Very Long Milestone Name That Should Not Break The Layout',
            details: {
              componentId: 'VERY-LONG-COMPONENT-ID-THAT-MIGHT-OVERFLOW-THE-UI-LAYOUT',
              componentType: 'Very Long Component Type Name That Should Be Handled Gracefully',
              milestoneName: 'Very Long Milestone Name That Should Not Break The Layout',
      ...
      expect(() => render(<ActivityFeed data={longNameData} />)).not.toThrow();
    });
    ...
    it('handles invalid timestamp data', () => {
      ...
      expect(() => render(<ActivityFeed data={invalidTimestampData} />)).not.toThrow();
    });
  });
