...
describe("TestPackageTable", () => {
	describe("Empty State", () => {
		it("renders empty state when data is null", () => {
			...
			expect(
				screen.getByText("Test Package Readiness"),
			).toBeInTheDocument();
			expect(
				screen.getByText("No test packages configured"),
			).toBeInTheDocument();
		});
	...
	describe("Table Rendering", () => {
		...
		it("renders all package rows with correct data", () => {
			...
			expect(
				screen.getByText("Pressure Test Package A"),
			).toBeInTheDocument();
			expect(screen.getByText("Leak Test Package B")).toBeInTheDocument();
			expect(
				screen.getByText("Function Test Package C"),
			).toBeInTheDocument();
	...
	describe("Ready Status Logic", () => {
		it("correctly identifies ready packages", () => {
			...
			const readyRow = screen
				.getByText("Pressure Test Package A")
				.closest("tr");
		...
		it("correctly identifies not ready packages", () => {
			...
			const notReadyRow = screen
				.getByText("Leak Test Package B")
				.closest("tr");
	...
	describe("Visual Styling", () => {
		it("applies correct styling to ready badges", () => {
			const { container } = render(
				<TestPackageTable data={smallTestPackageReadiness} />,
			);
		...
		it("applies correct styling to not ready badges", () => {
			...
			const notReadyBadges = container.querySelectorAll(
	...
	describe("Accessibility", () => {
		...
		it("provides sortable column indicators", async () => {
			...
			const sortableHeaders = screen
				.getAllByRole("button")
				.filter((button) =>
					["Package Name", "Components", "Completion %"].some(
						(text) => button.textContent?.includes(text),
					),
				);
	...
	describe("Data Formatting", () => {
		...
		it("handles very long package names gracefully", () => {
				...
				testPackages: [
					{
						packageId: "long-name",
						packageName:
							"Very Long Test Package Name That Might Overflow The Table Cell Width",
						totalComponents: 10,
			...
			expect(
				screen.getByText(/Very Long Test Package Name/),
			).toBeInTheDocument();
		});
	});
	...
	describe("Edge Cases", () => {
		...
		it("handles undefined or invalid data gracefully", () => {
			...
			expect(() =>
				render(<TestPackageTable data={invalidData} />),
			).not.toThrow();
		});
	});
});
