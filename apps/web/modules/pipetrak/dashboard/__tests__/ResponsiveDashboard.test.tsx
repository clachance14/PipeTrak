...
vi.mock('../components/TabletDashboard', () => ({
  TabletDashboard: ({ projectName, metrics, onProjectChange, onRefresh }: any) => (
    <div data-testid="tablet-dashboard">
      <span>Tablet Dashboard: {projectName}</span>
      <span>Components: {metrics?.totalComponents || 0}</span>
      <button onClick={() => onProjectChange('test-project')}>Change Project</button>
...
vi.mock('../components/MobileDashboard', () => ({
  MobileDashboard: ({ projectName, metrics, onProjectChange, onRefresh }: any) => (
...
describe('ResponsiveDashboard', () => {
  ...
  describe('Error States', () => {
    ...
    it('shows warning when metrics are missing', () => {
      ...
      expect(screen.getByText('Unable to load dashboard metrics. Please try refreshing the page.')).toBeInTheDocument();
    });
  });
  ...
  describe('Desktop Layout (≥1024px)', () => {
    ...
    it('renders desktop layout components', () => {
      ...
      expect(screen.getByTestId('test-package-table')).toBeInTheDocument();
    ...
    it('displays project name and metrics correctly', () => {
      ...
      expect(screen.getByText(`${defaultProps.project!.jobName} Dashboard`)).toBeInTheDocument();
  ...
  describe('Tablet Layout (768-1024px)', () => {
    ...
    it('does not render desktop components in tablet layout', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByTestId('area-system-grid')).not.toBeInTheDocument();
        expect(screen.queryByTestId('drawing-hierarchy')).not.toBeInTheDocument();
      });
    });
    ...
    it('handles project change in tablet layout', async () => {
      ...
      expect(mockPush).toHaveBeenCalledWith('/app/pipetrak/test-project/dashboard');
    });
  });
  ...
  describe('Mobile Layout (<768px)', () => {
    ...
    it('does not render desktop or tablet components in mobile layout', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByTestId('dashboard-top-bar')).not.toBeInTheDocument();
        expect(screen.queryByTestId('area-system-grid')).not.toBeInTheDocument();
        expect(screen.queryByTestId('tablet-dashboard')).not.toBeInTheDocument();
      });
    });
    ...
    it('handles empty data gracefully', () => {
      render(
        <ResponsiveDashboard
          ...
          testPackageReadiness={emptyDashboardData.testPackageReadiness}
  ...
  describe('Navigation Handlers', () => {
    it('handles project change correctly', () => {
      const { rerender } = render(<ResponsiveDashboard {...defaultProps} />);
      ...
      expect(screen.getByText('New Project Dashboard')).toBeInTheDocument();
    });
  ...
  describe('Event Listener Cleanup', () => {
    it('removes resize event listener on unmount', () => {
      ...
      const { unmount } = render(<ResponsiveDashboard {...defaultProps} />);
  ...
  describe('Performance', () => {
    ...
    it('handles large datasets efficiently', () => {
      const largeMockData = {
        ...defaultProps,
        areaSystemMatrix: {
          matrixData: Array(1000).fill(null).map((_, i) => ({
            area: `Area-${i}`,
            system: `System-${i}`,
            totalCount: 10,
            completedCount: 5,
            completionPercent: 50,
            stalledCounts: { stalled7Days: 1, stalled14Days: 0, stalled21Days: 0 },
          })),
