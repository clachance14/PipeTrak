...
export async function fetchDashboardComponentsClient(
  ...
  try {
    ...
    if (filters.testPackage?.length) params.set("testPackage", filters.testPackage.join(","));
    if (filters.drawing?.length) params.set("drawing", filters.drawing.join(","));
    if (filters.search) params.set("search", filters.search);

    const response = await fetch(`/api/pipetrak/dashboard/components?${params}`);
    ...
    console.error("[Client API] Error fetching dashboard components:", error);
