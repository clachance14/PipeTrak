...
import type { DashboardData, DashboardMetrics, AreaSystemMatrix, DrawingRollups, TestPackageReadiness, RecentActivity } from "../types";
...
export async function getDashboardData(
  ...
  try {
    ...
    if (!project) {
      console.log('[getDashboardData] No project found - returning null data');
      return {
        metrics: null,
    ...
    return {
      ...
      areaSystemMatrix: matrixResult.status === 'fulfilled' ? matrixResult.value : null,
      drawingRollups: drawingsResult.status === 'fulfilled' ? drawingsResult.value : null,
      testPackageReadiness: packagesResult.status === 'fulfilled' ? packagesResult.value : null,
 ...
 */
export async function fetchDashboardMetrics(projectId: string): Promise<DashboardMetrics | null> {
  const supabase = await createClient();
 ...
 */
export async function fetchAreaSystemMatrix(projectId: string): Promise<AreaSystemMatrix | null> {
  const supabase = await createClient();
      ...
      matrixData: [
        {
          ...
          stalledCounts: { stalled7Days: 5, stalled14Days: 3, stalled21Days: 1 }
        },
        {
          area: 'Area 100',
          system: 'Electrical',
          ...
          completedCount: 16,
          completionPercent: 40,
          stalledCounts: { stalled7Days: 2, stalled14Days: 1, stalled21Days: 0 }
        },
        {
          area: 'Area 200',
          system: 'Piping',
          ...
          completedCount: 32,
          completionPercent: 40,
          stalledCounts: { stalled7Days: 6, stalled14Days: 4, stalled21Days: 2 }
        },
        {
          area: 'Area 200',
          system: 'Electrical',
          ...
          completedCount: 14,
          completionPercent: 40,
          stalledCounts: { stalled7Days: 3, stalled14Days: 2, stalled21Days: 1 }
        }
      ],
      generatedAt: Date.now()
    };
 ...
 */
export async function fetchDrawingRollups(projectId: string): Promise<DrawingRollups | null> {
  const supabase = await createClient();
 ...
 */
export async function fetchTestPackageReadiness(projectId: string): Promise<TestPackageReadiness | null> {
  const supabase = await createClient();
  
  try {
    const { data, error } = await supabase.rpc('get_test_package_readiness', {
      p_project_id: projectId
    });
 ...
 */
export async function fetchRecentActivity(projectId: string, limit = 50): Promise<RecentActivity | null> {
  const supabase = await createClient();
...
export async function fetchDashboardComponents(
  ...
  try {
    ...
    const transformedComponents = components.map(component => ({
      ...component,
      description: `${component.type || ''} ${component.spec || ''} ${component.size || ''}`.trim() || null,
