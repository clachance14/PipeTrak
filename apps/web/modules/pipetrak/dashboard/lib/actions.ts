...
export async function getProjectDashboard(projectId: string) {
  ...
  const notStartedComponents = components.filter(c => c.status === 'NOT_STARTED').length;
  
  const overallProgress = totalComponents > 0 
    ? components.reduce((sum, c) => sum + c.completionPercent, 0) / totalComponents
    : 0;
  ...
  components.forEach(c => {
    if (c.area) {
      const current = areaProgress.get(c.area) || { count: 0, progress: 0 };
  ...
  const areaStats = Array.from(areaProgress.entries()).map(([area, stats]) => ({
    area,
  ...
  const systemProgress = new Map<string, { count: number; progress: number }>();
  components.forEach(c => {
    if (c.system) {
      const current = systemProgress.get(c.system) || { count: 0, progress: 0 };
  ...
  const systemStats = Array.from(systemProgress.entries()).map(([system, stats]) => ({
    system,
    componentCount: stats.count,
    averageProgress: stats.progress / stats.count
