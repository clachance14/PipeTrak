...
export function formatActivityDescription(
  ...
  const action = activityType === 'milestone_completed' ? 'completed' : 'updated';
...
export function getTotalStalledCount(stalledCounts: {
  ...
  return stalledCounts.stalled7Days + stalledCounts.stalled14Days + stalledCounts.stalled21Days;
}

/**
 * Build hierarchical tree from flat drawing array
 */
export function buildDrawingTree(drawings: Array<{
  drawingId: string;
 ...
 */
export function getUniqueSystems(matrixData: Array<{ system: string }>): string[] {
