 ...
 */
export function QuickStatsChips({ metrics, testPackages }: QuickStatsChipsProps) {
  if (!metrics) {
    return (
      <div className="flex gap-2 overflow-hidden">
        <div className="animate-pulse">
          <Badge variant="secondary" className="h-8 w-20"></Badge>
        </div>
        <div className="animate-pulse">
          <Badge variant="secondary" className="h-8 w-24"></Badge>
        </div>
        <div className="animate-pulse">
          <Badge variant="secondary" className="h-8 w-16"></Badge>
        </div>
      </div>
    );
  }

  const readyPackages = testPackages?.testPackages.filter(pkg => pkg.isReady).length || 0;
        ...
        {statsChips.map((chip) => {
          ...
          return (
            <Badge
              ...
              `}
            >
              <Icon className={`${chip.primary ? 'h-4 w-4' : 'h-3 w-3'}`} />
