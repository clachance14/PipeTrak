...
function DrawingNodeContainer({ node, level }: { node: DrawingTreeNode; level: number }) {
...
export function DrawingHierarchy({ data }: DrawingHierarchyProps) {
  ...
  const totalStats = useMemo(() => {
    if (!data?.drawings?.length) {
      return { totalComponents: 0, completedComponents: 0, totalStalled: 0 };
    }

    return data.drawings.reduce(
      (acc, drawing) => ({
        totalComponents: acc.totalComponents + drawing.componentCount,
        completedComponents: acc.completedComponents + drawing.completedCount,
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Hierarchical tree view of drawings with progress rollups.
          </CardDescription>
        </CardHeader>
        <CardContent>
  ...
  const overallProgress = totalStats.totalComponents > 0 
    ? Math.round((totalStats.completedComponents / totalStats.totalComponents) * 100)
    : 0;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {totalStats.totalComponents.toLocaleString()} total components, {overallProgress}% complete
