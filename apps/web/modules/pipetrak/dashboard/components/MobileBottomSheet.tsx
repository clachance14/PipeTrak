...
export function MobileBottomSheet({
	...
	const [updatingMilestones, setUpdatingMilestones] = useState<Set<string>>(
		new Set(),
	);
							...
							<div className="space-y-3 pr-4">
								...
								component.milestones.length > 0 ? (
									component.milestones
										...
										.map((milestone) => (
											<MilestoneItem
												key={milestone.id}
												milestone={milestone}
												isUpdating={updatingMilestones.has(
													milestone.id,
												)}
												onToggle={(completed) =>
									...
									<div className="text-center py-8 text-muted-foreground">
										...
										<p className="text-sm">
											Contact your project manager to set
											up milestones
										</p>
									</div>
								)}
				...
				<div className="flex items-start justify-between">
					<div className="flex-1 min-w-0">
						...
						{(milestone.percentageComplete !== null ||
							milestone.quantityComplete !== null) && (
							<div className="mt-1 text-xs text-muted-foreground">
								{milestone.percentageComplete !== null && (
									<span>
										{milestone.percentageComplete}% complete
									</span>
								)}
								{milestone.quantityComplete !== null &&
									milestone.quantityTotal && (
										<span>
											{milestone.quantityComplete}/
											{milestone.quantityTotal} units
										</span>
									)}
							</div>
						)}
