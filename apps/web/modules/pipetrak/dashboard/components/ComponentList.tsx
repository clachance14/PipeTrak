...
export function ComponentList({
  ...
  const isStalled = (component: ComponentWithMilestones): boolean => {
    ...
    const daysSinceUpdate = Math.floor(
      (Date.now() - new Date(component.updatedAt).getTime()) / (1000 * 60 * 60 * 24)
    );
        ...
        <div className="space-y-2">
              ...
              >
                <Switch
                  ...
                  onCheckedChange={(checked) => 
                    onMilestoneToggle?.(component.id, milestone.id, checked)
                  }
                  className="scale-75"
                />
          ...
          {hasMoreMilestones && (
            <div className="text-xs text-muted-foreground text-center pt-1">
              +{(component.milestones?.length || 0) - 4} more milestones (tap to view all)
            </div>
          )}
        </div>
      )}
