...
export function ResponsiveDashboard({
  ...
  useEffect(() => {
    if (showMilestoneMatrix && project?.id && components.length === 0) {
      loadComponents();
    }
  ...
  const loadComponents = async () => {
    ...
    try {
      const result = await fetchDashboardComponentsClient(project.id, {}, 500, 0);
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-center">
        <ToggleGroup 
          ...
          onValueChange={(value) => setShowMilestoneMatrix(value === 'milestone')}
          className="bg-muted/50"
        >
          <ToggleGroupItem value="area" aria-label="Area × System View">
            <LayoutGrid className="h-4 w-4 mr-2" />
            Area × System
          </ToggleGroupItem>
          <ToggleGroupItem value="milestone" aria-label="Milestone Progress View">
