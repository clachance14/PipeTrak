...
export function AreaSystemGrid3x3({ data }: AreaSystemGrid3x3Props) {
		...
		return (
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Interactive grid showing completion status by area and
						system combinations.
					</CardDescription>
				</CardHeader>
				<CardContent>
	...
	const calculatedCellSize = Math.min(
		...
		Math.max(
			minCellSize,
			Math.floor(availableWidth / Math.max(systems.length, 1)),
		),
	);
		...
		<TooltipProvider>
			<Card>
				<CardHeader>
					<CardTitle>Area × System Progress Matrix</CardTitle>
					<CardDescription>
						Completion percentage by area and system. Click cells to
						drill down.
					</CardDescription>
				</CardHeader>
				<CardContent>
						...
						>
							...
							{areas.map((area, rowIndex) =>
								systems.map((system, colIndex) => {
									...
									const totalStalled = getTotalStalledCount(
										item.stalledCounts,
									);
									const fillColor = getHeatmapFillColor(
					...
					<div className="mt-4 flex items-center justify-center text-sm text-muted-foreground">
						<div className="flex items-center space-x-4 mx-auto">
							...
							<div className="ml-8 pl-8 border-l">
								<p className="text-xs">
									{areas.length} areas × {systems.length}{" "}
									systems = {data.matrixData.length}{" "}
									combinations
								</p>
							</div>
						</div>
