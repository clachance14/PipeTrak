...
import { Activity, Filter, User, Wrench } from 'lucide-react';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@ui/components/card';
import { ScrollArea } from '@ui/components/scroll-area';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@ui/components/select';
import { Badge } from '@ui/components/badge';
import { Avatar, AvatarFallback } from '@ui/components/avatar';
import { formatRelativeTime, formatActivityDescription, generateSparklinePath } from '../lib/utils';
...
function ActivityItemComponent({ activity }: ActivityItemComponentProps) {
  ...
  const activityIcon = activity.activityType === 'milestone_completed' 
    ...
    <div className="flex items-start gap-3 p-3 hover:bg-muted/50 rounded-lg transition-colors">
      ...
      <div className="flex-1 min-w-0 space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          {activity.activityType === 'milestone_completed' ? 'completed' : 'updated'} 
          <span className="font-medium text-foreground mx-1">
            {activity.componentId}
          </span>
          {activity.milestoneName && (
            <>
              milestone: <span className="font-medium text-foreground">{activity.milestoneName}</span>
            </>
          )}
        </p>
...
function ActivitySparkline({ activities }: { activities: ActivityItem[] }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs text-muted-foreground">Activity trend:</span>
      <svg width="100" height="20" className="text-blue-500">
        ...
        {dailyCounts.map((count, index) => {
          const x = 2 + (index * (100 - 4)) / Math.max(dailyCounts.length - 1, 1);
...
export function ActivityFeed({ data }: ActivityFeedProps) {
  ...
  const { filteredActivities, uniqueUsers, uniqueAreas, uniqueSystems } = useMemo(() => {
    ...
    const uniqueUsers = [...new Set(data.activities.map(a => a.userName))].sort();
    ...
    return data.activities.reduce(
      (acc, activity) => {
        ...
        return {
          ...
          thisWeek: acc.thisWeek + (activityTime >= weekStart ? 1 : 0)
        };
      },
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Live feed of milestone completions, component updates, and project activity.
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <div className="space-y-1 pr-4">
            {filteredActivities.length > 0 ? (
              filteredActivities.map((activity, index) => (
                <ActivityItemComponent key={`${activity.componentId}-${activity.timestamp}-${index}`} activity={activity} />
              ))
            ) : (
