...
export const dashboardHandlers = [
  ...
  http.post(`${API_BASE}/get_dashboard_metrics`, async ({ request }) => {
    ...
    if (shouldSimulateError) {
      return HttpResponse.json(createErrorResponse(errorMessage), { status: 500 });
    }
    ...
    if (!projectId) {
      return HttpResponse.json(createErrorResponse('Project ID is required'), { status: 400 });
    }
    ...
    switch (scenario) {
      ...
      case 'stalled':
        ...
        break;
      case 'error':
        return HttpResponse.json(createErrorResponse('Project not found'), { status: 404 });
      default:
        metrics = smallDashboardData.metrics;
    }
  ...
  http.post(`${API_BASE}/get_recent_activity`, async ({ request }) => {
    ...
    const body = await request.json() as { project_id: string; activity_limit?: number };
    const projectId = body.project_id;
  ...
  ...dashboardHandlers.map(handler => {
    ...
    if (handler.info?.path?.includes('/rpc/')) {
      return http.post(handler.info.path, () => {
        return HttpResponse.json(createErrorResponse('Database connection failed'), { status: 500 });
      });
    }
    return handler;
  }),
  ...
  return [
    http.post(`${API_BASE}/get_dashboard_metrics`, async ({ request }) => {
      ...
      if (body.project_id === projectId) {
        return HttpResponse.json(customData.metrics || smallDashboardData.metrics);
      }
      return HttpResponse.json(smallDashboardData.metrics);
    }),
