...
import { type CreateWelderData, useCreateWelder } from "../hooks/useWelders";
...
export function AddWelderModal({
	open,
	onOpenChange,
	projectId,
}: AddWelderModalProps) {
		...
		<Dialog open={open} onOpenChange={handleClose}>
			<DialogContent className="sm:max-w-[500px]">
				<DialogHeader>
					...
					<DialogDescription>
						Create a new welder record. Stencil must be unique
						across all projects.
					</DialogDescription>
				</DialogHeader>
				...
				<form onSubmit={handleSubmit} className="space-y-4">
					...
					<div className="space-y-2">
						...
						<Input
							...
							onChange={(e) =>
								updateFormData("stencil", e.target.value)
							}
						...
						/>
						{validationErrors.stencil && (
							<p className="text-base font-medium text-destructive bg-destructive/10 p-3 rounded-md">
								{validationErrors.stencil}
							</p>
						)}
						<p className="text-sm text-muted-foreground font-medium">
							Letters, numbers, and hyphens only. Must be globally
							unique.
						</p>
					</div>
					...
					<div className="flex items-center space-x-2">
						<Checkbox
							...
							onCheckedChange={(checked) =>
								updateFormData("active", Boolean(checked))
							}
						/>
