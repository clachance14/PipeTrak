	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (!formData.welderId) {
			validationErrors.push(
				"Please select a welder before creating the weld.",
			);
		}
		...
		try {
			...
			if (response.ok) {
				...
				setErrors([
					error.message || "Failed to create weld. Please try again.",
				]);
			}
		} catch (error) {
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-[600px]">
				<DialogHeader>
					...
					<DialogDescription>
						Create a new field weld record with weld details and
						quality information
					</DialogDescription>
				</DialogHeader>
				...
				<form onSubmit={handleSubmit} className="space-y-4">
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
						...
						<div className="space-y-2">
							...
							<Input
								...
								onChange={(e) =>
									updateFormData("drawingId", e.target.value)
								}
						...
						<div className="space-y-2">
							...
							<Input
								...
								onChange={(e) =>
									updateFormData("weldSize", e.target.value)
								}
						...
						<div className="space-y-2">
							...
							<Select
								...
								onValueChange={(value) =>
									updateFormData("weldTypeCode", value)
								}
								required
							>
								<SelectTrigger>
							...
							{welders.length === 0 && !weldersLoading && (
								<Alert variant="destructive" className="mt-2">
									...
									<AlertDescription className="text-sm">
										No active welders available. Add welders
										in the Welders section first.
									</AlertDescription>
								</Alert>
							)}
						</div>
						...
						<div className="space-y-2">
							...
							<Input
								...
								onChange={(e) =>
									updateFormData("baseMetal", e.target.value)
								}
					...
					<div className="flex items-center space-x-2">
						<input
							...
							onChange={(e) =>
								updateFormData("pwhtRequired", e.target.checked)
							}
							className="h-4 w-4"
						/>
