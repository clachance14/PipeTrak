...
export function MobileQCView({
	...
	const stats = useMemo(() => {
		...
		const pwhtCompleted = data.filter(
			(w) => w.pwhtRequired && w.datePwht,
		).length;
	...
	const filteredData = useMemo(() => {
		return data.filter((weld) => {
			...
			// Package filter
			const packageMatch =
				packageFilter === "all" || weld.packageNumber === packageFilter;
	...
	const fetchFieldWelds = async () => {
		...
		try {
			...
			if (response.ok) {
				...
				const normalized: FieldWeldData[] = (
					result.fieldWelds || []
				).map((weld: any) => {
	...
	const handleApproveWeld = async (weldId: string) => {
		try {
			const response = await fetch(
				`/api/pipetrak/field-welds/${weldId}`,
				{
					method: "PATCH",
				...
				{loading ? (
					...
					filteredData.map((weld) => (
						<FieldWeldCard
							...
							onSelect={(selected) =>
								handleSelectWeld(weld.id, selected)
							}
