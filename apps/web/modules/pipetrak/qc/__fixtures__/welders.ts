...
export const generateMockWelders = (count: number, projectId = 'test-project-id'): Welder[] => {
  const firstNames = ['John', 'Sarah', 'Bob', 'Mike', 'Lisa', 'Dave', 'Anna', 'Chris', 'Emma', 'Ryan'];
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      projectId,
      stencil: useHyphen ? `${initials}-${number}` : `${initials}${number}`,
