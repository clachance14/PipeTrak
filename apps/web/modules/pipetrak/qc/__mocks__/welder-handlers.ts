...
import { mockWelders, mockFieldWelds } from '../__fixtures__/welders';
import type { Welder, CreateWelderData, UpdateWelderData } from '../hooks/useWelders';
...
const findWelderByStencil = (stencil: string, excludeId?: string) => {
  return welderDatabase.find(w => 
    w.stencil.toLowerCase() === stencil.toLowerCase() && w.id !== excludeId
  );
};
...
export const welderHandlers = [
  ...
  http.put(`${API_BASE}/welders/:id`, async ({ params, request }) => {
    ...
    return HttpResponse.json(
      { welder: updatedWelder, message: 'Welder updated successfully' }
    );
  }),
  ...
  http.get(`${API_BASE}/welders/:id/stats`, ({ params }) => {
    ...
    const stats = {
      ...
      lastWeldDate: welder.weldCount > 0 ? new Date().toISOString() : null,
  ...
  http.post(`${API_BASE}/field-welds`, async ({ request }) => {
    ...
    const welderIndex = welderDatabase.findIndex(w => w.id === data.welderId);
    if (welderIndex !== -1) {
      welderDatabase[welderIndex] = {
        ...welderDatabase[welderIndex],
