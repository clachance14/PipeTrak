...
export function FieldWeldValidationPreview({
  ...
  const filteredAndSortedIssues = useMemo(() => {
    ...
    if (filters.field !== 'all') {
      filtered = filtered.filter(issue => issue.field === filters.field);
    }
    ...
    if (filters.showOnlyErrors) {
      filtered = filtered.filter(issue => issue.category === ValidationCategory.ERROR);
    }
  ...
  return (
    <div className={cn("space-y-6", className)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4">
            ...
            <div className="text-2xl font-bold">{validationReport.summary.totalRows}</div>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-xs text-muted-foreground">
              {((validationReport.summary.validRows / validationReport.summary.totalRows) * 100).toFixed(1)}% success rate
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-xs text-muted-foreground">Must be fixed to proceed</p>
          </CardContent>
        </Card>
      ...
      {(validationReport.inheritanceApplied.testPressure > 0 || validationReport.inheritanceApplied.specCode > 0) && (
        <Alert>
          ...
          <AlertDescription>
            <div className="mt-2 space-y-1">
              {validationReport.inheritanceApplied.testPressure > 0 && (
                <p>• Test Pressure inherited for {validationReport.inheritanceApplied.testPressure} welds</p>
              )}
              {validationReport.inheritanceApplied.specCode > 0 && (
                <p>• Spec Code inherited for {validationReport.inheritanceApplied.specCode} welds</p>
              )}
            </div>
          </AlertDescription>
        </Alert>
      ...
      {validationReport.recommendations.length > 0 && (
        <Card>
          ...
          <CardContent className="pt-0">
            <div className={cn(
              "space-y-2",
              !expandedRecommendations && validationReport.recommendations.length > 3 && "max-h-24 overflow-hidden"
            )}>
                    ...
                    <TableRow key={index} className="group">
                      ...
                      <TableCell>
                        <Badge 
                          ...
                          className={cn(
                            "text-xs",
                            issue.category === ValidationCategory.WARNING && "bg-yellow-100 text-yellow-800 hover:bg-yellow-200"
                          )}
                        >
                          <div className="flex items-center gap-1">
        ...
        <div className="flex items-center gap-2">
          {validationReport.summary.errorCount > 0 ? (
            <Alert className="inline-flex p-2 border-red-200">
              ...
              <span className="text-sm">Fix {validationReport.summary.errorCount} errors to proceed</span>
            </Alert>
          ) : (
            onProceedToImport && (
