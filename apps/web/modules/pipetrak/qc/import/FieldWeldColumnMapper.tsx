...
const convertMappingsToDetectionFormat = (
	mappings: Record<string, string>,
): ColumnMappings => {
...
export function FieldWeldColumnMapper({
	...
	const [manualOverrides, setManualOverrides] = useState<Set<number>>(
		new Set(),
	);
	...
	useEffect(() => {
		if (parsedData.headers.length > 0) {
			...
			if (!hasInitialMappingsRef.current) {
				const newMappings = convertDetectionToMappingsFormat(
					result.mappings,
				);
	...
	const handleColumnMappingChange = (
		...
		Object.keys(newMappings).forEach((key) => {
			if (
				newMappings[key] === fieldValue &&
				Number(key) !== columnIndex
			) {
		...
		<div className="space-y-6">
				...
				<div className="flex items-center gap-3">
					...
					<div>
						<p className="font-medium">
							{parsedData.metadata?.filename || "WELD LOG.xlsx"}
						</p>
			...
			<Card>
				<CardHeader>
					...
					<p className="text-sm text-muted-foreground">
						...
						{mappingSummary?.mapped.length || 0} matching fields
					</p>
				</CardHeader>
				<CardContent>
									...
									<div className="space-y-2">
										...
										{validationStatus.recommendations
											...
											<ul className="list-disc list-inside space-y-1 text-sm">
												{validationStatus.recommendations.map(
													(rec, index) => (
														<li key={index}>
															{rec}
														</li>
						...
						<div className="grid grid-cols-3 gap-4">
							<div className="text-center p-3 bg-green-50 rounded-lg border border-green-200">
								<div className="text-2xl font-bold text-green-700">
									{
										detectionResult.essentialFields.found
											.length
									}
								</div>
							...
							<div className="text-center p-3 bg-blue-50 rounded-lg border border-blue-200">
								<div className="text-2xl font-bold text-blue-700">
									{
										detectionResult.optionalFields.found
											.length
									}
								</div>
			...
			<Card>
				<CardHeader>
					...
					<p className="text-sm text-muted-foreground">
						Review and adjust the automatically detected field
						mappings below
					</p>
				</CardHeader>
				<CardContent>
								...
								<div className="space-y-2">
									{parsedData.headers.map((header, index) => {
										...
										const currentMapping =
											mappings[index.toString()] || "";
										const priority = currentMapping
											? getFieldPriority(currentMapping)
											: "optional";
										const isManualOverride =
											manualOverrides.has(index);
										const isAutoDetected =
											!isManualOverride && currentMapping;

										return (
											<div
												key={index}
												className="grid grid-cols-5 gap-4 items-center p-3 rounded-lg hover:bg-muted/50 border"
											>
			...
			{showPreview && previewRows.length > 0 && (
				<Card>
					<CardHeader>
						...
						<p className="text-sm text-muted-foreground">
							Preview of how your data will be imported with
							current mappings
						</p>
					</CardHeader>
					<CardContent>
