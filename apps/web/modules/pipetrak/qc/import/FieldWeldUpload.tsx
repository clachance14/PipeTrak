...
import { Upload, FileSpreadsheet, X, AlertCircle, Download, CheckCircle22 } from "lucide-react";
import { Alert, AlertDescription } from "@ui/components/alert";
import { cn } from "@ui/lib";
// Using basic file validation instead of Excel parsing

interface FieldWeldUploadProps {
  onFileSelect: (file: File, validationResult: FieldWeldValidationResult) => void;
  onFileRemove: () => void;
  onImportClick?: (file: File, validationResult: FieldWeldValidationResult) => void;
  loading?: boolean;
  progress?: number;
  error?: string;
  className?: string;
...
export function FieldWeldUpload({
  ...
  const validateWeldLogFormat = useCallback(async (file: File): Promise<FieldWeldValidationResult> => {
    ...
    try {
      ...
      const isLikelyWeldLog = file.name.toLowerCase().includes('weld') || 
      ...
      if (isLikelyWeldLog) {
        ...
        const estimatedRows = Math.max(1, Math.floor(file.size / 150) - 1); // -1 for header
        ...
        return {
          ...
          metadata: {
            ...
            requiredColumnsFound: ['Weld ID Number (Column A)', 'Drawing Number (Column D)'],
            missingColumns: []
          }
        };
      }
  ...
  if (selectedFile && validationResult?.isValid && !loading && !isValidating) {
    return (
      <Card className={className}>
        ...
        <CardContent className="space-y-4">
              ...
              <div>
                ...
                {validationResult.metadata && (
                  <p className="text-sm text-green-600">
                    {validationResult.metadata.totalRows} rows, {validationResult.metadata.totalColumns} columns detected
                  </p>
                )}
              </div>
            </div>
          ...
          {validationResult.metadata?.requiredColumnsFound && validationResult.metadata.requiredColumnsFound.length > 0 && (
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-sm font-medium text-green-800 mb-2">Validated Format:</p>
          ...
          {onImportClick && !importResult?.success && (
            <div className="flex gap-3 pt-2">
              <Button
                onClick={() => onImportClick(selectedFile, validationResult)}
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium">
                {isValidating ? "Validating WELD LOG format..." : "Processing file..."}
              </span>
            </div>
            <Progress value={isValidating ? 50 : progress} className="w-full" />
            <p className="text-sm text-muted-foreground">
              {isValidating ? "Checking file format and structure..." : `${progress}% complete`}
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className={className}>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-sm font-medium text-blue-800 mb-2">Need the correct format?</p>
            <Button 
              variant="outline" 
              size="sm"
  ...
  return (
    <Card className={className}>
      <CardHeader>
        ...
        <CardDescription>
          Select a WELD LOG Excel file (.xlsx or .xls) to import field weld data. Files up to {MAX_FILE_SIZE_MB}MB are supported.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm text-muted-foreground max-w-md mx-auto">
                Only Excel files (.xlsx, .xls) are accepted. The file will be validated 
                against the standard WELD LOG format with columns A-AA.
              </p>
            </div>

            <Button variant="outline" disabled={loading || isValidating} className="mt-4">
              <Upload className="h-4 w-4 mr-2" />
              Select Excel File
            </Button>
          </div>
        ...
        <div className="border rounded-lg p-4 bg-muted/30">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-medium text-sm">Need the WELD LOG template?</p>
              <p className="text-sm text-muted-foreground">
                Download the official template with the correct column structure
              </p>
            </div>
            <Button 
              variant="outline" 
