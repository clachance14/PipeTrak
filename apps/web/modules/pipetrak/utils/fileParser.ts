...
export async function parseFileHeaders(file: File): Promise<{ headers: string[], sampleData?: any[] }> {
...
async function parseCSVHeadersAndSample(file: File): Promise<{ headers: string[], sampleData?: any[] }> {
...
async function parseExcelHeadersAndSample(file: File): Promise<{ headers: string[], sampleData?: any[] }> {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    ...
    reader.onload = (e) => {
      try {
        ...
        const headers = sheetData[0].filter(header => header !== undefined && header !== null && header !== '');
...
async function parseExcelHeaders(file: File): Promise<string[]> {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
