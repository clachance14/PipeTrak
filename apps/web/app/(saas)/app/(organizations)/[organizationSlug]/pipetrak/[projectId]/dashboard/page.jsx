...
async function DashboardContent({ projectId }) {
    ...
    const { project, metrics, drawingRollups, testPackageReadiness, recentActivity, } = dashboardData;
    return (<ResponsiveDashboard project={project} metrics={metrics} drawingRollups={drawingRollups} testPackageReadiness={testPackageReadiness} recentActivity={recentActivity} availableProjects={[]} // TODO: Fetch user's available projects
    />);
}
// Loading skeleton component
function DashboardSkeleton() {
    return (<div className="space-y-6 animate-pulse px-4 lg:px-6">
			{/* Top bar skeleton */}
			<div className="flex items-center justify-between gap-4 py-4">
				<div className="h-10 bg-gray-200 rounded w-[280px]"/>
				<div className="flex gap-2">
					{[1, 2, 3, 4].map((i) => (<div key={i} className="h-8 bg-gray-200 rounded w-20"/>))}
				</div>
			</div>
			...
			<div className="grid gap-4 grid-cols-2 lg:grid-cols-5">
				{[1, 2, 3, 4, 5].map((i) => (<div key={i} className="p-6 border rounded-lg">
			...
			<div className="space-y-6">
				<div className="h-80 bg-gray-100 rounded-lg border-2 border-dashed border-gray-300"/>
