	...
	try {
		...
		if (!debugInfo.checks.hasDatabase) {
			debugInfo.warnings.push(
				"DATABASE_URL or POSTGRES_URL not configured - authentication will fail",
			);
		}

		if (
			!debugInfo.checks.hasAuthSecret &&
			process.env.NODE_ENV !== "development"
		) {
			debugInfo.warnings.push(
		...
		if (authConfig.trustAllOrigins) {
			debugInfo.warnings.push(
				"Accepting all origins - only safe in development",
			);
		}
		...
		if (process.env.NODE_ENV === "development") {
			debugInfo.hints.push(
				"Set MOCK_USER environment variable to enable mock authentication",
			);
			debugInfo.hints.push(
				"Check console logs for magic link URLs in development",
			);
			debugInfo.hints.push(
				"Use /api/debug/auth-test to test authentication flows",
			);
		}

		if (
			debugInfo.auth.features.socialLogin &&
			!debugInfo.checks.hasGoogleOAuth &&
			!debugInfo.checks.hasGitHubOAuth
		) {
			debugInfo.hints.push(
				"Configure GOOGLE_CLIENT_ID/SECRET or GITHUB_CLIENT_ID/SECRET for social login",
			);
		}
