...
import { CSVProcessor, ExcelProcessor, DataValidator, calculateInstanceNumbers, } from "@repo/api/src/lib/file-processing";
...
async function createComponentMilestones(tx, // Transaction context
components, templateMapper) {
    ...
    console.log(`Import-full: Found ${createdComponents.length} created components for milestone creation`);
    // Group components by their milestone template
    const componentsByTemplate = new Map();
    ...
    console.log(`Import-full: Components grouped into ${componentsByTemplate.size} different milestone templates`);
    ...
    componentsByTemplate.forEach((templateComponents, templateId) => {
        ...
        if (!template ||
            ...
            console.warn(`Import-full: No milestone definitions found for template ID: ${templateId}`);
            return;
        }
        console.log(`Import-full: Template ${templateCount}/${componentsByTemplate.size}: ${template.name} → ${templateComponents.length} components (${template.milestones.length} milestones each)`);
        ...
        templateComponents.forEach((component) => {
            template.milestones.forEach((milestone, index) => {
                milestonesToCreate.push({
                    ...
                    percentageValue: component.workflowType === "MILESTONE_PERCENTAGE"
                        ? 0
                        : null,
                    quantityValue: component.workflowType === "MILESTONE_QUANTITY"
                        ? 0
                        : null,
                });
            });
    ...
    if (milestonesToCreate.length > 0) {
        console.log(`Import-full: Creating ${milestonesToCreate.length} milestone records across ${componentsByTemplate.size} different templates...`);
...
export async function POST(request) {
    try {
        ...
        if (!project) {
            console.log("Import-full: Project not found or insufficient permissions");
            return NextResponse.json({
        ...
        console.log(`Import-full: Parsed file with ${parseResult.rows.length} rows for component import`);
        // Get existing drawings for validation - use drawing numbers, not IDs
        const existingDrawings = new Set((await prisma.drawing.findMany({
            where: { projectId },
            select: { number: true },
        })).map((d) => d.number));
        // Get existing milestone templates for validation
        const existingTemplates = new Set((await prisma.milestoneTemplate.findMany({
            where: { projectId },
            select: { id: true },
        })).map((t) => t.id));
        ...
        const validation = await validator.validateComponentData(parseResult.rows, mappings, {
            projectId,
        ...
        if (!validation.isValid && options.rollbackOnError) {
            console.log("Import-full: Early return due to validation failure with rollbackOnError");
            return NextResponse.json({
                success: false,
                summary: {
        ...
        console.log(`Import-full: Validated ${validComponents.length} components out of ${parseResult.rows.length}`);
        ...
        console.log(`Import-full: Template mapper loaded ${templateStats.totalTemplates} templates:`, templateStats.templateNames);
        // Ensure at least one template exists
        if (templateStats.totalTemplates === 0) {
            console.log("Import-full: No milestone templates found, creating default template");
            await templateMapper.ensureDefaultTemplate();
        }
        // Convert drawing numbers to drawing IDs for database operations
        const drawingNumberToIdMap = new Map();
        // Get existing drawings and create missing ones
        const uniqueDrawingNumbers = new Set();
        validComponents.forEach((comp) => {
            if (comp.drawingId) {
                uniqueDrawingNumbers.add(comp.drawingId);
            }
        });
        ...
        existingDrawingsData.forEach((d) => drawingNumberToIdMap.set(d.number, d.id));
        ...
        const componentsWithDrawingIds = validComponents.map((comp) => ({
            ...comp,
            drawingId: drawingNumberToIdMap.get(comp.drawingId) ||
                comp.drawingId,
        }));
        console.log("Import-full: Components with drawing IDs mapped:", componentsWithDrawingIds.length);
        // Optimized batch processing - fetch all existing components once
        console.log("Import-full: Pre-fetching all existing components for InstanceTracker...");
        ...
        console.log(`Import-full: Found ${existingComponents.length} existing components`);
        ...
        console.log(`Import-full: Built InstanceTracker map with ${existingComponentsMap.size} component groups`);
        // STEP 1: Consolidate duplicate entries before passing to InstanceTracker
        console.log("Import-full: Consolidating duplicate component entries...");
        ...
        componentGroups.forEach((components, key) => {
            if (components.length === 1) {
                ...
                console.log(`Import-full: Consolidated ${components.length} entries for ${componentId}${size ? ` (size ${size})` : ""} on drawing ${drawingId}: total quantity ${totalQuantity}`);
            }
        });
        const consolidationTime = Date.now() - consolidationStartTime;
        console.log(`Import-full: Consolidation completed in ${consolidationTime}ms - ${componentsWithDrawingIds.length} entries → ${consolidatedComponents.length} consolidated (${duplicateEntriesFound} duplicates merged)`);
        // Use InstanceTracker to handle quantity expansion and instance calculation
        console.log("Import-full: Using InstanceTracker to process consolidated components...");
        const startTime = Date.now();
        // Call InstanceTracker to expand components based on quantity and assign instance numbers
        const expandedComponents = await calculateInstanceNumbers(consolidatedComponents, existingComponentsMap);
        console.log(`Import-full: InstanceTracker expanded ${consolidatedComponents.length} consolidated components to ${expandedComponents.length} instances`);
        let createdCount = 0;
        let updatedCount = 0;
        let skipCount = 0;
        const finalErrors = [];
        ...
        const batchComponentKeys = new Set();
        for (const comp of expandedComponents) {
            try {
                ...
                const instanceNumber = comp.instanceNumber;
                const totalInstancesOnDrawing = comp
                    .totalInstancesOnDrawing;
                const displayId = comp.displayId;
                ...
                if (batchComponentKeys.has(batchKey)) {
                    console.warn(`Import-full: Skipping duplicate component in batch: ${batchKey}`);
                    skipCount++;
                    continue;
                }
                ...
                for (const field of allowedFields) {
                    if (comp[field] !== undefined) {
                        filteredCompData[field] = comp[field];
                    }
                }
                ...
                if (createdCount + updatedCount < 10) {
                    console.log(`Import-full: Component debug - componentId: ${comp.componentId}, type: '${comp.type}', typeOf: ${typeof comp.type}`);
                }
                // Determine appropriate milestone template based on component type
                const appropriateTemplate = templateMapper.getTemplateForComponentType(comp.type);
                if (!appropriateTemplate) {
                    ...
                    const defaultTemplate = await templateMapper.ensureDefaultTemplate();
                    if (defaultTemplate) {
                        console.warn(`Import-full: Using emergency default template for ${comp.componentId}`);
                ...
                const finalTemplate = appropriateTemplate ||
                    (await templateMapper.ensureDefaultTemplate());
                if (!finalTemplate ||
                ...
                if (createdCount + updatedCount < 20) {
                    console.log(`Import-full: ✅ ${comp.componentId} (type: '${comp.type}') → ${finalTemplate.name} (${finalTemplate.milestones.length} milestones)`);
                }
                const componentData = {
                    ...filteredCompData,
                    projectId,
                    ...
                    status: existingComponent?.status ||
                        ...
                        "NOT_STARTED",
                    completionPercent: existingComponent?.completionPercent ||
        ...
        await prisma.$transaction(async (tx) => {
            ...
            if (componentsToCreate.length > 0) {
                console.log(`Import-full: Creating ${componentsToCreate.length} new components...`);
                ...
                try {
                    ...
                    console.log(`Import-full: ✅ Created ${componentsToCreate.length} components in ${createTime}ms`);
                    // Create milestones for all newly created components
                    if (componentsToCreate.length > 0) {
                        const milestoneStartTime = Date.now();
                        console.log(`Import-full: Creating milestones for ${componentsToCreate.length} components...`);
                        await createComponentMilestones(tx, componentsToCreate, templateMapper);
                    ...
                    console.error("Import-full: First 5 components in batch:", componentsToCreate.slice(0, 5).map((comp) => ({
                        drawingId: comp.drawingId,
                        componentId: comp.componentId,
                        instanceNumber: comp.instanceNumber,
                    ...
                    if (duplicateComponents.length > 0) {
                        console.error(`Import-full: Found ${duplicateComponents.length} potential duplicates in batch:`, duplicateComponents.map((comp) => ({
                            index: comp.index,
                            drawingId: comp.drawingId,
                            componentId: comp.componentId,
                            instanceNumber: comp.instanceNumber,
            ...
            if (componentsToUpdate.length > 0) {
                console.log(`Import-full: Updating ${componentsToUpdate.length} existing components...`);
                const updateStartTime = Date.now();
                const updateChunkSize = 100;
                for (let i = 0; i < componentsToUpdate.length; i += updateChunkSize) {
                    const chunk = componentsToUpdate.slice(i, i + updateChunkSize);
                    await Promise.all(chunk.map((update) => tx.component.update(update)));
                    // Log progress for large updates
                    if (componentsToUpdate.length > updateChunkSize) {
                        const processed = Math.min(i + updateChunkSize, componentsToUpdate.length);
                        const chunkTime = Date.now() - updateStartTime;
                        console.log(`Import-full: Updated ${processed}/${componentsToUpdate.length} components (${chunkTime}ms)...`);
                    }
                }
                const updateTime = Date.now() - updateStartTime;
                console.log(`Import-full: ✅ Updated ${componentsToUpdate.length} components in ${updateTime}ms`);
            }
        });
        ...
        console.log("Import-full: Performance Summary:", {
            ...
            componentsPerSecond: Math.round((componentsToCreate.length + componentsToUpdate.length) /
                (totalTime / 1000)),
        });
