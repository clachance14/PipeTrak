...
async function createComponentMilestones(
  ...
  for (const [templateId, templateComponents] of componentsByTemplate) {
    ...
    if (!template || !template.milestones || template.milestones.length === 0) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const componentsWithDrawingIds = validComponents.map(comp => ({
      ...comp,
      drawingId: drawingNumberToIdMap.get(comp.drawingId as string) || comp.drawingId
    }));
    ...
    const existingComponentsMap = new Map<string, Array<{ componentId: string; instanceNumber: number; drawingId?: string; size?: string }>>();
    const existingComponentLookup = new Map<string, typeof existingComponents[0]>();
    ...
    for (const [key, components] of componentGroups) {
    ...
    const allowedFields = [
      'componentId', 'weldId', 'type', 'workflowType', 'drawingId', 'spec', 'size', 'material', 
      'pressureRating', 'description', 'area', 'system', 'testPackage', 'testPressure', 
      'testRequired', 'totalLength', 'lengthUnit', 'totalQuantity', 'quantityUnit', 
      'completionPercent', 'status', 'installationDate', 'installerUserId'
    ];
    ...
    for (const comp of expandedComponents) {
      try {
        ...
        const totalInstancesOnDrawing = (comp as any).totalInstancesOnDrawing;
        ...
        if (createdCount + updatedCount < 10) {
          console.log(`InstanceTracker result for ${comp.componentId}:`, {
            ...
            displayId,
            quantityExpanded: !!comp.totalQuantity && Number(comp.totalQuantity) > 1
          });
        }
        ...
        const appropriateTemplate = templateMapper.getTemplateForComponentType(comp.type as string);
        ...
        const finalTemplate = appropriateTemplate || await templateMapper.ensureDefaultTemplate();
        if (!finalTemplate || !finalTemplate.milestones || finalTemplate.milestones.length === 0) {
        ...
        const componentData = {
          ...
          totalInstancesOnDrawing,
          displayId,
          status: existingComponent?.status || filteredCompData.status || "NOT_STARTED",
          completionPercent: existingComponent?.completionPercent || filteredCompData.completionPercent || 0,
        };
    ...
    console.log("Import-full: Executing batch operations...", {
      ...
      toSkip: skipCount,
      totalProcessed: componentsToCreate.length + componentsToUpdate.length + skipCount
    });
      ...
      if (componentsToCreate.length > 0) {
        ...
        try {
          ...
          if (duplicateComponents.length > 0) {
            console.error(`Import-full: Found ${duplicateComponents.length} potential duplicates in batch:`, 
    ...
    console.log("Import-full: Performance Summary:", {
      ...
      componentsPerSecond: Math.round((componentsToCreate.length + componentsToUpdate.length) / (totalTime / 1000))
    });

    const success = finalErrors.length === 0 || (finalErrors.length > 0 && !options.rollbackOnError);
