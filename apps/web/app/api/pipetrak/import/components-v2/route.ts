...
import { parseExcel, calculateTotalInstances, detectColumnMapping } from "@repo/api/src/lib/import/excel-parser";
import { ComponentImportData, ComponentInstanceData, ImportPreviewResult, ImportResult } from "@repo/api/src/lib/import/types";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!project) {
      console.log("Import V2: Project not found or insufficient permissions");
      return NextResponse.json({ 
    ...
    console.log(`Import V2: Parsed Excel file with ${parseResult.rows.length} rows`);
    ...
    console.log(`Import V2: Found ${types.length} component types to analyze`);
    ...
    if (preview) {
      ...
      const estimatedInstances = calculateTotalInstances(parseResult.rows);
    ...
    for (let i = 0; i < parseResult.rows.length; i++) {
      ...
      try {
        ...
        const componentId = row[columnMapping.componentId || 'CMDTY CODE'];
        const type = row[columnMapping.type || 'TYPE'];
        
        // Validate required fields
        if (!drawingId || !componentId) {
          errors.push(`Row ${rowNumber}: Missing required fields (Drawing: ${drawingId}, Component ID: ${componentId})`);
          continue;
        }
        ...
        const componentData: ComponentImportData = {
          ...
          spec: row[columnMapping.spec || 'SPEC'] ? String(row[columnMapping.spec || 'SPEC']).trim() : undefined,
          size: row[columnMapping.size || 'SIZE'] ? String(row[columnMapping.size || 'SIZE']).trim() : undefined,
          description: row[columnMapping.description || 'DESCRIPTION'] ? String(row[columnMapping.description || 'DESCRIPTION']).trim() : undefined,
          material: row[columnMapping.material || 'MATERIAL'] ? String(row[columnMapping.material || 'MATERIAL']).trim() : undefined,
          notes: row[columnMapping.comments || 'Comments'] ? String(row[columnMapping.comments || 'Comments']).trim() : undefined,
          quantity: parseInt(String(row[columnMapping.quantity || 'QTY'])) || 1
        };
    ...
    console.log(`Import V2: Processed ${componentDataList.length} valid components, ${errors.length} errors`);
    ...
    componentDataList.forEach(comp => {
      ...
      excelDuplicateMap.set(key, (excelDuplicateMap.get(key) || 0) + comp.quantity);
    });
    
    const duplicateComponents = Array.from(excelDuplicateMap.entries()).filter(([key, totalQty]) => {
      return componentDataList.filter(comp => `${comp.drawingId}:${comp.componentId}` === key).length > 1;
    });
    
    if (duplicateComponents.length > 0) {
      console.log(`Import V2: Found ${duplicateComponents.length} component/drawing combinations with multiple Excel rows:`);
      duplicateComponents.slice(0, 5).forEach(([key, totalQty]) => {
        const [drawingId, componentId] = key.split(':');
        console.log(`  - ${componentId} on ${drawingId} (total qty: ${totalQty})`);
      });
    }

    // Group components by (drawing, componentId) and sum quantities
    console.log('Import V2: Grouping components by drawing+componentId to prevent duplicates...');
    ...
    for (const comp of componentDataList) {
      ...
      if (componentGroups.has(key)) {
        ...
        console.log(`Import V2: Merged duplicate ${comp.componentId} on ${comp.drawingId}, total qty now: ${existing.quantity}`);
      } else {
    ...
    console.log(`Import V2: Grouped ${componentDataList.length} rows into ${groupedComponentList.length} unique components`);
    ...
    const uniqueDrawings = [...new Set(groupedComponentList.map(c => c.drawingId))];
    ...
    const componentIds = [...new Set(groupedComponentList.map(c => c.componentId))];
    ...
    for (const comp of existingComponents) {
      ...
      existingInstanceMap.set(key, Math.max(currentMax, comp.instanceNumber));
    }

    console.log(`Import V2: Found ${existingComponents.length} existing component instances`);
    if (existingComponents.length > 0) {
      console.log('Import V2: Sample existing components:', existingComponents.slice(0, 3).map(c => 
        ({ drawingId: c.drawingId, componentId: c.componentId, instanceNumber: c.instanceNumber })));
    }
    ...
    for (const compData of groupedComponentList) {
      ...
      const existingMaxInstance = existingInstanceMap.get(existingKey) || 0;
      
      // Check if this exact component already exists with the same quantity
      const existingCount = existingComponents.filter(
        c => c.drawingId === drawingId && c.componentId === compData.componentId
      ).length;
      
      // Debug logging for first few components
      if (componentInstances.length < 3) {
        console.log(`Import V2: Processing ${compData.componentId} on drawing ${compData.drawingId} (UUID: ${drawingId})`);
        console.log(`Import V2: Existing count: ${existingCount}, quantity: ${quantity}, max instance: ${existingMaxInstance}`);
        console.log(`Import V2: Will create ${quantity - existingCount} instances starting from ${existingMaxInstance + 1}`);
      }
      
      // If we already have this exact quantity, skip it
      if (existingCount >= quantity) {
        console.log(`Import V2: Skipping ${compData.componentId} on ${compData.drawingId} - already exists`);
      ...
      for (let i = 0; i < instancesToCreate; i++) {
        ...
        const instance: ComponentInstanceData = {
          ...
          displayId: totalOnDrawing > 1 
            ? `${compData.componentId} (${instanceNumber} of ${totalOnDrawing})`
            : compData.componentId,
          milestoneTemplateId: templateAssigner.getTemplateForType(compData.type as ComponentType),
    ...
    console.log(`Import V2: Expanded to ${componentInstances.length} new component instances`);
    console.log(`Import V2: Skipped ${skippedDuplicates} duplicate instances`);
    ...
    for (const comp of componentsToCreate) {
      ...
      if (duplicateChecker.has(key)) {
        ...
        actualDuplicates.push({
          key,
          existing: { drawingId: existing.drawingId, componentId: existing.componentId, instanceNumber: existing.instanceNumber },
          duplicate: { drawingId: comp.drawingId, componentId: comp.componentId, instanceNumber: comp.instanceNumber }
        });
      }
    ...
    if (actualDuplicates.length > 0) {
      console.error(`Import V2: FOUND ${actualDuplicates.length} DUPLICATE ENTRIES IN BATCH:`);
    ...
    componentsToCreate.slice(0, 5).forEach((comp, idx) => {
      console.log(`  ${idx + 1}. Drawing: ${comp.drawingId}, Component: ${comp.componentId}, Instance: ${comp.instanceNumber}`);
    });

    // Execute in transaction only if we have components to create
    let result = { count: 0, skipped: skippedDuplicates };
    
    if (componentsToCreate.length > 0) {
      const transactionResult = await prisma.$transaction(async (tx) => {
        // Create components
        console.log(`Import V2: Creating ${componentsToCreate.length} components...`);
        const createdComponents = await tx.component.createManyAndReturn({
          data: componentsToCreate
        });

        console.log(`Import V2: Created ${createdComponents.length} components`);

        // Create milestones for new components
      ...
      result.count = transactionResult.count;
    } else {
      console.log("Import V2: No new components to create - all were duplicates");
    }
    ...
    if (error.code === 'P2002') {
      errorMessage = "Duplicate components detected. This usually means the same components are already imported. Please check your data or contact support.";
      status = 409; // Conflict
    } else if (error.code === 'P2003') {
      errorMessage = "Foreign key constraint violation. Please ensure all referenced data exists.";
      status = 400; // Bad Request
    } else if (error.code === 'P2025') {
      errorMessage = "Required record not found. Please check your project and drawing data.";
...
async function ensureDrawingsExist(projectId: string, drawingNumbers: string[]) {
  console.log(`Import V2: Ensuring ${drawingNumbers.length} drawings exist...`);
  ...
  if (missing.length > 0) {
    console.log(`Import V2: Creating ${missing.length} missing drawings...`);
