...
export async function POST(request) {
    try {
            ...
            try {
                ...
                const componentData = {
                    ...
                    description: row[columnMapping.description || "DESCRIPTION"]
                        ? String(row[columnMapping.description || "DESCRIPTION"]).trim()
                        : undefined,
                    material: row[columnMapping.material || "MATERIAL"]
                        ? String(row[columnMapping.material || "MATERIAL"]).trim()
                        : undefined,
                    ...
                    testPackage: row[columnMapping.testPackage || "Test Package"]
                        ? String(row[columnMapping.testPackage || "Test Package"]).trim()
                        : undefined,
                    notes: row[columnMapping.comments || "Comments"]
                        ? String(row[columnMapping.comments || "Comments"]).trim()
                        : undefined,
        ...
        const duplicateComponents = Array.from(excelDuplicateMap.entries()).filter(([key, _]) => {
        ...
        const uniqueDrawings = Array.from(new Set(groupedComponentList.map((c) => c.drawingId)));
        ...
        const componentIds = Array.from(new Set(groupedComponentList.map((c) => c.componentId)));
        ...
        const existingInstanceMap = new Map();
        ...
        if (existingComponents.length > 0) {
            console.log("Import V2: Sample existing components:", existingComponents
                .slice(0, 3)
                .map((c) => ({
        ...
        for (const compData of groupedComponentList) {
            ...
            const existingCount = existingComponents.filter((c) => c.drawingId === drawingId &&
                c.componentId === compData.componentId).length;
            ...
            for (let i = 0; i < instancesToCreate; i++) {
                ...
                const instance = {
                    ...
                    milestoneTemplateId: templateAssigner.getTemplateForType(compData.type),
        ...
        if (componentsToCreate.length > 0) {
            console.log(`Import V2: Processing ${componentsToCreate.length} components in batches of ${BATCH_SIZE}...`);
            ...
            for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
                ...
                console.log(`Import V2: Processing batch ${batchNumber}/${batches.length} (${batch.length} components)...`);
                const batchResult = await prisma.$transaction(async (tx) => {
                    ...
                    if (createdComponents.length > 0) {
                        await templateAssigner.createMilestonesForComponents(createdComponents.map((c) => ({
                            id: c.id,
                ...
                console.log(`Import V2: Batch ${batchNumber} completed - created ${batchResult.count} components (${totalCreated}/${componentsToCreate.length} total)`);
            }
            result.count = totalCreated;
            console.log(`Import V2: All batches completed - total created: ${totalCreated} components`);
        }
        else {
...
async function ensureDrawingsExist(projectId, drawingNumbers) {
