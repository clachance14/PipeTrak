...
export default async function DocumentationPage(props) {
    ...
    return (<DocsPage toc={page.data.toc} full={page.data.full} breadcrumb={{
            enabled: true,
            includePage: true,
			...
			<DocsBody>
				...
				{page.data.description && (<p className="-mt-6 text-foreground/50 text-lg lg:text-xl">
    ...
    components={{
            ...
            Folder,
            Files,
            img: (props) => (<ImageZoom {...props} className="rounded-lg border-4 border-secondary/10"/>),
        }}/>
				</div>
			</DocsBody>
		</DocsPage>);
