...
async function examineExcelTypes() {
	try {
		...
		const filePath = path.join(
			process.cwd(),
			"../../project-documentation/TAKEOFF - 5932.xlsx",
		);
		...
		for (const sheetName of sheetNames) {
			...
			const typeColumnIndex = headerRow.findIndex(
				(header: any) =>
			...
			for (let i = 1; i < data.length && i < 1000; i++) {
				// Limit to first 1000 rows for analysis
				const row = data[i];
				...
				if (
					...
					const componentIdIndex = headerRow.findIndex(
						(header: any) =>
							header &&
					...
					if (
						componentIdIndex !== -1 &&
						typeExamples.get(typeStr)!.length < 3
					) {
						typeExamples
							.get(typeStr)!
							.push(String(row[componentIdIndex] || "N/A"));
					}
				}
			}
			...
			if (emptyTypeCount > 0) {
				console.log(
					`  - ${emptyTypeCount} rows with empty/null type values`,
				);
				issuesFound = true;
			}
			...
			for (const [type, count] of sortedTypes) {
				...
				if (
					!expectedTypes.includes(upperType) &&
					!expectedTypes.some((expected) =>
						upperType.includes(expected),
					)
				) {
					console.log(
						`  - "${type}" (${count} rows) doesn't match expected component types`,
					);
					issuesFound = true;
				}
			}
			...
			for (const [type, count] of sortedTypes.slice(0, 10)) {
				...
				else if (upperType.includes("INSTRUMENT"))
					suggestion = "INSTRUMENT";
				else if (upperType.includes("SPOOL")) suggestion = "SPOOL";
				else if (
					upperType.includes("PIPING") ||
					upperType.includes("PIPE")
				)
					suggestion = "SPOOL";
				else if (upperType.includes("WELD") || upperType.includes("FW"))
					suggestion = "FIELD_WELD";
			...
			console.log(
				`  - Most common type: "${sortedTypes[0][0]}" (${sortedTypes[0][1]} occurrences)`,
			);
		}
	} catch (error: any) {
