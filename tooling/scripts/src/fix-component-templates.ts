...
import { resolveTemplateForComponent, analyzeComponentTypes, validateTemplateAssignment } from "./lib/template-resolver";
...
async function loadTemplatesForProject(projectId: string): Promise<Map<string, any>> {
...
async function fixComponentTemplate(
  ...
  const correctTemplate = Array.from(templates.values()).find(t => t.id === correctTemplateId);
  if (!correctTemplate) {
    stats.warnings.push(`No correct template found for component ${component.componentId}`);
    return;
  }
  
  // Check if component is already using the correct template
  if (component.milestoneTemplateId === correctTemplateId) {
    console.log(`✅ Component ${component.componentId} already uses correct template: ${correctTemplate.name}`);
    return;
  }
  
  console.log(`🔄 Fixing component ${component.componentId}: ${component.milestoneTemplate.name} → ${correctTemplate.name}`);
  
  // Validate the assignment
  const validation = validateTemplateAssignment(component.type, component.componentId, correctTemplate.name);
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      const newMilestones = JSON.parse(correctTemplate.milestones as string);
      ...
      stats.templateAssignments[correctTemplate.name] = (stats.templateAssignments[correctTemplate.name] || 0) + 1;
      
      console.log(`✅ Fixed ${component.componentId}: ${milestonesWithCompletion.length} milestones, ${milestonesWithCompletion.filter(m => m.isCompleted).length} completed`);
    });
  } catch (error) {
    console.error(`❌ Failed to fix component ${component.componentId}:`, error);
    stats.warnings.push(`Failed to fix component ${component.componentId}: ${error}`);
  }
}
...
async function analyzeProject(projectId: string): Promise<void> {
  ...
  for (const [type, stats] of Object.entries(analysis)) {
    console.log(`${type}: ${stats.count} components → ${stats.template} template`);
...
async function fixProjectComponents(projectId: string, dryRun: boolean = false): Promise<MigrationStats> {
  ...
  for (let i = 0; i < components.length; i++) {
    ...
    console.log(`Processing ${i + 1}/${components.length}: ${component.componentId}`);
    ...
    if (!dryRun) {
      ...
      if (component.milestoneTemplateId !== correctTemplateId) {
        console.log(`WOULD FIX: ${component.componentId}: ${component.milestoneTemplate.name} → ${correctTemplate?.name}`);
        stats.componentsFixed++;
        stats.templateAssignments[correctTemplate?.name || 'Unknown'] = (stats.templateAssignments[correctTemplate?.name || 'Unknown'] || 0) + 1;
      }
    }
  }
...
async function fixAllProjects(dryRun: boolean = false): Promise<void> {
  ...
  for (const project of projects) {
    ...
    overallStats.completionDataPreserved += projectStats.completionDataPreserved;
    overallStats.warnings.push(...projectStats.warnings);
    
    for (const [template, count] of Object.entries(projectStats.templateAssignments)) {
      overallStats.templateAssignments[template] = (overallStats.templateAssignments[template] || 0) + count;
    }
  }
  ...
  console.log(`Components ${dryRun ? 'that would be' : ''} fixed: ${overallStats.componentsFixed}`);
  
  if (!dryRun) {
    console.log(`Milestones recreated: ${overallStats.milestonesRecreated}`);
    console.log(`Completion data preserved: ${overallStats.completionDataPreserved}`);
  }
  
  console.log(`\nTemplate assignments ${dryRun ? '(projected)' : ''}:`);
  for (const [template, count] of Object.entries(overallStats.templateAssignments)) {
    console.log(`  ${template}: ${count} components`);
  }
...
Examples:
  ...
  try {
    if (projectIdIndex !== -1 && args[projectIdIndex + 1]) {
      ...
      if (analyzeOnly) {
        const projects = await prisma.project.findMany({ select: { id: true, jobName: true } });
        for (const project of projects) {
          console.log(`\n=== ${project.jobName} ===`);
          await analyzeProject(project.id);
        }
