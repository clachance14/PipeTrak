...
export class PipeTrakImporter {
    ...
    try {
      ...
      if (dryRun) {
        return {
          ...
          successfulRows: validationResult.isValid ? data.components.length : 0,
  ...
  private async processBatchWithRetry(
    ...
    try {
      ...
      if (attempt < maxRetries) {
        logger.warn(`Batch ${batchNumber} failed, retrying (attempt ${attempt}/${maxRetries})`);
      ...
      for (const component of batch) {
        try {
          ...
          const componentData = {
            ...
            workflowType: component.workflowType || 'MILESTONE_DISCRETE',
            status: this.calculateStatus(component.milestones || []),
            completionPercent: this.calculateCompletionPercent(component.milestones || []),
            createdAt: new Date(),
            updatedAt: new Date(),
          };
          ...
          if (component.milestones && component.milestones.length > 0) {
            ...
            for (let i = 0; i < component.milestones.length; i++) {
              ...
              const isCompleted = milestone.completed ?? milestone.isCompleted ?? false;
              ...
              await tx.componentMilestone.create({
                data: {
                  ...
                  completedAt: isCompleted && milestone.completedDate
  ...
  private calculateStatus(
    ...
    if (completedCount === 0) {
      return 'NOT_STARTED';
    } else if (completedCount === milestones.length) {
      return 'COMPLETED';
    } else {
      return 'IN_PROGRESS';
    }
   ...
   */
  private calculateCompletionPercent(milestones: MilestoneImportData[]): number {
    if (!milestones || milestones.length === 0) {
      return 0;
    }
  ...
  private consolidateResults(results: ImportResult[]): ImportResult {
    const consolidated: ImportResult = {
      ...
      successfulRows: results.reduce((sum, r) => sum + r.successfulRows, 0),
...
export class ImportValidator {
  async validateImportData(
    ...
    for (let i = 0; i < data.components.length; i++) {
      ...
      if (component.workflowType && 
          !['MILESTONE_DISCRETE', 'MILESTONE_PERCENTAGE', 'MILESTONE_QUANTITY'].includes(component.workflowType)) {
        errors.push({
          row: rowNumber,
          field: 'workflowType',
      ...
      if (component.drawingNumber && data.drawings) {
        const drawingExists = data.drawings.some(d => d.number === component.drawingNumber);
        if (!drawingExists) {
          warnings.push({
            row: rowNumber,
            field: 'drawingNumber',
      ...
      const duplicates = data.components.filter(
        (c, idx) => idx !== i && c.componentId === component.componentId
      );
        ...
        for (let j = 0; j < component.milestones.length; j++) {
          ...
          if (component.workflowType === 'MILESTONE_PERCENTAGE' && 
            ...
            warnings.push({
              ...
              message: 'Percentage value expected for percentage workflow',
              severity: 'warning',
            });
          }
          ...
          if (component.workflowType === 'MILESTONE_QUANTITY' && 
            ...
            warnings.push({
              ...
              message: 'Quantity value expected for quantity workflow',
              severity: 'warning',
            });
          }
        }
  ...
  generateRemediationReport(errors: ValidationError[]): string {
    ...
    for (const [code, codeErrors] of Object.entries(errorsByType)) {
      ...
      for (const error of codeErrors.slice(0, 5)) { // Show first 5 of each type
    ...
    report.push('2. Correct invalid workflow types to one of: MILESTONE_DISCRETE, MILESTONE_PERCENTAGE, MILESTONE_QUANTITY');
...
export class ImportTransformer {
  ...
  async transformImportData(
    ...
    const transformed: ImportData = {
      ...data,
      components: data.components.map(component => this.transformComponent(component)),
    };
   ...
   */
  private transformComponent(component: ComponentImportData): ComponentImportData {
    const transformed: ComponentImportData = {
      ...component,
      // Normalize field names
      componentId: component.componentId || component['Component ID'] || component['component_id'],
      type: component.type || component['Type'] || component['type'] || 'PIPE',
      spec: component.spec || component['Spec'] || component['specification'] || '',
      ...
      testPackage: component.testPackage || component['Test Package'] || component['test_package'] || '',
      drawingNumber: component.drawingNumber || component['Drawing'] || component['drawing_number'],
      workflowType: this.normalizeWorkflowType(
        component.workflowType || component['Workflow Type'] || 'MILESTONE_DISCRETE'
      ),
    };
   ...
   */
  private transformMilestone(milestone: MilestoneImportData): MilestoneImportData {
    return {
      ...
      completedDate: milestone.completedDate || milestone['Completed Date'],
      weight: milestone.weight || milestone['Weight'],
      percentageValue: milestone.percentageValue || milestone['Percentage'],
  ...
  private transformDrawing(drawing: DrawingImportData): DrawingImportData {
    return {
      ...
      title: drawing.title || drawing.description || drawing['Title'] || '',
  ...
  private normalizeWorkflowType(
    ...
    if (normalized.includes('DISCRETE') || normalized.includes('CHECKBOX')) {
