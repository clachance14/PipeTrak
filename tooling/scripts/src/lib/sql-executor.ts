...
export class SQLExecutor {
  ...
  constructor(options: SQLExecutorOptions = {}) {
    ...
    const connectionString = options.connectionString || process.env.DIRECT_URL;
      ...
      try {
        ...
        if (error.message.includes('already exists')) {
          ...
          if (!this.options.continueOnError) {
            ...
            throw new Error(`SQL execution failed at statement ${i + 1}/${statements.length}: ${error.message}`);
          }
  ...
  private parseSQLStatements(sqlContent: string): string[] {
    ...
    for (const line of lines) {
      ...
      if ((line.trim().startsWith('--') && currentStatement.trim() === '') || line.trim() === '') {
        continue;
      }
...
export async function executeSQLFile(
  ...
  try {
    ...
    if (showProgress) {
      ...
      console.log(`   ⚠️  Statements skipped: ${result.statementsSkipped}`);
 ...
 */
export async function testDatabaseConnection(showProgress: boolean = false): Promise<boolean> {
