...
export function resolveTemplateForComponent(
  ...
  if (!templateName) {
    ...
    console.log(`⚠️  No template mapping found for type "${componentType}" and ID "${componentId}", using default: ${templateName}`);
  }
  ...
  if (!template) {
    ...
    console.error(`❌ Template "${templateName}" not found, falling back to: ${fallbackTemplate.name}`);
 ...
 */
export function analyzeComponentTypes(components: Array<{type: string, componentId: string}>): Record<string, {count: number, template: string, examples: string[]}> {
  const stats: Record<string, {count: number, template: string, examples: string[]}> = {};
  
  // Create a dummy template map for analysis
  const dummyTemplates = new Map([
    ['Full Milestone Set', { id: 'full', name: 'Full Milestone Set', description: '', milestones: '' }],
    ['Reduced Milestone Set', { id: 'reduced', name: 'Reduced Milestone Set', description: '', milestones: '' }],
    ['Field Weld', { id: 'fieldweld', name: 'Field Weld', description: '', milestones: '' }],
    ['Insulation', { id: 'insulation', name: 'Insulation', description: '', milestones: '' }],
    ['Paint', { id: 'paint', name: 'Paint', description: '', milestones: '' }],
  ]);
  
  for (const component of components) {
    const type = component.type || 'UNKNOWN';
    const templateName = resolveTemplateForComponent(type, component.componentId, undefined, dummyTemplates);
    const template = Array.from(dummyTemplates.values()).find(t => t.id === templateName)?.name || 'Unknown';
