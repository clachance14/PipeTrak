...
async function seedSDOTankJob() {
  ...
  try {
    ...
    if (existingProject) {
      console.log(`Project already exists: ${existingProject.jobName} (${existingProject.id})`);
    ...
    const project = await prisma.project.create({
      data: {
        ...
        description: sdoData.project.description || "Tank Job at Dow Seadrift",
        status: "ACTIVE",
        location: sdoData.project.location,
        startDate: sdoData.project.startDate ? new Date(sdoData.project.startDate) : new Date("2024-11-01"),
        targetDate: sdoData.project.targetDate ? new Date(sdoData.project.targetDate) : new Date("2025-03-31"),
        createdBy: user.id,
        createdAt: new Date(),
      }
    });
    ...
    for (const drawingData of sdoData.drawings) {
      const drawing = await prisma.drawing.create({
        data: {
          ...
          title: drawingData.description || `Drawing ${drawingData.number}`,
    ...
    for (const componentData of sdoData.components) {
      ...
      const completedMilestones = componentData.milestones.filter(m => 
        m.completed || m.isCompleted
      ).length;
      ...
      if (!drawingId) {
        console.warn(`Warning: Drawing ${componentData.drawingNumber} not found for component ${componentData.componentId}`);
        continue;
      }
      ...
      const component = await prisma.component.create({
        data: {
          ...
          workflowType: (componentData.workflowType || "MILESTONE_DISCRETE") as "MILESTONE_DISCRETE" | "MILESTONE_PERCENTAGE" | "MILESTONE_QUANTITY",
          status,
          completionPercent,
          drawingId,
      ...
      const defaultWeight = milestoneCount > 0 ? 100 / milestoneCount : 20;
        ...
        await prisma.componentMilestone.create({
          data: {
            ...
            completedAt: isCompleted ? 
              (milestone.completedDate ? new Date(milestone.completedDate) : new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000)) : 
              null,
    ...
    console.log("\n📊 Project Status:");
    console.log(`- Overall Progress: ${Math.round(projectStats._avg.completionPercent || 0)}%`);
    
    for (const statusCount of statusCounts) {
      console.log(`- ${statusCount.status}: ${statusCount._count.id} components`);
    }
