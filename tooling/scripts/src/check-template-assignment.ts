...
async function checkTemplateAssignment() {
  try {
    ...
    templates.forEach(t => {
      const milestones = Array.isArray(t.milestones) ? t.milestones as any[] : [];
      console.log(`• ${t.name} (ID: ${t.id.slice(-8)})`);
      console.log(`  Milestones: ${milestones.map((m: any) => m.name).join(', ')}`);
    });
    ...
    for (const group of componentsByTemplate) {
      const template = templates.find(t => t.id === group.milestoneTemplateId);
      console.log(`• ${template?.name || 'Unknown'}: ${group._count.id} components`);
    }
    ...
    sampleComponents.forEach(c => {
      const template = templates.find(t => t.id === c.milestoneTemplateId);
      console.log(`${c.componentId} (Type: ${c.type || 'N/A'})`);
      console.log(`  Template: ${template?.name}`);
      console.log(`  Milestones: ${c.milestones.map(m => m.milestoneName).join(', ')}`);
    });
    ...
    if (fieldWelds.length > 0) {
      ...
      fieldWelds.forEach(fw => {
        const template = templates.find(t => t.id === fw.milestoneTemplateId);
    ...
    console.log('The import system assigns one template to ALL components.');
    console.log('Components should get different templates based on their type:');
