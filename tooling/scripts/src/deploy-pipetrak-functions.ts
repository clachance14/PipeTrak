  ...
  try {
    ...
    try {
      ...
      if (projectResult.rows[0]) {
        ...
        if (result.rows[0]?.progress) {
          ...
          console.log(`   - Total components: ${progress.overall?.total_components || 0}`);
          console.log(`   - Not started: ${progress.overall?.not_started || 0}`);
          console.log(`   - In progress: ${progress.overall?.in_progress || 0}`);
          console.log(`   - Completed: ${progress.overall?.completed || 0}`);
          console.log(`   - Average completion: ${Math.round(progress.overall?.avg_completion || 0)}%\n`);
        }
      }
    } catch (error: any) {
    ...
    try {
      ...
      if (componentResult.rows[0]) {
        ...
        console.log(`   - Component: ${component.displayId || component.componentId}`);
        console.log(`   - Completion: ${Math.round(result.rows[0].completion || 0)}%\n`);
        ...
        if (statusResult.rows[0]) {
          ...
          console.log(`   - Completion: ${statusResult.rows[0].completionPercent}%\n`);
        }
      }
    } catch (error: any) {
      console.error('❌ calculate_component_completion failed:', error.message);
    }
