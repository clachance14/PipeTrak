...
async function addUserToOrganization() {
  try {
    ...
    memberships.forEach(member => {
      console.log(`- ${member.user.email} is ${member.role} in ${member.organization.name}`);
    });
    ...
    if (users.length > 0 && organizations.length > 0) {
      ...
      if (usersWithoutOrg.length > 0) {
        ...
        console.log(`✅ Successfully added ${newMembership.user.email} as ${newMembership.role} to ${newMembership.organization.name}`);
      } else {
      ...
      if (organizations.length === 0) {
        ...
        if (users.length > 0) {
          ...
          console.log(`✅ Added ${firstUser.email} as owner of ${newOrg.name}`);
        }
      }
    }
    ...
    projects.forEach(project => {
      console.log(`- ${project.jobName} (${project.jobNumber}) - Org: ${project.organization.name}`);
    });
