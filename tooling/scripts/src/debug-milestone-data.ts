...
async function debugMilestoneData() {
    ...
    `,
		...
		console.log(
			...
			statsResult.rows[0].completed_milestones,
		);
		console.log(
			"Has percentage value:",
			statsResult.rows[0].has_percentage,
		);
		console.log(
			"Has effective date:",
			statsResult.rows[0].has_effective_date,
		);
		console.log(
			"Unique components:",
			statsResult.rows[0].unique_components,
		);
		console.log(
			"Unique milestone types:",
			statsResult.rows[0].unique_milestone_types,
		);
		console.log();
    ...
    `,
		...
		typesResult.rows.forEach((row) => {
			console.log(
				`  ${row.milestoneName}: ${row.completed}/${row.count} completed (${Math.round(row.avg_percentage)}% avg)`,
			);
		});
		console.log();
    ...
    `,
		...
		areaResult.rows.forEach((row) => {
			console.log(
				`  ${row.area}: ${row.completed_milestones}/${row.milestone_count} milestones (${row.completion_percent}%)`,
			);
		});
		console.log();
    ...
    `,
		...
		sampleResult.rows.forEach((row) => {
			...
			console.log(
				`    Completed: ${row.isCompleted}, Percentage: ${row.percentageValue}%`,
			);
    ...
    `,
		...
		console.log(
			"Components without milestones:",
			missingResult.rows[0].components_without_milestones,
		);
	} catch (error) {
