...
async function verifyTables() {
    ...
    `;
		...
		if (tableResult.rows.length === 0) {
			...
			console.log(
				"   Run the migration script first: pnpm tsx src/migrate-pipetrak-tables.ts",
			);
			return;
		}
        ...
        `;
				...
				const indexCount = Number.parseInt(
					indexResult.rows[0].index_count,
				);

				console.log(
					`✅ ${tableName.padEnd(20)} | ${table.column_count} columns | ${indexCount} indexes`,
				);
			} else {
    ...
    `;
		...
		if (fkResult.rows.length > 0) {
			...
			fkResult.rows.forEach((fk) => {
				...
				console.log(
					`  → ${fk.column_name} references ${fk.foreign_table_name}(${fk.foreign_column_name})`,
				);
			});
		}
		...
		for (const table of tableResult.rows) {
			...
			const rowCount = Number.parseInt(countResult.rows[0].row_count);
