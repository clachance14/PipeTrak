	...
	try {
		...
		if (organizations.length === 0) {
			...
			if (values.fix) {
				...
				console.log(
					"💡 Run with --fix to create a default organization",
				);
				process.exit(1);
			}
		}
		...
		if (memberships.length === 0) {
			...
			memberships.forEach((m) => {
				console.log(
					`  • ${m.user.email} → ${m.role} in ${m.organization.name}`,
				);
			});
		}
		console.log();
		...
		if (usersWithoutOrg.length === 0) {
			console.log(
				"✅ All users are members of at least one organization!",
			);
			return;
		}

		console.log(
			`⚠️  Found ${usersWithoutOrg.length} users without organization membership:`,
		);
		usersWithoutOrg.forEach((user) => {
			console.log(
				`  • ${user.email} (${user.name || "No name"}) - Role: ${user.role || "user"}`,
			);
		});
