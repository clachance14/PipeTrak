...
async function verifyFunction() {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      console.log('✅ Function exists:', result.rows.length, 'version(s)');
      result.rows.forEach((row, i) => {
        console.log(`  Version ${i + 1}: arg types = ${row.proargtypes}`);
      });
    } else {
    ...
    try {
      await client.query(`SELECT initialize_component_milestones('test-id')`);
