...
function validateTemplate(template: TemplateDefinition): void {
	...
	if (new Set(orders).size !== orders.length) {
		throw new Error(
			`Template "${template.name}" has duplicate milestone orders`,
		);
	}
}

async function createMilestoneTemplatesForProject(
	projectId: string,
	...
	for (const templateDef of MILESTONE_TEMPLATES) {
		...
		if (existing) {
			console.log(
				`Template "${templateDef.name}" already exists, skipping...`,
			);
		...
		console.log(
			`✅ Created template: ${templateDef.name} (${templateDef.milestones.length} milestones, ${templateDef.milestones.reduce((s, m) => s + m.weight, 0)}% total weight)`,
		);
	}
