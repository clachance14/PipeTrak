...
async function applyMigrations() {
  ...
  try {
    ...
    // Migration files
    const migrationDir = path.join(__dirname, '../../../packages/database/prisma/migrations');
    const migrations = [
      ...
      for (const migration of migrations) {
        ...
        try {
          ...
          if (error.message.includes('already exists') || 
            ...
            console.log(`⚠ Skipped ${migration}: ${error.message.split('\n')[0]}`);
          } else {
    ...
    } else if (columns.includes('jobName') && columns.includes('jobNumber') && columns.includes('name')) {
    ...
    if (finalColumns.includes('jobName') && finalColumns.includes('jobNumber') && !finalColumns.includes('name')) {
      console.log('✅ Migration completed successfully!');
    } else {
      console.log('⚠ Migration may be incomplete. Please verify manually.');
    }
