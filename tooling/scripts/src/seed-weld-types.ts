...
async function seedWeldTypes() {
  ...
  try {
    ...
    for (const weldType of weldTypes) {
      ...
      if (isNew) {
        created++;
        console.log(`   ✅ Created weld type: ${weldType.code} - ${weldType.description}`);
      } else {
        updated++;
        console.log(`   🔄 Updated weld type: ${weldType.code} - ${weldType.description}`);
      }
    }
    ...
    console.log(`   Database now contains ${totalWeldTypes} total weld types`);
