...
function detectFileFormat(filePath: string): 'excel' | 'csv' | 'json' {
  ...
  throw new Error(`Unsupported file format: ${ext}. Supported formats: .xlsx, .xls, .csv, .json`);
}
...
async function parseExcelFile(filePath: string): Promise<ImportData> {
  ...
  const componentSheet = workbook.Sheets['Components'] || 
  ...
  const drawingSheet = workbook.Sheets['Drawings'] || workbook.Sheets['Drawing'];
  if (drawingSheet) {
  ...
  const milestoneSheet = workbook.Sheets['Milestones'] || workbook.Sheets['Milestone'];
  if (milestoneSheet) {
...
function parseExcelComponentRow(row: any): ComponentImportData {
  ...
  const component: ComponentImportData = {
    componentId: row['Component ID'] || row['ComponentID'] || row['ID'] || row['Tag'],
    ...
    workflowType: normalizeWorkflowType(row['Workflow Type'] || row['Workflow']),
    milestones: [],
  };

  // Check for inline milestone columns (e.g., "Received", "Fit-up", etc.)
  const milestoneColumns = [
    'Received', 'Fit-up', 'Fitted', 'Welded', 'Tested', 'Insulated',
    'Delivered', 'Installed', 'Connected', 'Commissioned'
  ];
...
async function parseJSONFile(filePath: string): Promise<ImportData> {
  ...
  throw new Error('Invalid JSON format. Expected either {components: [...]} or an array of components');
}

// ========== Helper Functions ==========

function normalizeWorkflowType(value: any): 'MILESTONE_DISCRETE' | 'MILESTONE_PERCENTAGE' | 'MILESTONE_QUANTITY' | undefined {
  if (!value) return undefined;
  
  const str = String(value).toUpperCase();
  if (str.includes('DISCRETE') || str.includes('CHECKBOX')) return 'MILESTONE_DISCRETE';
  ...
  try {
    ...
    const progressCallback = (progress: any) => {
      if (progress.phase === 'importing' && progress.batch) {
        logger.info(`Processing batch ${progress.batch}/${progress.totalBatches}`);
      } else {
        logger.info(`${progress.phase}: ${progress.processed}/${progress.total}`);
      }
    };
...
async function main() {
  ...
  if (args.length === 0) {
    console.error('Usage: pnpm --filter scripts import <file> --project-id <id> [options]');
    console.error('\nOptions:');
    console.error('  --project-id <id>      Project ID to import into (required)');
    console.error('  --user-id <id>         User ID for audit (optional, uses default)');
    console.error('  --dry-run              Validate without saving to database');
    console.error('  --skip-duplicates      Skip components that already exist');
    console.error('  --update-existing      Update existing components');
    console.error('  --allow-partial        Continue on errors');
    console.error('\nExamples:');
    console.error('  pnpm --filter scripts import data.xlsx --project-id abc123');
    console.error('  pnpm --filter scripts import data.csv --project-id abc123 --dry-run');
    console.error('  pnpm --filter scripts import data.json --project-id abc123 --skip-duplicates');
    process.exit(1);
  }
