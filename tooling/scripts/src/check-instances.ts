...
async function checkInstances() {
  ...
  try {
    ...
    console.log(`Found ${multipleInstances.length} components with multiple instances on the same drawing\n`);
    
    // Get examples of multi-instance components
    const examples = await prisma.component.findMany({
      where: {
        componentId: {
          in: ["GSWAZ1DZZASG5331", "MCPLALTLEAA1H1795", "VGATU-SECBFLR02F-017"]
        }
      },
      include: {
        drawing: true
    ...
    for (const comp of examples) {
      ...
      console.log(`    ✓ Instance ${comp.instanceNumber}/${comp.totalInstancesOnDrawing} - ${comp.displayId}`);
      console.log(`      Status: ${comp.status}, Progress: ${comp.completionPercent}%`);
    }
    ...
    console.log(`Average instances per component: ${stats._avg.totalInstancesOnDrawing?.toFixed(1)}`);
    console.log(`Maximum instances of one component: ${stats._max.totalInstancesOnDrawing}`);
