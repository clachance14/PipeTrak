...
async function checkTemplateRawData() {
  try {
    ...
    for (const template of templates) {
      ...
      console.log(`  Type of milestones field: ${typeof template.milestones}`);
      console.log(`  Milestones value: ${JSON.stringify(template.milestones)}`);
      
      // Try to parse if it's a string
      if (typeof template.milestones === 'string') {
        try {
          const parsed = JSON.parse(template.milestones);
          console.log(`  Parsed milestones: ${JSON.stringify(parsed)}`);
        } catch (e) {
