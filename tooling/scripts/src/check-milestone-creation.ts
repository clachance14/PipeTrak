...
async function checkMilestoneCreation() {
  try {
    ...
    templates.forEach(t => {
      console.log(`\nTemplate: ${t.name}`);
      console.log(`  ID: ${t.id}`);
      console.log(`  Components using this template: ${t._count.components}`);
      
      const milestones = Array.isArray(t.milestones) ? t.milestones : 
                        typeof t.milestones === 'object' ? [t.milestones] : [];
      
      console.log(`  Milestone definitions: ${milestones.length}`);
      if (milestones.length > 0) {
        console.log('  Milestones:');
        milestones.forEach((m: any, i: number) => {
          console.log(`    ${i + 1}. ${m.name || 'UNNAMED'} (order: ${m.order}, weight: ${m.weight})`);
        });
      }
    });
    ...
    console.log(`  Components WITHOUT milestones: ${totalComponents - componentsWithMilestones}`);
    console.log(`  Percentage with milestones: ${((componentsWithMilestones / totalComponents) * 100).toFixed(1)}%`);
    ...
    recentComponents.forEach(c => {
      ...
      if (c.milestones.length > 0) {
        c.milestones.forEach(m => {
          console.log(`    - ${m.milestoneName} (order: ${m.milestoneOrder}, weight: ${m.weight})`);
        });
      } else {
    ...
    const componentsWithTemplateNoMilestones = await prisma.component.findMany({
      where: {
        AND: [
    ...
    if (componentsWithTemplateNoMilestones.length > 0) {
      ...
      componentsWithTemplateNoMilestones.forEach(c => {
        console.log(`  ${c.displayId || c.componentId} (Template: ${c.milestoneTemplateId})`);
      });
    }
    ...
    console.log(`  Distinct components with milestones: ${distinctComponents.length}`);
    
    if (totalMilestones > 0) {
      const avgMilestonesPerComponent = totalMilestones / distinctComponents.length;
      console.log(`  Average milestones per component: ${avgMilestonesPerComponent.toFixed(1)}`);
    }
