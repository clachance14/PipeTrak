  ...
  try {
    ...
    for (const project of oldProjects) {
      ...
      if (components.length > 0) {
        ...
        console.log(`  Deleted milestones for ${components.length} components`);
    ...
    console.log(`Creating ${sdoData.components.length} components with milestones...`);
    let componentCount = 0;
    let milestoneCount = 0;

    for (const componentData of sdoData.components) {
      // Calculate completion status
      const completedMilestones = componentData.milestones.filter(m => m.completed).length;
      ...
      if (!drawingId) {
        console.warn(`  ⚠️  Drawing ${componentData.drawingNumber} not found for component ${componentData.componentId}`);
        continue;
      }
      ...
      const component = await prisma.component.create({
        data: {
          ...
          workflowType: (componentData.workflowType || "MILESTONE_DISCRETE") as any,
          status,
          completionPercent,
          drawingId,
      ...
      for (let i = 0; i < componentData.milestones.length; i++) {
        ...
        await prisma.componentMilestone.create({
          data: {
            ...
            completedAt: milestone.completed ? new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000) : null,
      ...
      if (componentCount % 10 === 0) {
        console.log(`  Progress: ${componentCount}/${sdoData.components.length} components created...`);
      }
    }
