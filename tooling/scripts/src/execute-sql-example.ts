...
import { executeSQLFile, SQLExecutor } from './lib/sql-executor';
...
async function exampleUsage() {
  ...
  try {
    ...
    const examplePath = path.resolve(__dirname, '../../../packages/database/supabase/migrations/example.sql');
  ...
  try {
    ...
    if (dbInfo.version) {
      ...
      console.log(`   🗄️  Database: ${version.split(' ')[0]} ${version.split(' ')[1]}`);
    }
