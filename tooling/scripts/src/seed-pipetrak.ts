  ...
  try {
    ...
    for (const area of areas) {
      for (const system of systems) {
        for (let i = 0; i < 20; i++) { // 20 components per area/system
          ...
          const templateId = resolveTemplateForComponent(type, componentId, "MILESTONE_DISCRETE", templates);
    ...
    for (const component of components.slice(0, 100)) { // Only do first 100 to speed up
      const template = Array.from(templates.values()).find(t => t.id === component.milestoneTemplateId);
    ...
    for (const component of recentComponents) {
      ...
      if (milestone) {
        await prisma.componentMilestone.update({
          ...
          data: {
            isCompleted: true,
            completedAt: new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000), // Random within last 7 days
            completedBy: user.id,
          }
        });
      }
