...
async function applyDashboardMigration() {
  ...
  const migrationPath = path.join(__dirname, '../../../packages/database/supabase/migrations/20250810T1201_dashboard_functions.sql');
  ...
  try {
    ...
    if (result.error) {
      console.error('❌ Failed to apply migration:', result.error.message);
      process.exit(1);
    }
    ...
    for (const funcName of functions) {
      try {
        ...
        if (error) {
          ...
          console.log(`✅ ${funcName}: Function exists and returns data`);
        }
      } catch (err: any) {
