...
async function dropTrigger() {
  ...
  try {
    ...
    await client.query('DROP FUNCTION IF EXISTS component_after_insert_trigger() CASCADE');
    console.log('Dropped function component_after_insert_trigger');
    
    await client.query('DROP FUNCTION IF EXISTS initialize_component_milestones(uuid) CASCADE');
    await client.query('DROP FUNCTION IF EXISTS initialize_component_milestones(text) CASCADE');
