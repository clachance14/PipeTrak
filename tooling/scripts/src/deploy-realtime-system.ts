...
async function runSystemTest() {
  ...
  try {
    ...
    console.log('✅ System test completed - check database logs for results');
    return true;
  } catch (error) {
...
async function testAPIEndpoints() {
  ...
  try {
    ...
    const healthResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/pipetrak/realtime/health`);
