...
async function verifyGasketFix() {
  ...
  for (const milestone of gasket.milestones) {
    ...
    console.log(`${status} ${milestone.milestoneName.padEnd(20)} ${weight}% credit`);
    
    // Check for wrong milestones
    if (milestone.milestoneName === 'Fit-up' || 
  ...
  const expectedMilestones = ['Receive', 'Install', 'Punch', 'Test', 'Restore'];
  ...
  if (gasket.milestoneTemplate?.name === 'Reduced Milestone Set') {
    ...
    console.log(`❌ Template assignment wrong: ${gasket.milestoneTemplate?.name}`);
  }
  ...
  for (const g of allGaskets) {
    const hasInstall = g.milestones.some(m => m.milestoneName === 'Install');
    const hasWrong = g.milestones.some(m => 
      m.milestoneName === 'Fit-up' || m.milestoneName === 'Welded'
    );
