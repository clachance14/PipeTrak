...
async function applyDashboardFunctions() {
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, '../../../packages/database/supabase/migrations/20250811T1500_create_dashboard_functions.sql');
    ...
    try {
      ...
      const functions = [
        ...
        { name: 'get_test_package_readiness', params: ['test-project-id'] },
        { name: 'get_recent_activity', params: ['test-project-id', '10'] }
      ];

      for (const func of functions) {
        try {
          const paramStr = func.params.map(p => `'${p}'`).join(', ');
          const result = await client.query(`SELECT ${func.name}(${paramStr}) as result`);
          
          const data = result.rows[0]?.result;
          if (data && typeof data === 'object') {
            if (data.error) {
              console.log(`⚠️  ${func.name}: ${data.error} (expected for test project)`);
            } else {
              console.log(`✅ ${func.name}: Function working, returns valid data`);
            }
          } else {
            console.log(`✅ ${func.name}: Function exists and executed`);
          }
        } catch (err: any) {
