...
const ProgressReportSchema = z.object({
	...
	options: z
		.object({
			...
			groupBy: z
				.enum(["area", "system", "testPackage", "drawing"])
				.default("area"),
		})
		.optional()
		.default({}),
...
export const exportsRouter = new Hono()
	...
	.post("/components", async (c) => {
		try {
			...
			if (format === "csv") {
				...
				c.header(
					"Content-Disposition",
					`attachment; filename="${project.jobNumber}_components_export.csv"`,
				);
				return c.body(csvData);
			}
			const excelBuffer = await generateComponentExcel(
				components,
				project,
				options,
			);
			...
			c.header(
				"Content-Disposition",
				`attachment; filename="${project.jobNumber}_components_export.xlsx"`,
			);
	...
	.post("/progress-report", async (c) => {
		try {
			...
			c.header(
				"Content-Disposition",
				`attachment; filename="${project.jobNumber}_progress_report.xlsx"`,
			);
			return c.body(excelBuffer);
		} catch (error) {
			if (error instanceof z.ZodError) {
		...
		try {
			...
			const templates = [
				...
				},
				{
					...
					description:
						"Complete component data with milestones and specifications",
					format: "excel",
					columns: [
						"componentId",
						"displayId",
	...
	.post("/custom", async (c) => {
		try {
			...
			if (format === "csv") {
				const csvData = await generateCustomCSV(
					components,
					exportColumns || [],
					exportOptions,
				);
				...
				c.header(
					"Content-Disposition",
					`attachment; filename="${project.jobNumber}_custom_export.csv"`,
				);
				return c.body(csvData);
			}
			const excelBuffer = await generateCustomExcel(
				components,
				project,
				exportColumns || [],
				exportOptions,
			...
			c.header(
				"Content-Disposition",
				`attachment; filename="${project.jobNumber}_custom_export.xlsx"`,
			);
			return c.body(excelBuffer);
		} catch (error) {
			if (error instanceof z.ZodError) {
				return c.json(
					{
						error: "Invalid custom export parameters",
						details: error.errors,
					},
					400,
				);
	...
	.post("/roc-progress", async (c) => {
      ...
      `) as any[];
			...
			if (format === "csv") {
				...
				c.header(
					"Content-Disposition",
					`attachment; filename="${project.jobNumber}_roc_progress.csv"`,
				);
				return c.body(csvData);
			}
			if (format === "pdf") {
				const pdfBuffer = await generateROCProgressPDF(
					rocData,
					project,
					options,
				);
				...
				c.header(
					"Content-Disposition",
					`attachment; filename="${project.jobNumber}_roc_progress.pdf"`,
				);
				return c.body(pdfBuffer);
			}
			const excelBuffer = await generateROCProgressExcel(
				rocData,
				project,
				options,
			);
			...
			c.header(
				"Content-Disposition",
				`attachment; filename="${project.jobNumber}_roc_progress.xlsx"`,
			);
			return c.body(excelBuffer);
		} catch (error) {
			if (error instanceof z.ZodError) {
			...
			return c.json(
				{ error: "Failed to export ROC progress report" },
				500,
			);
		}
	})
	...
	.post("/components/stream", async (c) => {
		try {
			...
			if (format === "excel") {
				...
				c.header(
					"Content-Disposition",
					`attachment; filename="${project.jobNumber}_components_streamed.xlsx"`,
				);
			...
			c.header(
				"Content-Disposition",
				`attachment; filename="${project.jobNumber}_components_streamed.csv"`,
			);
			...
			while (hasMore) {
				const componentChunk = await getComponentChunk(
					projectId,
					filters,
					options,
					offset,
				...
				if (isFirstChunk) {
					csvOutput += await generateComponentCSV(
						componentChunk,
						options,
					);
					isFirstChunk = false;
					...
					const chunkCSV = await generateComponentCSV(
						componentChunk,
						options,
					);
	...
	.post("/progress-summary", async (c) => {
		try {
			...
			const { projectId, weekEnding, groupBy, format, options } = z
				.object({
					...
					weekEnding: z
						...
						.optional(),
					groupBy: z
						.enum(["area", "system", "testPackage", "iwp"])
						.default("area"),
					format: z.enum(["csv", "excel", "pdf"]).default("excel"),
					options: z
						.object({
			...
			if (format === "csv") {
				...
				c.header(
					"Content-Disposition",
					`attachment; filename="PipeTrak_Progress_${project.jobNumber}_WE_${reportData.weekEnding}_FINAL.csv"`,
				);
				return c.body(csvData);
			}
			if (format === "pdf") {
				...
				c.header(
					"Content-Disposition",
					`attachment; filename="PipeTrak_Progress_${project.jobNumber}_WE_${reportData.weekEnding}_FINAL.pdf"`,
				);
				return c.body(pdfBuffer);
			}
			...
			c.header(
				"Content-Disposition",
				`attachment; filename="PipeTrak_Progress_${project.jobNumber}_WE_${reportData.weekEnding}_FINAL.xlsx"`,
			);
			return c.body(excelBuffer);
		} catch (error) {
			if (error instanceof z.ZodError) {
			...
			return c.json(
				{ error: "Failed to export Progress Summary Report" },
				500,
			);
		}
	})
	...
	.post("/test-packages", async (c) => {
      ...
      `) as any[];
			...
			if (!testPackageData) {
				return c.json(
					{ error: "Failed to get test package data" },
					500,
				);
			}

			if (format === "csv") {
				const csvData = await generateTestPackageCSV(
					testPackageData,
					options,
				);
				...
				c.header(
					"Content-Disposition",
					`attachment; filename="${project.jobNumber}_test_packages.csv"`,
				);
				return c.body(csvData);
			}
			if (format === "pdf") {
				const pdfBuffer = await generateTestPackagePDF(
					testPackageData,
					project,
					options,
				);
				...
				c.header(
					"Content-Disposition",
					`attachment; filename="${project.jobNumber}_test_packages.pdf"`,
				);
				return c.body(pdfBuffer);
			}
			const excelBuffer = await generateTestPackageExcel(
				testPackageData,
				project,
				options,
			);
			...
			c.header(
				"Content-Disposition",
				`attachment; filename="${project.jobNumber}_test_packages.xlsx"`,
			);
			return c.body(excelBuffer);
		} catch (error) {
			if (error instanceof z.ZodError) {
			...
			return c.json(
				{ error: "Failed to export test package report" },
				500,
			);
		}
	});
...
async function generateComponentCSV(
	components: any[],
	options: any,
): Promise<string> {
	const headers = [
	...
	const rows = components.map((component) => {
		...
		if (options.includeMilestones && component.milestones) {
			const completedMilestones = component.milestones.filter(
				(m: any) => m.isCompleted,
			).length;
...
async function generateComponentExcel(
	...
	if (options.includeMilestones) {
		headers.push(
			{
				key: "milestonesCompleted",
				header: "Milestones Completed",
				width: 15,
			},
...
async function generateProgressReportExcel(data: any): Promise<Buffer> {
	...
	summarySheet.getCell("A1").value =
		`${project.jobName} (${project.jobNumber}) Progress Report`;
	...
	summarySheet.getCell(`B${currentRow}`).value =
		`${(statistics.overall._avg.completionPercent || 0).toFixed(1)}%`;
	currentRow++;
	...
	for (const stat of statistics.byStatus) {
		...
		summarySheet.getCell(`C${currentRow}`).value =
			`${((stat._count / statistics.overall._count) * 100).toFixed(1)}%`;
		currentRow++;
	}
...
async function addComponentsToSheet(
...
async function addStatisticsToSheet(
	worksheet: ExcelJS.Worksheet,
	stats: any[],
	groupName: string,
) {
...
async function addDrawingsToSheet(
	worksheet: ExcelJS.Worksheet,
	drawings: any[],
) {
	const headers = [
...
function buildIncludeOptions(columns: string[]): any {
	...
	if (
		columns.includes("drawing") ||
		columns.includes("drawingNumber") ||
		columns.includes("drawingTitle")
	) {
...
async function generateCustomExcel(
	...
	const worksheetColumns = columns
...
async function generateROCProgressExcel(
	...
	summarySheet.getCell("A1").value =
		`${project.jobName} (${project.jobNumber}) ROC Progress Report`;
	summarySheet.getCell("A1").font = { size: 16, bold: true };
	summarySheet.getCell("A1").alignment = { horizontal: "center" };
	...
	summarySheet.getCell(`A${currentRow}`).value =
		"ROC Configuration (Milestone Weights):";
	summarySheet.getCell(`A${currentRow}`).font = { bold: true };
	currentRow++;

	if (rocData.summary?.rocConfiguration) {
		Object.entries(rocData.summary.rocConfiguration).forEach(
			([milestone, weight]) => {
				summarySheet.getCell(`B${currentRow}`).value =
					`${milestone}: ${weight}%`;
				currentRow++;
	...
	if (rocData.breakdowns?.systems?.length > 0) {
		...
		await addBreakdownToSheet(
			systemSheet,
			rocData.breakdowns.systems,
			"System",
		);
	}
	...
	if (rocData.breakdowns?.testPackages?.length > 0) {
		...
		await addBreakdownToSheet(
			testPackageSheet,
			rocData.breakdowns.testPackages,
			"Test Package",
		);
	}
...
async function generateTestPackageExcel(
	testPackageData: any,
	project: any,
	options: any,
): Promise<Buffer> {
	...
	summarySheet.getCell("A1").value =
		`${project.jobName} Test Package Readiness Report`;
	summarySheet.getCell("A1").font = { size: 16, bold: true };
	summarySheet.getCell("A1").alignment = { horizontal: "center" };
	...
	const headers = [
		...
		{
			key: "estimatedDaysToReady",
			header: "Est. Days to Ready",
			width: 16,
		},
	];
	...
	if (options.includeBlockingDetails) {
		...
		const blockingHeaders = [
			...
			{ key: "completionPercent", header: "Completion %", width: 12 },
			{
				key: "blockingMilestones",
				header: "Blocking Milestones",
				width: 25,
			},
		];
...
async function addBreakdownToSheet(
	worksheet: ExcelJS.Worksheet,
	breakdownData: any[],
	categoryName: string,
) {
...
async function generateProgressSummaryCSV(reportData: any): Promise<string> {
	...
	if (options.includeGrandTotal) {
		const totalComponents = currentWeekData.reduce(
			(sum: number, row: any) => sum + Number(row.component_count),
			0,
		);
...
async function generateProgressSummaryExcel(reportData: any): Promise<Buffer> {
	...
	worksheet.getCell("A4").value =
		`Report Status: ${isLocked ? "FINAL" : "PRELIMINARY"} ${isLocked ? `(Locked ${tuesday9AM.toLocaleDateString()}, 9:00 AM)` : ""}`;
...
function calculateWeightedTotals(data: any[]): any {
	...
	data.forEach((row: any) => {
		...
		weightedInstalled +=
			componentCount * Number(row.installed_percent || 0);
	...
	return {
		...
		installed:
			Math.round((weightedInstalled / totalComponents) * 100) / 100,
