...
const BroadcastSchema = z.object({
  projectId: z.string().cuid(),
  type: z.enum(['user_presence', 'component_edit', 'milestone_celebration', 'import_progress', 'custom']),
  payload: z.any(),
});
...
export const realtimeRouter = new Hono()
  ...
  .post(
    ...
    async (c) => {
      try {
        ...
        return c.json({ error: 'Failed to setup realtime subscription' }, 500);
      }
    }
  )
  ...
  .post(
    '/presence',
    validator('json', PresenceUpdateSchema),
    async (c) => {
      try {
        ...
        const channel = createProjectChannel(supabase, data.projectId, user.id);
    ...
    async (c) => {
      try {
        ...
        const sanitizedPayload = {
          ...
          message: typeof data.payload.message === 'string' 
            ? data.payload.message.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '')
        ...
        channel.subscribe(async (status) => {
          if (status === 'SUBSCRIBED') {
            ...
            console.log(`Broadcast ${data.type} to project ${data.projectId}:`, result);
...
export async function broadcastImportProgress(
  ...
  try {
    const supabase = createRealtimeClient();
    if (!supabase) {
      console.log('Realtime disabled - skipping import progress broadcast');
      return;
    }
    const channel = createProjectChannel(supabase, projectId, userId);
    ...
    const broadcastData: Omit<BroadcastEvent, 'timestamp'> = {
      ...
      payload: {
        importJobId,
        ...progress,
        progressPercent: progress.totalRows && progress.processedRows
