...
const ComponentDetailsSchema = z.object({
  ...
  sorting: z.object({
    field: z.enum([
      'componentId', 'type', 'area', 'system', 'completionPercent', 
      'status', 'updatedAt', 'testPackage'
    ]).default('componentId'),
...
const TestPackageReadinessSchema = z.object({
  ...
  filters: z.object({
    ...
    areas: z.array(z.string()).optional(),
    systems: z.array(z.string()).optional(),
    readinessStatus: z.array(z.enum(['ready', 'nearly_ready', 'in_progress', 'not_started'])).optional(),
  }).optional().default({}),
  options: z.object({
...
const AuditTrailSchema = z.object({
  projectId: z.string(),
  filters: z.object({
    entityTypes: z.array(z.enum(['Component', 'Milestone', 'Project'])).optional(),
...
function calculateOverallProgress(data: any[]): number {
  ...
  return totalComponents > 0 ? Math.round((weightedProgress / totalComponents) * 100) / 100 : 0;
}
...
export const reportsRouter = new Hono()
  ...
  .post("/progress-summary", async (c) => {
    try {
      ...
      const { projectId, weekEnding, groupBy, options, outputFormat } = ProgressSummaryReportSchema.parse(body);
      ...
      const reportData = {
        ...
        summary: {
          totalComponents: (currentWeekData as any[]).reduce((sum, row) => sum + Number(row.component_count), 0),
          overallProgress: calculateOverallProgress(currentWeekData as any[]),
        },
      };
      
      // Log summary
      console.log(`[Progress Summary] Generated report with ${currentWeekData.length} rows, ${reportData.summary.totalComponents} components, ${reportData.summary.overallProgress}% overall progress`);
        ...
        try {
          await prisma.progressSnapshots.create({
            data: {
              ...
              overallCompletionPercent: calculateOverallProgress(currentWeekData as any[]),
              rocWeightedPercent: calculateOverallProgress(currentWeekData as any[]), // Same as overall for now
      ...
      console.error("[API] Progress Summary Report generation error:", error);
  ...
  .post("/generate/progress", async (c) => {
    try {
      const body = await c.req.json();
      const { projectId, filters = {}, options = {} } = ProgressReportSchema.parse(body);
      const userId = c.get("user")?.id;
  ...
  .post("/generate/components", async (c) => {
    try {
      const body = await c.req.json();
      const { projectId, filters = {}, pagination = {}, sorting = {} } = ComponentDetailsSchema.parse(body);
      const userId = c.get("user")?.id;
      ...
      return c.json({ error: "Failed to generate component details report" }, 500);
    }
  })
  ...
  .post("/generate/test-packages", async (c) => {
    try {
      const body = await c.req.json();
      const { projectId, filters = {}, options = {} } = TestPackageReadinessSchema.parse(body);
      const userId = c.get("user")?.id;
      ...
      return c.json({ error: "Failed to generate test package readiness report" }, 500);
    }
  })
  ...
  .post("/generate/trends", async (c) => {
    try {
      const body = await c.req.json();
      const { projectId, timeframe = {}, options = {} } = TrendAnalysisSchema.parse(body);
      const userId = c.get("user")?.id;
      ...
      return c.json({ error: "Failed to generate trend analysis report" }, 500);
    }
  })
  ...
  .post("/generate/audit", async (c) => {
    try {
      const body = await c.req.json();
      const { projectId, filters = {}, pagination = {} } = AuditTrailSchema.parse(body);
      const userId = c.get("user")?.id;
      ...
      return c.json({ error: "Failed to generate audit trail report" }, 500);
    }
  })
  ...
  .post("/generate/bulk", async (c) => {
    try {
      const body = await c.req.json();
      const { projectId, reportTypes, outputFormat, deliveryMethod, options } = BulkReportGenerationSchema.parse(body);
      const userId = c.get("user")?.id;
      ...
      return c.json({ error: "Failed to start bulk report generation" }, 500);
    }
  })
  ...
  .post("/validate-backdating", async (c) => {
    try {
      ...
      const effectiveWeekEnding = weekEndingResult[0]?.effective_week_ending;
      ...
      if (!isAllowed) {
        ...
        if (effectiveDateObj > now) {
          validationMessage = "Cannot set completion date in the future";
        } else if (now > tuesday9AM && effectiveDateObj < currentWeekEnding) {
      ...
      return c.json({
        ...
        data: {
          isAllowed,
          effectiveDate,
          currentWeekEnding: currentWeekEnding.toISOString().split('T')[0],
          effectiveWeekEnding: effectiveWeekEnding.toISOString().split('T')[0],
          tuesday9AMCutoff: tuesday9AM.toISOString(),
          isAfterCutoff: new Date() > tuesday9AM,
          validationMessage: validationMessage || "Backdating is allowed",
        },
      });
  ...
  .post("/snapshot/:projectId", async (c) => {
    try {
      ...
      const { snapshotDate } = z.object({
        snapshotDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Must be YYYY-MM-DD format").optional(),
      }).parse(body);
      ...
      if (!snapshotDateFinal) {
        ...
        ` as any[];
        snapshotDateFinal = lastSundayResult[0].week_ending.toISOString().split('T')[0];
      }
      ...
      return c.json({ error: "Failed to generate progress snapshot" }, 500);
    }
  })
