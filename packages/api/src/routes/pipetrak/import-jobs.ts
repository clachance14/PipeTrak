...
export const importJobsRouter = new Hono()
	...
	.post("/validate", async (c) => {
		try {
			...
			if (!fileData || !mappings) {
				return c.json(
					{ error: "File data and mappings are required" },
					400,
				);
			}
			...
			const previewData = validation.validRows
				.slice(0, 10)
				.map((validRow, index) => ({
					rowNumber: index + 1,
					data: validRow,
	...
	.get("/:id", async (c) => {
		try {
			...
			console.log(
				`[API] GET /api/pipetrak/import/${id} - User: ${userId}`,
			);
			...
			if (!job) {
				console.log(
					`[API] Import job ${id} not found for user ${userId}`,
				);
				return c.json({ error: "Import job not found" }, 404);
			}

			console.log(
				`[API] Found import job ${id} with status: ${job.status}`,
			);
			return c.json(job);
		} catch (error) {
			console.error(
				`[API] Error fetching import job ${c.req.param("id")}:`,
				error,
			);
	...
	.post("/cleanup-stuck", async (c) => {
		try {
			...
			const accessibleStuckJobs = stuckJobs.filter(
				(job) => job.project.organization.members.length > 0,
			);
						...
						errors: [
							{
								...
								timestamp: new Date(),
								details:
									"Job was stuck in PROCESSING status for more than 10 minutes",
							},
						],
					},
				}),
			...
			console.log(
				`Cleaned up ${accessibleStuckJobs.length} stuck import jobs by user ${userId}`,
			);
	...
	.post("/:id/process", async (c) => {
		try {
			...
			if (!job) {
				return c.json(
					{ error: "Import job not found or already processed" },
					403,
				);
			}
...
async function processImportJob(
	...
	const timeoutPromise = new Promise((_, reject) => {
		setTimeout(() => {
			reject(
				new Error(
					`Import job timed out after ${MAX_PROCESSING_TIME / 1000} seconds`,
				),
			);
		}, MAX_PROCESSING_TIME);
	});
	...
	try {
		...
		if (error.message.includes("timeout")) {
			...
			userFriendlyMessage =
				"There was an issue with component instance calculations. This may be caused by duplicate components in your import file. Please review your data and try again.";
		} else if (error.message.includes("validation")) {
			userFriendlyMessage =
				"Data validation failed. Please check your import file format and ensure all required fields are present.";
		} else if (error.message.includes("connection pool")) {
			userFriendlyMessage =
				"Database connection issue. Please try importing smaller batches or try again in a few minutes.";
		}
		...
		try {
			...
			console.error(
				`Import job ${jobId}: Failed to update job status to FAILED:`,
				updateError,
			);
...
async function processImportJobInternal(
	...
	try {
		...
		console.log(
			`Import job ${jobId}: Starting background processing with ${MAX_PROCESSING_TIME / 1000}s timeout`,
		);
		...
		console.log(
			`Import job ${jobId}: Extracting drawing numbers from import data...`,
		);
		const uniqueDrawingNumbers = new Set<string>();

		for (const row of processRows) {
			// Find the drawing ID column from mappings
			const drawingMapping = mappings.find(
				(m) => m.targetField === "drawingId",
			);
			if (drawingMapping && row[drawingMapping.sourceColumn]) {
				const drawingNumber = row[drawingMapping.sourceColumn]
					.toString()
					.trim();
				if (drawingNumber) {
		...
		console.log(
			`Import job ${jobId}: Found ${uniqueDrawingNumbers.size} unique drawing numbers`,
		);
		...
		if (missingDrawings.length > 0) {
			...
			console.log(
				`Import job ${jobId}: Created ${createdDrawings.length} missing drawings in bulk`,
			);
		}

		console.log(
			`Import job ${jobId}: Drawing number to ID mapping created with ${drawingNumberToIdMap.size} entries`,
		);

		// Get validation context - pass drawing numbers for validation
		const existingDrawingNumbers = new Set(
			Array.from(drawingNumberToIdMap.keys()),
		);
		...
		console.log(
			`Import job ${jobId}: Validation completed with ${validation.validRows.length} valid rows, ${validation.errors.length} errors`,
		);

		// Debug: Check if we have valid rows
		if (validation.validRows.length === 0) {
			console.error(
				`Import job ${jobId}: CRITICAL - No valid rows after validation! This is likely the problem.`,
			);
			console.error(
				"Sample validation error (if any):",
				validation.errors[0],
			);
		...
		try {
			console.log(
				`Import job ${jobId}: Fetching existing components for project ${uploadData.projectId}`,
			);
			...
			console.log(
				`Import job ${jobId}: Found ${existingComponentData.length} existing components`,
			);
		} catch (existingError: any) {
			console.error(
				`Import job ${jobId}: Failed to fetch existing components:`,
		...
		console.log(
			`Import job ${jobId}: Built existing components map with ${existingComponents.size} unique drawing+component combinations`,
		);

		// Ensure milestone templates exist and assign them to components
		console.log(
			`Import job ${jobId}: Ensuring milestone templates exist for project`,
		);
		const templatesByName = await TemplateResolver.ensureTemplatesExist(
			uploadData.projectId,
		);
		console.log(
			`Import job ${jobId}: Created/loaded ${templatesByName.size} templates:`,
			Array.from(templatesByName.keys()),
		);

		if (!templatesByName || templatesByName.size === 0) {
			console.error(
				`Import job ${jobId}: CRITICAL - No templates created or found!`,
			);
		...
		console.log(
			`Import job ${jobId}: Built templates by ID map with ${templatesById.size} templates`,
		);

		// Assign templates to components that don't have them
		const componentsWithTemplates = validation.validRows.map(
			(component) => {
		...
		console.log(
			`Import job ${jobId}: Converting drawing numbers to drawing IDs...`,
		);
		const failedDrawingMappings: string[] = [];
		const componentsWithDrawingIds = componentsWithTemplates.map(
			(component) => {
				if (component.drawingId) {
					const originalDrawingNumber = component.drawingId;
					const drawingId = drawingNumberToIdMap.get(
						component.drawingId,
					);
					if (drawingId) {
						...
						console.warn(
							`Import job ${jobId}: Warning - no drawing ID found for drawing number: ${originalDrawingNumber}`,
						);
		...
		console.log(
			`Import job ${jobId}: Converted drawing numbers to IDs for ${componentsWithDrawingIds.length} components`,
		);

		// Debug: Log conversion results
		if (failedDrawingMappings.length > 0) {
			console.warn(
				`Import job ${jobId}: Failed to map ${failedDrawingMappings.length} drawing numbers:`,
				failedDrawingMappings,
			);
		}

		// Validate all components have valid drawing IDs
		const componentsWithoutDrawing = componentsWithDrawingIds.filter(
			(component) =>
				!component.drawingId ||
				failedDrawingMappings.includes(component.drawingId),
		);

		if (componentsWithoutDrawing.length > 0) {
			const missingDrawings = [
				...new Set(
					componentsWithoutDrawing.map(
						(c) => c.drawingId || "MISSING",
					),
				),
			];
			const errorMessage =
				failedDrawingMappings.length > 0
		...
		console.log(
			`Import job ${jobId}: Starting instance tracking for ${componentsWithDrawingIds.length} valid rows`,
		);
		...
		console.log(
			`Import job ${jobId}: Instance tracking completed. Processing ${componentsWithInstances.length} components with instances`,
		);

		if (componentsWithInstances.length === 0) {
			console.error(
				`Import job ${jobId}: NO COMPONENTS TO PROCESS! This is the problem.`,
			);
			console.error(
			...
			throw new Error(
				"No valid components to process after instance tracking",
			);
		}
		...
		console.log(
			`Import job ${jobId}: Starting batch processing of ${componentsWithInstances.length} components`,
		);

		// Use smaller batch sizes to prevent connection pool exhaustion and timeouts
		const hasFieldWelds = componentsWithInstances.some(
			(c) => c.type === "FIELD_WELD",
		);
		...
		await processInBatches(
			...
			async (batch) => {
				...
				if (Date.now() - jobStartTime > MAX_PROCESSING_TIME) {
					console.error(
						`Import job ${jobId}: Timeout exceeded (${MAX_PROCESSING_TIME / 1000}s), stopping processing`,
					);
					throw new Error(
						`Import processing timed out after ${MAX_PROCESSING_TIME / 1000} seconds. This may indicate a problem with the import data or system performance.`,
					);
				}
				...
				const fieldWeldComponents = batch.filter(
					(comp) => comp.type === "FIELD_WELD",
				);
				...
				if (fieldWeldComponents.length > 0) {
					const uniqueDrawingIds = [
						...new Set(
							fieldWeldComponents.map((fw) => fw.drawingId),
						),
					];
					...
					const existingDrawingIds = new Set(
						existingDrawings.map((d) => d.id),
					);
					missingDrawingIds = uniqueDrawingIds.filter(
						(id) => !existingDrawingIds.has(id),
					);

					if (missingDrawingIds.length > 0) {
						console.log(
							`Import job ${jobId}: ${missingDrawingIds.length} missing drawings will be skipped`,
						);
					}
					...
					existingFieldWelds = new Set(
						existingWelds.map((w) => w.weldIdNumber),
					);
					const existingComponentWeldIds = new Set(
						existingComponents.map((c) => c.weldId).filter(Boolean),
					);

					duplicatedWeldIds = uniqueWeldIds.filter((id) =>
						existingFieldWelds.has(id),
					);
					const componentDuplicates = uniqueWeldIds.filter((id) =>
						existingComponentWeldIds.has(id),
					);

					if (duplicatedWeldIds.length > 0) {
						console.log(
							`Import job ${jobId}: ${duplicatedWeldIds.length} existing field welds will be updated`,
						);
					}

					if (componentDuplicates.length > 0) {
						console.log(
							`Import job ${jobId}: ${componentDuplicates.length} existing components with weldIds found`,
						);
					}
				...
				await prisma.$transaction(
					async (tx) => {
						try {
							console.log(
								`Import job ${jobId}: Starting transaction for batch of ${batch.length} components`,
							);

							// Pre-validate all components before processing
							const invalidComponents = [];
							for (const component of batch) {
								const instanceNumber = (component as any)
									.instanceNumber;
								const totalInstances = (component as any)
							...
							if (invalidComponents.length > 0) {
								console.error(
									`Import job ${jobId}: Pre-validation failed for ${invalidComponents.length} components:`,
									invalidComponents,
								);
								throw new Error(
									`Validation failed: ${invalidComponents.map((c) => `${c.componentId}: ${c.error}`).join("; ")}`,
								);
							}
							...
							for (const componentData of batch) {
								...
								if (!componentData.milestoneTemplateId) {
									...
									componentData.milestoneTemplateId =
										defaultTemplate?.id;

									if (!componentData.milestoneTemplateId) {
										throw new Error(
											`No milestone templates available for project ${uploadData.projectId}`,
										);
									}
								}
								...
								if (existingComponentsMap.has(key)) {
									...
									newComponents.push({
										...cleanComponentData,
										type: convertToComponentType(
											componentData.type,
										),
										projectId: uploadData.projectId,
										displayId: (componentData as any)
											.displayId,
										instanceNumber:
											(componentData as any)
												.instanceNumber || 1,
							...
							const regularComponents = newComponents.filter(
								(comp) => comp.type !== "FIELD_WELD",
							);
							const fieldWeldComponents = newComponents.filter(
								(comp) => comp.type === "FIELD_WELD",
							);
							...
							if (regularComponents.length > 0) {
								try {
									console.log(
										`Import job ${jobId}: Creating ${regularComponents.length} regular components`,
									);
									...
									const validateComponent = (
										...
										if (!comp.projectId)
											throw new Error(
												`Component ${index}: missing projectId`,
											);
										if (!comp.componentId)
											throw new Error(
												`Component ${index}: missing componentId`,
											);
										if (!comp.drawingId)
											throw new Error(
												`Component ${index}: missing drawingId`,
											);
										if (!comp.milestoneTemplateId)
											throw new Error(
												`Component ${index}: missing milestoneTemplateId`,
											);
										if (
											comp.instanceNumber &&
											comp.totalInstancesOnDrawing &&
											comp.instanceNumber >
												comp.totalInstancesOnDrawing
										) {
											throw new Error(
												`Component ${index} (${comp.componentId}): instanceNumber ${comp.instanceNumber} > totalInstancesOnDrawing ${comp.totalInstancesOnDrawing}`,
											);
										}
									};

									regularComponents.forEach(
										validateComponent,
									);
									...
									console.log(
										`Import job ${jobId}: Successfully created ${createResult.length} regular components`,
									);
									createdComponents.push(...createResult);
									successCount += createResult.length;
								} catch (error: any) {
									console.error(
										`Import job ${jobId}: Failed to create regular components:`,
										{
											error: error.message,
											code: error.code,
									...
									if (
										error.message.includes("instanceNumber")
									) {
										console.error(
											`Import job ${jobId}: Instance number validation errors. Sample components:`,
											regularComponents
												.slice(0, 3)
												.map((c) => ({
							...
							console.log(
								`Import job ${jobId}: Processing ${fieldWeldComponents.length} field welds in transaction`,
							);

							if (fieldWeldComponents.length > 0) {
								// Filter out field welds with missing drawings
								const validFieldWelds =
									fieldWeldComponents.filter(
										(fw) =>
											!missingDrawingIds.includes(
												fw.drawingId,
											),
									);

								// Track skipped welds with missing drawings
								const skippedWelds = fieldWeldComponents.filter(
									(fw) =>
										missingDrawingIds.includes(
											fw.drawingId,
										),
								...
								if (validFieldWelds.length > 0) {
									console.log(
										`Import job ${jobId}: Processing ${validFieldWelds.length} valid field welds (${skippedWelds.length} skipped)`,
									);
									...
									for (const fieldWeldData of validFieldWelds) {
										...
										const weldIdentifier =
											fieldWeldData.weldId ||
											fieldWeldData.componentId;
										const isExistingWeld =
											existingFieldWelds.has(
												weldIdentifier,
											);
										const existingComponent =
											existingComponentsByWeldId.get(
												weldIdentifier,
											);
										...
										if (isExistingWeld) {
											...
											existingWeldUpdates.push({
												...
												updateData: {
													...
													baseMetal:
														baseMetal ||
														fieldWeldData.material,
													pwhtRequired:
														pwhtRequired || false,
													ndeTypes: ndeTypes || [],
													packageNumber:
														fieldWeldData.testPackage ||
														"TBD",
													specCode:
														fieldWeldData.spec ||
														"TBD",
													comments:
														comments ||
														`Updated from ${uploadData.originalName}`,
												},
											});
											...
											if (existingComponent) {
												componentUpdates.push({
													...
													data: {
														area:
															fieldWeldData.area ||
															existingComponent.area ||
															"",
														system:
															fieldWeldData.system ||
															existingComponent.system ||
															"",
														testPackage:
															fieldWeldData.testPackage ||
															existingComponent.testPackage ||
															"TBD",
													},
												});
											...
											newFieldWeldData.push({
												...
												testPressure: {},
												specCode:
													fieldWeldData.spec || "TBD",
												comments:
													comments ||
													`Imported from ${uploadData.originalName}`,
											});
										}
									}
									...
									if (
										existingWeldUpdates.length > 0 ||
										newFieldWeldData.length > 0
									) {
										console.log(
											`Import job ${jobId}: Bulk processing - ${existingWeldUpdates.length} updates, ${newFieldWeldData.length} new field welds`,
										);
									}
									...
									for (const update of existingWeldUpdates) {
										try {
											...
											console.error(
												`Failed to update field weld ${update.weldId}:`,
												error.message,
											);
											errorCount++;
											errors.push({
									...
									if (componentUpdates.length > 0) {
										...
										for (const [_, group] of updateGroups) {
											try {
												...
												console.error(
													`Failed to bulk update ${group.ids.length} components:`,
													error.message,
												);
									...
									if (newComponentData.length > 0) {
										try {
											...
											console.error(
												"Failed to bulk create components for field welds:",
												error.message,
											);
											errorCount +=
												newComponentData.length;
									...
									if (newFieldWeldData.length > 0) {
										try {
											...
											successCount +=
												newFieldWeldData.length;
										} catch (error: any) {
											console.error(
												"Failed to bulk create field welds:",
												error.message,
											);
											errorCount +=
												newFieldWeldData.length;
							...
							if (existingComponents.length > 0) {
								...
								const existingUpdateGroups = new Map<
									string,
									{ ids: string[]; data: any }
								>();
								...
								for (const {
									...
									const componentUpdateData = {
										...
										instanceNumber: (data as any)
											.instanceNumber,
										totalInstancesOnDrawing: (data as any)
											.totalInstancesOnDrawing,
										updatedAt: new Date(),
									};

									const updateKey =
										JSON.stringify(componentUpdateData);
									if (!existingUpdateGroups.has(updateKey)) {
										existingUpdateGroups.set(updateKey, {
											ids: [],
											data: componentUpdateData,
										});
									}
									existingUpdateGroups
										.get(updateKey)!
								...
								for (const [_, group] of existingUpdateGroups) {
									try {
										...
										console.error(
											`Failed to bulk update ${group.ids.length} existing components:`,
											error.message,
										);
										...
										for (const id of group.ids) {
											try {
												...
												console.error(
													`Failed to update existing component ${id}:`,
													fallbackError.message,
												);
												errorCount++;
											}
							...
							for (const component of createdComponents) {
								...
								if (template?.milestones) {
									...
									if (
										typeof template.milestones === "string"
									) {
										try {
											milestoneData = JSON.parse(
												template.milestones,
											);
										} catch (e) {
											console.error(
												"Failed to parse milestone template JSON:",
												e,
											);
										}
									} else if (
										...
										typeof template.milestones === "object"
									) {
										milestoneData =
											(template.milestones as any).data ||
											Object.values(template.milestones);
									}
							...
							for (const {
								...
								const existingMilestoneCount =
									await tx.componentMilestone.count({
							...
							console.error(
								`Import job ${jobId}: Transaction failed for batch:`,
								{
							...
							if (batchError.code === "25P02") {
								...
								userFriendlyMessage =
									"Transaction aborted due to constraint violation. This may be caused by duplicate field welds.";
							} else if (batchError.code === "P2002") {
								errorCategory = "CONSTRAINT_VIOLATION";
								userFriendlyMessage =
									"Duplicate records detected. Some components may already exist in the project.";
							} else if (batchError.code === "P2003") {
								errorCategory = "FOREIGN_KEY_VIOLATION";
								userFriendlyMessage =
									"Reference to non-existent records (drawings, weld types, etc.).";
							} else if (batchError.message.includes("timeout")) {
								errorCategory = "TIMEOUT_ERROR";
								userFriendlyMessage =
									"Operation timed out. Try importing smaller batches.";
							}
							...
							console.error(
								`Import job ${jobId}: Detailed error info:`,
								{
									category: errorCategory,
									prismaCode: batchError.code,
									affectedComponents: batch.length,
									fieldWeldsInBatch:
										fieldWeldComponents.length,
								},
							);
				...
				if (batchErrors.length > 0) {
					console.log(
						`Import job ${jobId}: Batch completed with ${batchErrors.length} errors`,
					);

					// Log error details
					batchErrors.forEach((result) => {
						console.error(
							`Import job ${jobId}: Failed component ${result.error.componentId}: ${result.error.error}`,
						);
					});
				}
		...
		console.log(
			`Import job ${jobId}: Import completed. Success: ${successCount}, Errors: ${errorCount}, Total processed: ${successCount + errorCount}`,
		);
		console.log(
			`Import job ${jobId}: Components with instances: ${componentsWithInstances.length}`,
		);
		...
		console.log(
			`Import job ${jobId}: Job status updated to ${finalStatus}`,
		);
