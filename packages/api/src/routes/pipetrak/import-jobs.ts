import { AuditAction, ComponentType, ImportStatus } from "@prisma/client";
					...
					await prisma.component.upsert({
						where: {
							drawingId_componentId_instanceNumber: {
								...
								instanceNumber:
									componentData.instanceNumber || 1,
							},
						},
						create: {
...
async function processImportJobInternal(
	...
	if (componentsWithoutDrawing.length > 0) {
		const missingDrawings = [
			...new Set(
				componentsWithoutDrawing.map((c) => c.drawingId || "MISSING"),
			),
		];
		const errorMessage =
	...
	const componentsWithInstances = await calculateInstanceNumbers(
		...
		async (batch) => {
			...
			if (fieldWeldComponents.length > 0) {
				const uniqueDrawingIds = [
					...new Set(fieldWeldComponents.map((fw) => fw.drawingId)),
				];
				...
				async (tx) => {
					try {
						...
						for (const componentData of batch) {
							...
							if (existingComponentsMap.has(key)) {
								...
								newComponents.push({
									...
									instanceNumber:
										(componentData as any).instanceNumber ||
										1,
									totalInstancesOnDrawing:
										(componentData as any)
						...
						if (fieldWeldComponents.length > 0) {
							...
							const validFieldWelds = fieldWeldComponents.filter(
								(fw) =>
							...
							if (validFieldWelds.length > 0) {
								...
								for (const fieldWeldData of validFieldWelds) {
									...
									const {
										...
										weldTypeCode,
										baseMetal,
										xrayPercent: _xrayPercent,
										pwhtRequired,
										ndeTypes,
										welderId: _welderId,
										dateWelded,
										tieInNumber: _tieInNumber,
										comments,
										...componentFields
									} = fieldWeldData;
									...
									const isExistingWeld =
										existingFieldWelds.has(weldIdentifier);
						...
						if (existingComponents.length > 0) {
							...
							for (const {
								...
								const {
									...
									milestoneTemplateId: _milestoneTemplateId,
