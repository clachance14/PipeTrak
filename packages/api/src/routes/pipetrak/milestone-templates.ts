...
const MilestoneTemplateUpdateSchema = MilestoneTemplateCreateSchema.partial().omit({
  projectId: true,
});
  ...
  .patch("/:id", async (c) => {
    try {
      ...
      if (!template) {
        return c.json({
          code: "ACCESS_DENIED",
          message: "Template not found or insufficient permissions"
        }, 403);
      }
      ...
      if (template._count.components > 0) {
        if (updates.milestones) {
          return c.json({
            ...
            message: "Cannot modify milestones structure when template is in use",
            details: {
              componentsUsing: template._count.components,
              suggestion: "Create a new template or remove components using this template first"
            }
          }, 400);
        }
      }
      ...
      if (updates.milestones) {
        const totalWeight = updates.milestones.reduce((sum, m) => sum + m.weight, 0);
        if (Math.abs(totalWeight - 100) > 0.01) {
          return c.json({
            code: "INVALID_WEIGHTS",
            message: "Milestone weights must sum to 100",
        ...
        const sortedMilestones = updates.milestones.sort((a, b) => a.order - b.order);
        const orders = new Set(sortedMilestones.map(m => m.order));
        if (orders.size !== sortedMilestones.length) {
          return c.json({
            code: "DUPLICATE_ORDERS",
      ...
      if (template._count.components > 0 && !force) {
        return c.json({
          ...
          details: {
            ...
            suggestion: "Use force=true to delete anyway, or remove components using this template first"
          }
        }, 400);
      }
    ...
    try {
      ...
      if (components.length !== componentIds.length) {
        return c.json({
          ...
          message: "Some components not found or belong to different project",
          details: {
            requested: componentIds.length,
            found: components.length,
            missingIds: componentIds.filter(id => 
      ...
      for (const component of components) {
        try {
          ...
          if (existingMilestones.length > 0 && !options.replaceExisting) {
            results.skipped++;
            continue;
          }

          // Prepare new milestones
          const newMilestones = milestoneData.map((milestone, index) => ({
          ...
          if (options.preserveCompleted && existingMilestones.length > 0) {
            const completedMilestones = existingMilestones.filter(m => m.isCompleted);
            
            newMilestones.forEach(newMilestone => {
              const completedMatch = completedMilestones.find(
                cm => cm.milestoneName === newMilestone.milestoneName
              );
          ...
          console.error(`Failed to apply template to component ${component.id}:`, error);
          results.errors.push({
            componentId: component.id,
