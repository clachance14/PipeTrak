...
const fieldWeldCreateSchema = z.object({
  ...
  datePwht: z.string().transform((val) => new Date(val)).optional(),
  ndeTypes: z.array(z.string()).default([]),
  ndeResult: z.enum(["Accept", "Reject", "Repair", "Pending"]).optional(),
  ndeDate: z.string().transform((val) => new Date(val)).optional(),
  turnoverDate: z.string().transform((val) => new Date(val)).optional(),
...
export const fieldWeldsRouter = new Hono()
  ...
  .get(
    "/",
    validator("query", fieldWeldQuerySchema),
    async (c) => {
      const { 
        projectId, 
        packageNumber, 
  ...
  .post(
    "/",
    validator("json", fieldWeldCreateSchema),
    async (c) => {
      ...
      try {
        ...
        if (data.welderId) {
          ...
          if (!welder) {
            return c.json({ error: "Welder not found or inactive" }, 404);
          }
        }
        ...
        if (!milestoneTemplate) {
          return c.json({ error: "No milestone template found for FIELD_WELD components" }, 400);
        }
  ...
  .put(
    "/:id",
    validator("json", fieldWeldUpdateSchema),
    async (c) => {
      const id = c.req.param("id");
  ...
  .put(
    ...
    async (c) => {
      ...
      try {
        ...
        return c.json({ error: "Failed to bulk update field welds" }, 500);
      }
    }
  )
  ...
  .get("/stats/:projectId", async (c) => {
    ...
    try {
      ...
      const acceptanceRate = (acceptedWelds + rejectedWelds) > 0 
        ? Math.round((acceptedWelds / (acceptedWelds + rejectedWelds)) * 100) 
        : 0;

      const pwhtCompletionRate = pwhtRequiredWelds > 0
      ...
      return c.json({ error: "Failed to fetch field weld statistics" }, 500);
    }
  });