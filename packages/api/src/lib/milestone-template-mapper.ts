...
export class MilestoneTemplateMapper {
	...
	async loadTemplates(): Promise<void> {
		...
		for (const template of templates) {
			...
			if (Array.isArray(template.milestones)) {
				...
				try {
					...
					console.warn(
						`Failed to parse milestones for template ${template.name}:`,
						error,
					);
				}
			}
		...
		console.log(
			`MilestoneTemplateMapper: Loaded ${this.templateCache.size} templates for project ${this.projectId}`,
		);
	}
	 ...
	 */
	getTemplateForComponentType(
		componentType: string,
	): MilestoneTemplate | null {
		// Handle empty or null component type
		if (!componentType || typeof componentType !== "string") {
			console.warn(
				`MilestoneTemplateMapper: Invalid component type '${componentType}', using fallback`,
			);
		...
		console.log(
			`MilestoneTemplateMapper: Looking for template for component type: '${trimmedType}'`,
		);
		...
		if (templateName && this.templatesByName.has(templateName)) {
			...
			console.log(
				`MilestoneTemplateMapper: ✅ Exact matched ${componentType} → ${templateName} (${template.milestones.length} milestones)`,
			);
			return template;
		}
		...
		if (templateName && this.templatesByName.has(templateName)) {
			const template = this.templatesByName.get(templateName)!;
			console.log(
				`MilestoneTemplateMapper: ✅ Uppercase matched ${componentType} → ${templateName} (${template.milestones.length} milestones)`,
			);
			return template;
		}
		if (templateName) {
			console.warn(
				`MilestoneTemplateMapper: Template '${templateName}' exists in mapping but not loaded for component type '${normalizedType}'`,
			);
		}
		...
		if (fuzzyMatch && this.templatesByName.has(fuzzyMatch)) {
			...
			console.log(
				`MilestoneTemplateMapper: ✅ Fuzzy matched ${componentType} → ${fuzzyMatch} (${template.milestones.length} milestones)`,
			);
			return template;
		}
		if (fuzzyMatch) {
			console.warn(
				`MilestoneTemplateMapper: Fuzzy match '${fuzzyMatch}' found but template not loaded for component type '${normalizedType}'`,
			);
		}

		// Fallback to default template
		console.warn(
			`MilestoneTemplateMapper: ⚠️ No template mapping for component type '${componentType}' (normalized: '${normalizedType}'), using fallback`,
		);
		return this.getFallbackTemplate();
	}
	...
	private findFuzzyMatch(componentType: string): string | null {
		...
		if (
			componentType.includes("PIPE") ||
			componentType.includes("PIPING")
		) {
			return "Full Milestone Set";
		}
	...
	private getFallbackTemplate(): MilestoneTemplate | null {
		for (const templateName of FALLBACK_TEMPLATE_HIERARCHY) {
			if (this.templatesByName.has(templateName)) {
				const template = this.templatesByName.get(templateName)!;
				console.log(
					`MilestoneTemplateMapper: Using fallback template ${templateName} (${template.milestones.length} milestones)`,
				);
				return template;
			}
		}
		...
		if (availableTemplates.length > 0) {
			...
			console.warn(
				`MilestoneTemplateMapper: No standard fallback templates found, using ${fallbackTemplate.name} as fallback`,
			);
			return fallbackTemplate;
		}

		console.error(
			"MilestoneTemplateMapper: No templates available at all - this should not happen",
		);
		return null;
	}
	...
	async ensureDefaultTemplate(): Promise<MilestoneTemplate> {
		...
		const existingDefault = this.templatesByName.get(
			"Default Component Template",
		);
		if (existingDefault) {
			console.log(
				"MilestoneTemplateMapper: Using existing Default Component Template",
			);
		...
		if (this.templateCache.size === 0) {
			console.log(
				"MilestoneTemplateMapper: No templates found, creating default template",
			);
			...
			try {
				const defaultTemplate = await prisma.milestoneTemplate.create({
					data: {
						...
						description:
							"Auto-created template for component imports",
						milestones: [
							{
								name: "Receive",
								order: 1,
							...
							},
							{
								...
								weight: 20,
								description:
									"Component tested and commissioned",
							},
						],
					},
				});
				...
				console.log(
					`MilestoneTemplateMapper: ✅ Created default template with ${mappedTemplate.milestones.length} milestones`,
				);
				return mappedTemplate;
			} catch (error) {
				console.error(
					"MilestoneTemplateMapper: Failed to create default template:",
					error,
				);
		...
		if (fallbackTemplate) {
			console.warn(
				`MilestoneTemplateMapper: Using ${fallbackTemplate.name} as default template`,
			);
			return fallbackTemplate;
		}
		...
		throw new Error(
			"No milestone templates available and unable to create default template",
		);
	}
}
