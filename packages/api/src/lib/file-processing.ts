...
export const ComponentImportSchema = z.object({
  ...
  workflowType: z.enum(["MILESTONE_DISCRETE", "MILESTONE_PERCENTAGE", "MILESTONE_QUANTITY"]).optional(),
  ...
  xrayPercent: z.union([z.number(), z.string()]).optional().transform(val => {
  ...
  pwhtRequired: z.union([z.boolean(), z.string()]).optional().transform(val => {
    if (typeof val === "string") {
  ...
  ndeTypes: z.union([z.array(z.string()), z.string()]).optional().transform(val => {
    if (typeof val === "string") {
...
export class CSVProcessor {
  ...
  async parseCSV(buffer: Buffer): Promise<{ headers: string[]; rows: any[]; metadata: FileMetadata }> {
...
export class ExcelProcessor {
  ...
  async parseExcel(buffer: Buffer, sheetName?: string): Promise<{ headers: string[]; rows: any[]; metadata: FileMetadata }> {
  ...
  async createExcel(data: any[], headers: string[], sheetName: string = 'Sheet1'): Promise<Buffer> {
    const workbook = new ExcelJS.Workbook();
...
export class ColumnMapper {
  private standardMappings: Record<string, string[]> = {
    componentId: ['component_id', 'part_number', 'tag', 'id', 'component', 'part', 'commodity_code'],
    type: ['type', 'component_type', 'item_type', 'category'],
    workflowType: ['workflow_type', 'workflow', 'work_type'],
    drawingId: ['drawing_id', 'drawing', 'drawing_number', 'dwg', 'drawing_id'],
    spec: ['spec', 'specification', 'specs'],
    size: ['size', 'dimension', 'diameter'],
    material: ['material', 'mat', 'material_type'],
    pressureRating: ['pressure_rating', 'pressure', 'rating', 'press_rating'],
  ...
  autoMapColumns(sourceHeaders: string[]): ColumnMapping[] {
    ...
    const normalizedHeaders = sourceHeaders.map(h => h.toLowerCase().trim().replace(/[^a-z0-9]/g, '_'));

    for (const [targetField, variations] of Object.entries(this.standardMappings)) {
      ...
      for (let i = 0; i < normalizedHeaders.length; i++) {
        ...
        for (const variation of variations) {
          const score = this.calculateSimilarity(normalizedHeader, variation);
      ...
      if (bestMatch) {
        mappings.push({
          ...
          targetField,
          required: ['componentId', 'type', 'workflowType'].includes(targetField),
        });
      }
    }
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      for (const mapping of mappings) {
        ...
        if (targetValue === '' || targetValue === null || targetValue === undefined) {
          ...
          const stringFields = ['size', 'spec', 'material', 'pressureRating', 
                                'description', 'area', 'system', 'testPackage', 
                                'lengthUnit', 'quantityUnit', 'testRequired',
                                // Field weld string fields
                                'weldSize', 'schedule', 'weldTypeCode', 'baseMetal',
                                'welderId', 'dateWelded', 'tieInNumber', 'comments'];
          
          if (typeof targetValue === 'number' && stringFields.includes(mapping.targetField)) {
      ...
      if (mappedData.type === 'FIELD_WELD') {
        ...
        if (mappedData.schedule) {
          ...
          const schedulePattern = /^(Sch\s*\d+|S\d+|XS|XXS|STD|XS|Sch\s*(10|20|30|40|60|80|100|120|140|160))$/i;
        ...
        if (mappedData.weldTypeCode) {
          const weldTypeCode = ensureString(mappedData.weldTypeCode).toUpperCase();
          const validWeldTypes = ['BW', 'SW', 'TW', 'FW', 'CW', 'BT', 'ST'];
        ...
        if (mappedData.pwhtRequired !== undefined) {
          const pwhtValue = String(mappedData.pwhtRequired).toUpperCase();
          if (!['TRUE', 'FALSE', 'YES', 'NO', '1', '0'].includes(pwhtValue)) {
            rowErrors.push({
              row: i + 1,
              field: 'pwhtRequired',
            ...
            mappedData.pwhtRequired = ['TRUE', 'YES', '1'].includes(pwhtValue);
          }
        }

        // Validate NDE types if provided
        if (mappedData.ndeTypes) {
          const validNdeTypes = ['RT', 'PT', 'MT', 'UT', 'VT', 'ET', 'AE'];
          const ndeList = String(mappedData.ndeTypes).split(',').map(nde => nde.trim().toUpperCase());
          const invalidNdeTypes = ndeList.filter(nde => !validNdeTypes.includes(nde));
          ...
          mappedData.ndeTypes = ndeList.filter(nde => validNdeTypes.includes(nde));
        }
      ...
      try {
        ...
        if (validationContext) {
          if (mappedData.drawingId && validationContext.existingDrawings && 
              !validationContext.existingDrawings.has(mappedData.drawingId)) {
            warnings.push({
              row: i + 1,
              field: 'drawingId',
          ...
          if (mappedData.milestoneTemplateId && validationContext.existingTemplates && 
              !validationContext.existingTemplates.has(mappedData.milestoneTemplateId)) {
            warnings.push({
              row: i + 1,
        ...
        if (error instanceof z.ZodError) {
          const zodErrors = error.errors
            .filter(err => err.message && err.message.trim() !== '') // Filter out empty error messages
            .map(err => ({
              row: i + 1,
              field: err.path.join('.'),
              error: err.message || 'Validation error',
              value: err.path.reduce((obj, key) => obj?.[key], mappedData),
            }));
...
export class InstanceTracker {
  static async calculateInstanceNumbers(
    ...
    existingComponents: Map<string, Array<{ componentId: string; instanceNumber: number; drawingId?: string; size?: string }>> = new Map()
  ): Promise<ComponentImportData[]> {
    console.log(`InstanceTracker: Starting instance calculation for ${components.length} components`);
    ...
    console.log(`InstanceTracker: Expanded to ${expandedComponents.length} components after quantity expansion`);
    ...
    for (const component of expandedComponents) {
      if (!component.drawingId) {
        console.warn(`InstanceTracker: Component ${component.componentId} has no drawingId, will be handled separately`);
        continue;
      }
    ...
    console.log(`InstanceTracker: Created ${instanceGroups.size} instance groups`);
    ...
    for (const [key, groupComponents] of instanceGroups) {
      ...
      console.log(`InstanceTracker: Processing group ${key} with ${groupComponents.length} new components`);
      ...
      console.log(`InstanceTracker: Found ${existingInstances.length} existing instances for key ${key}`);
      
      // Calculate next instance number starting from existing + 1
      let nextInstanceNumber = existingInstances.length > 0 ? 
        Math.max(...existingInstances.map(c => c.instanceNumber)) + 1 : 1;
      
      // Calculate total instances on THIS drawing
      const totalInstancesOnThisDrawing = existingInstances.length + groupComponents.length;
      
      console.log(`InstanceTracker: For group ${key}, starting instance number: ${nextInstanceNumber}, total instances on drawing: ${totalInstancesOnThisDrawing}`);
      ...
      for (let i = 0; i < groupComponents.length; i++) {
        ...
        if (instanceNumber > totalInstancesOnThisDrawing) {
          console.error(`InstanceTracker: ERROR - Instance number ${instanceNumber} exceeds total instances ${totalInstancesOnThisDrawing} for ${key}`);
          console.error(`InstanceTracker: Debug - existingInstances: ${existingInstances.length}, newComponents: ${groupComponents.length}, starting: ${nextInstanceNumber}`);
          throw new Error(`Invalid instance calculation: instance ${instanceNumber} > total ${totalInstancesOnThisDrawing} for component ${componentId} on drawing ${drawingId}`);
        }
        ...
        (component as any).totalInstancesOnDrawing = totalInstancesOnThisDrawing;
        (component as any).displayId = displayId;
        
        console.log(`InstanceTracker: Created component instance: ${displayId}, instance ${instanceNumber} of ${totalInstancesOnThisDrawing}`);
    ...
    if (componentsWithoutDrawing > 0) {
      console.warn(`InstanceTracker: ${componentsWithoutDrawing} components processed without drawing IDs`);
    }
    
    console.log(`InstanceTracker: Completed instance calculation. Returning ${processedComponents.length} processed components`);
...
export class ErrorReporter {
  static generateErrorReport(errors: ValidationError[], warnings: ValidationError[]): {
    summary: any;
    csvData: string;
  } {
    const summary = {
      ...
      errorsByField: this.groupErrorsByField(errors),
      warningsByField: this.groupErrorsByField(warnings),
    };
  ...
  private static groupErrorsByField(errors: ValidationError[]): Record<string, number> {
...
export class TemplateResolver {
  ...
  private static readonly COMPONENT_TYPE_PATTERNS: Array<{pattern: RegExp, template: string}> = [
    // Gasket patterns
   ...
   */
  static async loadTemplatesForProject(projectId: string): Promise<Map<string, any>> {
  ...
  static resolveTemplateForComponent(
    ...
    let templateName = this.COMPONENT_TYPE_MAPPING[normalizedType];
    
    // If no exact match, try pattern matching on both type and ID
    if (!templateName) {
      for (const { pattern, template } of this.COMPONENT_TYPE_PATTERNS) {
        if (pattern.test(componentType || '') || pattern.test(componentId || '')) {
    ...
    if (!template) {
      console.error(`TemplateResolver: Template "${templateName}" not found in templates map. Available: ${Array.from(templates.keys()).join(', ')}`);
   ...
   */
  static async ensureTemplatesExist(projectId: string): Promise<Map<string, any>> {
    ...
    const existingTemplates = await this.loadTemplatesForProject(projectId);
  ...
  static async processComponentsForImport(
    ...
    const templates = await this.ensureTemplatesExist(projectId);
    
    // Assign templates to components
    return components.map(component => {
      if (!component.milestoneTemplateId) {
        component.milestoneTemplateId = this.resolveTemplateForComponent(
          component.type,
