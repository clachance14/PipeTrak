...
// Component types that exist in our database enum
type ComponentType = 
  | 'PIPE'
  | 'SPOOL' 
  | 'VALVE'
...
export class ComponentTypeMapper {
  ...
  mapType(excelType: string): ComponentType {
    ...
    if (this.containsKeyword(normalized, ['SUPPORT', 'HANG', 'CLAMP'])) return 'SUPPORT';
    if (this.containsKeyword(normalized, ['GASKET', 'GSKT', 'SEAL'])) return 'GASKET';
    if (this.containsKeyword(normalized, ['FLANGE', 'FLG', 'BLIND'])) return 'FLANGE';
    if (this.containsKeyword(normalized, ['ELBOW', 'TEE', 'FITTING', 'REDUCER'])) return 'FITTING';
    if (this.containsKeyword(normalized, ['INSTRUMENT', 'GAUGE', 'PSV', 'TRANSMITTER'])) return 'INSTRUMENT';
  ...
  getMappingStats(excelTypes: string[]): MappingStats {
    const stats: MappingStats = {
      total: excelTypes.length,
      mapped: new Map<ComponentType, number>(),
      unknown: []
    };
  ...
  getTypeCounts(excelTypes: string[]) {
...
// Project-specific mapping overrides (for future enhancement)
interface ProjectTypeMappings {
  projectId: string;
  customMappings: Record<string, ComponentType>;
}

export class ProjectAwareTypeMapper extends ComponentTypeMapper {
  constructor(private projectMappings?: ProjectTypeMappings) {
    super();
  }
