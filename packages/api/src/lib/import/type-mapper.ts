...
export class ComponentTypeMapper {
	...
	mapType(excelType: string): ComponentType {
		...
		if (this.TYPE_MAPPINGS[normalized]) {
			console.log(
				`Exact match: "${excelType}" → ${this.TYPE_MAPPINGS[normalized]}`,
			);
		...
		for (const [pattern, dbType] of Object.entries(this.TYPE_MAPPINGS)) {
			if (normalized.includes(pattern) || pattern.includes(normalized)) {
				console.log(
					`Fuzzy matched: "${excelType}" → ${dbType} (via "${pattern}")`,
				);
				return dbType;
			}
		}
	...
	mapType(excelType: string): ComponentType {
		...
		if (this.projectMappings?.customMappings) {
			...
			if (this.projectMappings.customMappings[normalized]) {
				console.log(
					`Project-specific mapping: "${excelType}" → ${this.projectMappings.customMappings[normalized]}`,
				);
