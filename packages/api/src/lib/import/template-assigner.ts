...
import { MilestoneTemplate, MilestoneDefinition } from './types';
...
export class MilestoneTemplateAssigner {
  ...
  async initialize(projectId: string) {
    console.log(`Initializing milestone templates for project ${projectId}...`);
    
    // Get or create the two standard templates
    this.templates.full = await this.ensureTemplate(projectId, {
      name: 'Full Milestone Set',
      description: 'For pipes and spools - comprehensive milestone tracking',
      milestones: [
  ...
  private async ensureTemplate(
    ...
    return {
      ...
      milestones: Array.isArray(existing.milestones) 
        ...
        : JSON.parse(existing.milestones as string) as MilestoneDefinition[]
    };
  }
  
  getTemplateForType(componentType: ComponentType): string {
    if (!this.templates.full || !this.templates.reduced) {
      throw new Error('Templates not initialized. Call initialize() first.');
    }
    
    // Simple rule: Pipes and Spools get full milestone set, everything else gets reduced
  ...
  async createMilestonesForComponents(
    components: { id: string; milestoneTemplateId: string; workflowType: string }[],
    txClient?: any // Optional transaction client
  ) {
    console.log(`Creating milestones for ${components.length} components...`);
    ...
    for (const [templateId, templateComponents] of componentsByTemplate) {
      ...
      const milestones = Array.isArray(template.milestones)
        ...
        : JSON.parse(template.milestones as string) as MilestoneDefinition[];
      
      console.log(`Creating ${milestones.length} milestones for ${templateComponents.length} components using template ${template.name}`);
    ...
    if (allMilestonesToCreate.length > 0) {
      console.log(`Bulk creating ${allMilestonesToCreate.length} milestone records...`);
