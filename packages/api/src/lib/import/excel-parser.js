...
export function calculateTotalInstances(rows) {
...
export function detectColumnMapping(headers) {
    ...
    for (const header of headers) {
        ...
        // Drawing
        if (/^(DRAWING|DRAWINGS|DWG|DWGS|DWGNO|DWG_NO|ISO)$/i.test(normalized)) {
            mapping.drawing = header;
        }
        // Component ID
        if (/^(CMDTY\s*CODE|COMMODITY\s*CODE|COMPONENT\s*ID|TAG|PART\s*NO)$/i.test(normalized)) {
