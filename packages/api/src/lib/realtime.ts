...
export interface BroadcastEvent {
  type: 'user_presence' | 'component_edit' | 'milestone_celebration' | 'import_progress' | 'custom';
  payload: any;
  userId: string;
  timestamp: Date;
}
...
export function createRealtimeClient() {
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    console.warn('Missing Supabase environment variables - realtime features disabled');
    return null;
  }
...
export function createProjectChannel(
  ...
  if (!supabase) {
    console.warn('Supabase client is null - cannot create realtime channel');
    return null;
  }
    ...
    channel.on(
      ...
      (payload: RealtimePostgresChangesPayload<any>) => {
        const changePayload: ComponentChangePayload = {
          ...
          component_id: (payload.new as any)?.id || (payload.old as any)?.id || '',
          drawing_id: (payload.new as any)?.drawingId || (payload.old as any)?.drawingId || '',
          status: (payload.new as any)?.status || (payload.old as any)?.status || '',
          completion_percent: (payload.new as any)?.completionPercent || (payload.old as any)?.completionPercent || 0,
        };
    ...
    channel.on(
      ...
      async (payload: RealtimePostgresChangesPayload<any>) => {
        ...
        const changePayload: MilestoneChangePayload = {
          type: 'milestone',
          action: payload.eventType as any,
          component_id: (payload.new as any)?.componentId || (payload.old as any)?.componentId || '',
          milestone_name: (payload.new as any)?.milestoneName || (payload.old as any)?.milestoneName || '',
    ...
    channel.on(
      ...
      (payload: RealtimePostgresChangesPayload<any>) => {
        const changePayload: ImportJobPayload = {
          type: 'import_job',
          action: payload.eventType as any,
          job_id: (payload.new as any)?.id || (payload.old as any)?.id || '',
          status: (payload.new as any)?.status || (payload.old as any)?.status || '',
          filename: (payload.new as any)?.filename || (payload.old as any)?.filename || '',
...
export function subscribeToPresence(
  ...
  callbacks: {
    ...
    onPresenceJoin?: (key: string, currentPresences: UserPresence[], newPresences: UserPresence[]) => void;
    onPresenceLeave?: (key: string, currentPresences: UserPresence[], leftPresences: UserPresence[]) => void;
  }
) {
  ...
  if (callbacks.onPresenceJoin) {
    channel.on('presence', { event: 'join' }, ({ key, currentPresences, newPresences }) => {
      callbacks.onPresenceJoin!(key, currentPresences as unknown as UserPresence[], newPresences as unknown as UserPresence[]);
    });
  }

  if (callbacks.onPresenceLeave) {
    channel.on('presence', { event: 'leave' }, ({ key, currentPresences, leftPresences }) => {
      callbacks.onPresenceLeave!(key, currentPresences as unknown as UserPresence[], leftPresences as unknown as UserPresence[]);
    });
  }
}
...
export const conflictResolutionStrategies = {
  ...
  lastWriteWins: (conflict: ConflictData, userModifiedAt: Date) =>
    userModifiedAt > conflict.lastModified ? conflict.userValue : conflict.serverValue,
  ...
  maxValue: (conflict: ConflictData) => {
    ...
    return isNaN(userNum) || isNaN(serverNum) ? conflict.serverValue : Math.max(userNum, serverNum);
  },
  
  // For arrays, merge unique values
  mergeArrays: (conflict: ConflictData) => {
    if (Array.isArray(conflict.userValue) && Array.isArray(conflict.serverValue)) {
      return [...new Set([...conflict.userValue, ...conflict.serverValue])];
    }
...
export function checkBroadcastRateLimit(userId: string, maxPerMinute: number = 60): boolean {
  const now = Date.now();
  const key = userId;
...
export function monitorRealtimeHealth(
  ...
  channel.on('system', {}, (payload) => {
    switch (payload.status) {
      case 'ok':
        ...
        break;
      case 'error':
        callbacks.onError?.(new Error(payload.message || 'Realtime error'));
        break;
      case 'timed_out':
  ...
  if (supabase) {
    ...
    try {
      ...
      if ('onClose' in supabase && typeof supabase.onClose === 'function') {
        supabase.onClose(() => callbacks.onDisconnected?.());
      }
      if ('onError' in supabase && typeof supabase.onError === 'function') {
        supabase.onError((error: any) => callbacks.onError?.(error));
      }
    } catch (error) {
      console.warn('Some realtime connection monitoring methods are not available:', error);
    }
  }
}