	...
	try {
		...
		for (let i = 0; i < projects.length; i += batchSize) {
			...
			const batchPromises = batch.map((project) =>
				generateProjectSnapshot(supabase, project, today),
			);
			...
			batchResults.forEach((result, index) => {
				if (result.status === "fulfilled") {
					...
					errorCount++;
					console.error(
						`Failed to generate snapshot for project ${batch[index].id}:`,
						result.reason,
					);
					results.push({
						projectId: batch[index].id,
		...
		console.log(
			`Daily snapshots completed: ${successCount} success, ${errorCount} errors, ${duration}ms`,
		);
...
async function generateProjectSnapshot(
	...
	console.log(
		`Generating snapshot for project ${project.id} (${project.jobNumber})`,
	);
	...
	try {
		...
		if (existingSnapshot) {
			console.log(
				`Snapshot already exists for project ${project.id} on ${snapshotDate}`,
			);
      ...
      `)
		...
		if (componentsError) {
			throw new Error(
				`Failed to fetch components: ${componentsError.message}`,
			);
		}

		if (components.length === 0) {
			console.log(
				`No components found for project ${project.id}, skipping snapshot`,
			);
		...
		const testPackageBreakdown = generateBreakdown(
			components,
			"testPackage",
		);
		...
		if (insertError) {
			throw new Error(
				`Failed to insert snapshot: ${insertError.message}`,
			);
		}
		...
		console.log(
			`Snapshot generated for project ${project.id}: ${overallCompletionPercent}% complete`,
		);
		...
		console.error(
			`Error generating snapshot for project ${project.id}:`,
			error,
		);
		throw error;
	}
...
function generateBreakdown(components: any[], groupField: string): any[] {
	...
	return Object.entries(groups).map(
		([groupName, groupComponents]: [string, any[]]) => ({
			[groupField]: groupName,
			componentCount: groupComponents.length,
			completedCount: groupComponents.filter(
				(c) => c.status === "COMPLETED",
			).length,
			completionPercent:
				Math.round(
...
async function calculateVelocityMetrics(supabase: any, projectId: string) {
	try {
		...
		const { data: milestones, error } = await supabase.rpc(
			"get_recent_activity",
			{
		...
		if (error || !milestones?.activities) {
			console.log(
				`No milestone data available for velocity calculation: ${projectId}`,
			);
			return {
				dailyVelocity: null,
				weeklyVelocity: null,
		...
		const recentCompletions = milestones.activities.filter(
			(activity: any) =>
		...
		const recentWeekCompletions = recentCompletions.filter(
			(activity: any) =>
				new Date(activity.timestamp * 1000) >= sevenDaysAgo,
		);

		const dailyVelocity =
			Math.round((recentWeekCompletions.length / 7) * 100) / 100;
		const weeklyVelocity = recentWeekCompletions.length;

		// Calculate milestone-specific velocity
		const milestoneVelocity = recentWeekCompletions.reduce(
		...
		Object.keys(milestoneVelocity).forEach((milestone) => {
			milestoneVelocity[milestone] =
				Math.round((milestoneVelocity[milestone] / 7) * 100) / 100;
		});
		...
		console.error(
			`Velocity calculation error for project ${projectId}:`,
			error,
		);
		return {
			dailyVelocity: null,
	...
	try {
		...
		for (const project of projects.slice(0, 5)) {
			...
			try {
				...
				console.error(
					`Weekly report error for project ${project.id}:`,
					error,
				);
				results.push({
					projectId: project.id,
			...
			JSON.stringify({
				...
				data: {
					...
					successCount: results.filter((r) => r.status === "success")
						.length,
					errorCount: results.filter((r) => r.status === "error")
						.length,
					results,
				},
			}),
...
async function cleanupExpiredData(supabase: any) {
	...
	try {
		...
		const { data: deletedCache, error: cacheError } = await supabase.rpc(
		...
		console.log(
			`Cleanup completed: ${totalDeleted} records deleted, ${errorCount} errors`,
		);
...
async function performHealthCheck(supabase: any) {
	...
	try {
		...
		const { data: storageTest, error: storageError } =
			await supabase.storage.from("reports").list("", { limit: 1 });
