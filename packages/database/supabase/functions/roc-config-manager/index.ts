...
async function handleGetConfigurations(supabase: any, url: URL) {
  ...
  try {
    ...
    const enrichedConfigs = configs.map(config => ({
      ...config,
      milestoneWeights: validateAndNormalizeMilestoneWeights(config.milestoneWeights),
      totalWeight: Object.values(config.milestoneWeights || {})
        .reduce((sum: number, weight: any) => sum + (Number(weight) || 0), 0),
    ...
    if (projectId) {
      effectiveConfig = getEffectiveConfiguration(enrichedConfigs, projectId);
    }
    ...
    return new Response(
      JSON.stringify({ error: "Failed to fetch configurations", details: error.message }),
      { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  }
}
...
async function handleCreateConfiguration(supabase: any, req: Request) {
  try {
    ...
    if (!validateMilestoneWeights(body.milestoneWeights)) {
      return new Response(
        JSON.stringify({ 
          ...
          details: "Weights must sum to 100 or less and contain valid milestone names"
        }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    const normalizedWeights = validateAndNormalizeMilestoneWeights(body.milestoneWeights);
    ...
    console.log(`ROC configuration created: ${newConfig.id} for org ${body.organizationId}`);
    ...
    return new Response(
      JSON.stringify({
        ...
        data: {
          ...newConfig,
          totalWeight: Object.values(normalizedWeights)
            .reduce((sum: number, weight: any) => sum + (Number(weight) || 0), 0),
          isValid: true,
        }
      }),
    ...
    return new Response(
      JSON.stringify({ error: "Failed to create configuration", details: error.message }),
      { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  }
}

async function handleUpdateConfiguration(supabase: any, req: Request, configId: string) {
  try {
    const body = await req.json();
    ...
    if (body.milestoneWeights && !validateMilestoneWeights(body.milestoneWeights)) {
      return new Response(
        JSON.stringify({ 
          error: "Invalid milestone weights",
          details: "Weights must sum to 100 or less and contain valid milestone names"
    ...
    if (body.milestoneWeights) {
      updateData.milestoneWeights = validateAndNormalizeMilestoneWeights(body.milestoneWeights);
    }
    ...
    await supabase.channel(`org:${existingConfig.organizationId}:config`).send({
      type: "broadcast",
    ...
    return new Response(
      JSON.stringify({
        ...
        data: {
          ...updatedConfig,
          totalWeight: Object.values(updatedConfig.milestoneWeights || {})
            .reduce((sum: number, weight: any) => sum + (Number(weight) || 0), 0),
          isValid: validateMilestoneWeights(updatedConfig.milestoneWeights),
        }
      }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
...
async function handleDeleteConfiguration(supabase: any, req: Request, configId: string) {
  try {
    ...
    return new Response(
      JSON.stringify({ error: "Failed to delete configuration", details: error.message }),
      { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  }
}
...
function validateMilestoneWeights(weights: any): boolean {
  ...
  const validMilestones = ["Receive", "Erect", "Connect", "Test", "Complete", "Pressure_Test"];
  const entries = Object.entries(weights);
  
  // Check if all keys are valid milestone names
  const hasValidKeys = entries.every(([key]) => validMilestones.includes(key));
  if (!hasValidKeys) {
    return false;
  }
...
function validateAndNormalizeMilestoneWeights(weights: any): Record<string, number> {
  if (!weights || typeof weights !== "object") {
