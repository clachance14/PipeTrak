...
serve(async (req) => {
	...
	try {
		...
		const {
			...
			userId,
		} = await validateRequest(req, supabase, BulkMilestoneProcessorSchema);
		...
		const transactionId =
			...
			`bulk_${Date.now()}_${crypto.randomUUID().slice(0, 8)}`;

		console.log(
			`Processing ${updates.length} milestone updates for transaction ${transactionId}`,
		);
			...
			.update({
				...
				metadata: {
					...
					batchCount: Math.ceil(
						updates.length / (options.batchSize || BATCH_SIZE),
					),
				},
		...
		if (affectedComponents.length > 0) {
			...
			if (updateError) {
				console.error(
					"Failed to update component completion:",
					updateError,
				);
			}
		}
		...
		if (options.notify && successful > 0) {
			...
			const projectIds = [
				...new Set(components?.map((c) => c.projectId) || []),
			];
		...
		console.log(
			`Bulk processing completed: ${successful} successful, ${failed} failed, ${processingTime}ms`,
		);
