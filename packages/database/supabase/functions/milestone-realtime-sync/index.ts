...
serve(async (req) => {
  ...
  try {
    ...
    const { projectId, clientState, lastSyncTimestamp, conflictResolution = 'latest_wins' } = requestData
    ...
    const syncTimestamp = lastSyncTimestamp ? new Date(lastSyncTimestamp) : new Date(Date.now() - 24 * 60 * 60 * 1000) // 24 hours ago default
    ...
    for (const clientMilestone of clientState) {
      const serverMilestone = serverChanges?.find(
        s => s.componentId === clientMilestone.componentId && 
      ...
      const hasConflict = (
        ...
        serverMilestone.percentageComplete !== clientMilestone.percentageComplete ||
        serverMilestone.quantityComplete !== clientMilestone.quantityComplete
      )
      ...
      if (hasConflict) {
        ...
        switch (conflictResolution) {
          case 'latest_wins':
            const serverTime = new Date(serverMilestone.updatedAt || 0).getTime()
            const clientTime = new Date(clientMilestone.timestamp || 0).getTime()
            resolvedValue = clientTime > serverTime ? clientMilestone : serverMilestone
            break
            ...
            conflicts.push({
              ...
              conflict: {
                ...
                serverValue: {
                  ...
                  percentageComplete: serverMilestone.percentageComplete,
                  quantityComplete: serverMilestone.quantityComplete,
                  updatedAt: serverMilestone.updatedAt
                },
                clientValue: {
                  isCompleted: clientMilestone.isCompleted,
                  percentageComplete: clientMilestone.percentageComplete,
                  quantityComplete: clientMilestone.quantityComplete,
        ...
        if (conflictResolution !== 'manual') {
          ...
          if ('percentageComplete' in resolvedValue) {
            updateData.percentageComplete = resolvedValue.percentageComplete
            updateData.isCompleted = resolvedValue.percentageComplete >= 100
          ...
          if ('quantityComplete' in resolvedValue) {
            updateData.quantityComplete = resolvedValue.quantityComplete
    ...
    for (const resolution of resolutions) {
      ...
      if (updateError) {
        console.error(`Failed to resolve conflict for milestone ${resolution.milestoneId}:`, updateError)
      }
    }
    ...
    const { data: finalState, error: finalStateError } = await supabase
      ...
      .select(`
        ...
        updatedAt
      `)
      .in('componentId', clientState.map(c => c.componentId))
    ...
    const response = {
      ...
      status: conflicts.length > 0 ? 'conflicts_detected' : 'synchronized'
    }

    console.log(`Real-time sync completed: ${resolutions.length} automatic resolutions, ${conflicts.length} manual conflicts`)
