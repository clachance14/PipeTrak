...
import { validateRequest, BulkReportGenerationSchema } from "../_shared/validation.ts";
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Validate request
    const validation = await validateRequest(req, BulkReportGenerationSchema);
    ...
    const { projectId, reportTypes, outputFormat, deliveryMethod, options = {} } = validation.data;
    ...
    console.log(`Starting bulk report generation for project ${projectId}, batch ${batchId}`);
    ...
    for (const reportType of reportTypes) {
      ...
      if (error) {
        console.error(`Failed to create report generation record for ${reportType}:`, error);
        continue;
      }
  ...
  try {
    ...
    switch (reportType) {
      case "progress_summary":
        ...
        resultRowCount = reportData?.data?.overview?.totalComponents || 0;
        break;

      case "component_details":
        reportData = await generateComponentDetailsReport(supabase, projectId, options);
        resultRowCount = reportData?.data?.components?.length || 0;
        break;

      case "test_readiness":
        reportData = await generateTestReadinessReport(supabase, projectId);
        resultRowCount = reportData?.data?.testPackages?.length || 0;
        break;

      case "trend_analysis":
        reportData = await generateTrendAnalysisReport(supabase, projectId);
        resultRowCount = reportData?.data?.dailyProgress?.length || 0;
        break;

      case "audit_trail":
        reportData = await generateAuditTrailReport(supabase, projectId, options);
    ...
    if (outputFormat === "excel") {
      const excelBuffer = await generateExcelReport(reportData, reportType, project);
      ...
      const pdfBuffer = await generatePDFReport(reportData, reportType, project);
      downloadUrl = await uploadToStorage(
        supabase,
...
async function generateComponentDetailsReport(supabase: any, projectId: string, options: any) {
...
async function generateTestReadinessReport(supabase: any, projectId: string) {
  const { data, error } = await supabase.rpc("get_test_package_readiness_detailed", {
    p_project_id: projectId,
    p_filters: {}
  });
...
async function generateAuditTrailReport(supabase: any, projectId: string, options: any) {
...
async function generateExcelReport(reportData: any, reportType: string, project: any): Promise<Uint8Array> {
  // Simplified Excel generation - in production, use a proper Excel library
  const content = JSON.stringify(reportData, null, 2);
  return new TextEncoder().encode(content);
}

async function generateCSVReport(reportData: any, reportType: string): Promise<string> {
  // Simplified CSV generation based on report type
  if (reportType === "component_details" && reportData.data.components) {
    const headers = ["Component ID", "Type", "Status", "Completion %", "Area", "System"];
...
async function generatePDFReport(reportData: any, reportType: string, project: any): Promise<Uint8Array> {
...
async function uploadToStorage(supabase: any, fileName: string, fileBuffer: Uint8Array): Promise<string> {
...
async function combineReports(
  ...
  try {
    ...
    const combinedBuffer = new TextEncoder().encode(JSON.stringify(combinedContent, null, 2));
