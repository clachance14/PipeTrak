...
-- Migration: 20250810T1201_dashboard_functions.sql  
-- Purpose: Implement optimized dashboard aggregation functions for PipeTrak
...
BEGIN
    ...
    -- Stalled = components with milestones but no progress in specified timeframes
    WITH component_last_progress AS (
        SELECT 
            c.id,
            MAX(cm."completedAt") as last_progress_date,
            MAX(cm."updatedAt") as last_update_date
        FROM "Component" c
    ...
    stalled_analysis AS (
        SELECT 
            c.id,
            CASE 
                WHEN clp.last_progress_date IS NULL THEN clp.last_update_date
                ELSE clp.last_progress_date
            END as effective_last_date
        FROM "Component" c
        LEFT JOIN component_last_progress clp ON c.id = clp.id
        WHERE c."projectId" = p_project_id
        AND c."status" != 'COMPLETED'
        AND c."status" != 'NOT_STARTED'
    )
    SELECT 
        COUNT(*) FILTER (WHERE effective_last_date < NOW() - INTERVAL '7 days'),
        COUNT(*) FILTER (WHERE effective_last_date < NOW() - INTERVAL '14 days'),
        COUNT(*) FILTER (WHERE effective_last_date < NOW() - INTERVAL '21 days')
    INTO v_stalled_7d, v_stalled_14d, v_stalled_21d
    FROM stalled_analysis;
...
BEGIN
    ...
    -- Calculate area/system matrix with stalled component counts
    WITH area_system_stats AS (
        SELECT 
            ...
            COALESCE(ROUND(AVG("completionPercent"), 2), 0) as avg_completion_percent
        FROM "Component"
        WHERE "projectId" = p_project_id
    ...
    stalled_by_area_system AS (
        SELECT 
            ...
            COUNT(*) FILTER (WHERE last_progress < NOW() - INTERVAL '7 days') as stalled_7d,
            COUNT(*) FILTER (WHERE last_progress < NOW() - INTERVAL '14 days') as stalled_14d,
            COUNT(*) FILTER (WHERE last_progress < NOW() - INTERVAL '21 days') as stalled_21d
        FROM "Component" c
        LEFT JOIN (
            SELECT 
                cm."componentId",
                COALESCE(MAX(cm."completedAt"), MAX(cm."updatedAt")) as last_progress
            FROM "ComponentMilestone" cm
            WHERE cm."isCompleted" = true OR cm."updatedAt" > NOW() - INTERVAL '30 days'
            GROUP BY cm."componentId"
        ) lp ON c.id = lp."componentId"
        WHERE c."projectId" = p_project_id
    ...
    SELECT jsonb_agg(
        jsonb_build_object(
            ...
            'completionPercent', ass.avg_completion_percent,
...
BEGIN
    ...
    drawing_stalled AS (
        SELECT 
            d.id as drawing_id,
            COUNT(*) FILTER (WHERE last_progress < NOW() - INTERVAL '7 days') as stalled_count
        FROM "Drawing" d
        LEFT JOIN "Component" c ON d.id = c."drawingId"
        LEFT JOIN (
            SELECT 
...
BEGIN
    ...
    package_stalled AS (
        SELECT 
            ...
            COUNT(*) FILTER (WHERE last_progress < NOW() - INTERVAL '14 days') as stalled_count
        FROM "Component" c
        LEFT JOIN (
            SELECT 
                cm."componentId",
    ...
    SELECT jsonb_agg(
        jsonb_build_object(
            ...
            'packageName', ps.package_id, -- Assuming package name is the same as ID for now
...
BEGIN
    ...
    -- Get recent activity from audit logs and milestone completions
    WITH recent_activity AS (
        ...
        -- Component status changes from audit logs
        SELECT 
        ...
        FROM "AuditLog" al
        JOIN "Component" c ON al."entityId" = c.id
    ...
    SELECT jsonb_agg(
        jsonb_build_object(
            ...
            'timestamp', extract(epoch from ra.activity_timestamp),
            'userId', ra.user_id,
            'userName', COALESCE(ra.user_name, 'Unknown User'),
