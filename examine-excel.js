...
const filePath =
	"/home/clachance14/projects/PipeTrak/project-documentation/TAKEOFF - 5932.xlsx";
...
try {
	...
	if (jsonData.length > 0) {
		...
		if (quantityColumns.length === 0) {
			...
			console.log(
				"   💡 Looking for columns with numeric values that might represent quantities...",
			);
			...
			headers.forEach((header, index) => {
				...
				if (totalValues > 0 && numericCount / totalValues > 0.7) {
					console.log(
						`   Possible quantity column: "${header}" (${numericCount}/${totalValues} numeric values)`,
					);
					...
					for (
						let row = 1;
						row < Math.min(6, jsonData.length);
						row++
					) {
						if (
							jsonData[row] &&
